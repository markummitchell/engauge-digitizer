...
Building the Poppler Library, and the Libraries It Depends On, In Windows (32 Bit)
----------------------------------------------------------------------------------
Prerequisites: 1) Qt 5.8 32 bit libraries and qmake are installed

1. Install Visual Studio 2015
2. Define the critical environment variables using these lines after replacing <root>:
   >set PBBUILD32=<root>\engauge-digitizer\dev\windows\poppler-32bit-build
   >set PBLIBS32=<root>\engauge-digitizer\dev\windows\poppler-32bit-libs
...
4. Extract the following from %PBTARBALLS% into %PBBUILD32% (with version numbers recommended by 
...
8. For libpng
   ...
   c. <PBBUILD32>/libpng-1.6.34/projects/vstudio/vstudio.sln
   d. Change dropdown at top from Debug to Release
   e. Verify other dropdown at top says Win32
   f. Build zlib:
         i. Select "zlib" in Solution Explorer
        ii. Right click on "zlib" and select "Properties"
       iii. Verify Platform dropdown at top says Active(Win32)
        iv. Add to the Include Directories
            ...
            g. Select "<PBBUILD32>/zlib-1.2.8/build"
       ...
       iii. Verify Platform dropdown at top says Active(Win32)
        iv. Add to the Include Directories
         ...
         v. Add to the Library Directories
            ...
            e. Select "<PBBUILD32>/zlib-1.2.8/build/Release"
            f. Select "Select Folder" button
            g. Select "Ok" button
            h. Select "Ok" button
       xvi. Right click on "libpng" in Solution Explorer and pick "Build". This builds
            libpng-1.6.34/projects/vstudio/Release/libpng16.lib and zlib.lib. If there is a linking
            ...
            for 64 bits instead of 32 bits - so repeat the previous steps making sure to pick 32 bits.
9. For libjpeg-turbo
   a. start Qt 32 bit window for VS 2015
   b. >cd %PBBUILD32%/libjpeg-turbo-1.5.3
   c. >mkdir build
   d. >cd build
   e. (Before continuing, make sure NASM is installed and NASM.exe is on the PATH, so CMake knows 
      how to build jsimdcpu.asm)
   f. >cmake ..    (if you see the line NASM=NASM-NOTFOUND then 
   ...
   g. Start "Visual Studio 2015"
   h. File / Open / Project Solution
   i. <PBBUILD32>/libjpeg-turbo-1.5.3/build/libjpeg-turbo.sln
   j. Change dropdown at top from Debug to Release
   k. Verify other dropdown at top says Win32
   l. Select "Solution / ALL_BUILD" in Solution Explorer window
   m. Build / Build Solution (creates files in libjpeg-turbo-1.5.3/build/Release and 
...
10. For freetype
   ...
   g. Properties
   h. Build (creates freetype-2.9 / objs / Win32 / Release / freetype.lib and freetype.dll)
   i. >cd objs / win32 / Release
...
12. For libxml2 we use nmake, since the Visual Studio project file is broken. Following 
   ...
   d. >set PATH=%PBBUILD32%\libiconv-win-build-master\build-VS2015\Release:%PATH%
   e. >cd win32
   f. >cscript configure.js compiler=msvc include=%PBBUILD32%\libiconv-win-build-master\include lib=%PBBUILD32%\libiconv-win-build-master\build-vs2015\Release debug=no (this creates file config.msvc)
...
13. For fontconfig (using ShiftMediaSolution release with its support for Windows)
   ...
   d. Change dropdown at top from Debug to Release
   e. Verify other dropdown at top says x86 (not Win32 or x64)
   ...
   i. C/C++
        ...
        ii. add path to <PBBUILD32>/freetype-2.9/include
       iii. add path to <PBBUILD32>/libiconv-win-build-master/include
        iv. add path to <PBBUILD32>/libxml2-2.9.7/include
   j. VC++ Directories
         i. Library Directories
        ii. add <PBBUILD32>/libiconv-win-build-master/build-VS2015/Release
       iii. add <PBBUILD32>/freetype-2.9/objs/Win32/Release
        iv. add <PBBUILD32>/libxml2-2.9.7/win32/bin.msvc
   k. Right click on libconfig in Solution Explorer window
   l. Build (creates SMP/msvc/lib/x86/libfontconfig.lib)
14. For poppler
   a. start Qt 32 bit window for VS 2015
   b. >cd %PBBUILD32%/poppler-0.62.0
   ...
   g. following lines define the environment variables:
      >set FREETYPE_INCLUDE_DIRS="%PBBUILD32%\freetype-2.9\include; %PBBUILD32%\freetype-2.9\include\freetype"
      >set FREETYPE_LIBRARY="%PBBUILD32%\freetype-2.9\objs\Win32\Release\freetype.lib"
      >set ICONV_LIBRARY="%PBBUILD32%\libiconv-win-build-master\build-VS2015\Release\libiconv.lib"
      >set JPEG_INCLUDE_DIR="%PBBUILD32%\libjpeg-turbo-1.5.3"
      >set JPEG_LIBRARY="%PBBUILD32%\libjpeg-turbo-1.5.3\build\Release\jpeg-static.lib"
      >set LIBOPENJPEG2_INCLUDE_DIR="%PBBUILD32%\openjpeg-2.3.0\src\lib\openjp2"
      >set LIBOPENJPEG2_LIBRARY="%PBBUILD32%\openjpeg-2.3.0\build\bin\Release\openjp2.lib"
      >set PNG_INCLUDE_DIR="%PBBUILD32%\libpng-1.6.34"
      >set PNG_LIBRARY="%PBBUILD32%\libpng-1.6.34\projects\vstudio\Release\libpng16.lib"
      >set ZLIB_INCLUDE_DIR="%PBBUILD32%\zlib-1.2.8"
      >set ZLIB_LIBRARY="%PBBUILD32%\zlib-1.2.8\build\Release\zlib.lib"
   h. >cmake .. -DFREETYPE_INCLUDE_DIRS=%FREETYPE_INCLUDE_DIRS% -DFREETYPE_LIBRARY=%FREETYPE_LIBRARY% -DICONV_LIBRARIES=%ICONV_LIBRARY% -DJPEG_INCLUDE_DIR=%JPEG_INCLUDE_DIR% -DJPEG_LIBRARY=%JPEG_LIBRARY% -DLIBOPENJPEG2_INCLUDE_DIRS=%LIBOPENJPEG2_INCLUDE_DIR% -DLIBOPENJPEG2_LIBRARIES=%LIBOPENJPEG2_LIBRARY% -DPNG_PNG_INCLUDE_DIR=%PNG_INCLUDE_DIR% -DPNG_LIBRARY=%PNG_LIBRARY% -DZLIB_INCLUDE_DIR=%ZLIB_INCLUDE_DIR% -DZLIB_LIBRARY=%ZLIB_LIBRARY%
   ...
   m. Verify the other dropdown at top says Win32
   n. Right click on poppler in Solution Explorer window
       ...
       iii. Select "Include Directories"
        iv. Select "arrow down" button to the right of "Include Directories"
         v. Select "<Edit>"
        vi. Add <PBBUILD32>\freetype-2.9\include
       vii. Add <PBBUILD32>\libjpeg-turbo-1.5.3
      viii. Add <PBBUILD32>\libjpeg-turbo-1.5.3\build
        ix. Add <PBBUILD32>\libpng-1.6.34
         x. Add <PBBUILD32>\openjpeg-2.3.0\build\src\lib\openjp2
        xi. Add <PBBUILD32>\zlib-1.2.8\
       xii. Add <PBBUILD32>\zlib-1.2.8\build
...
15. Copy built files to %PBLIBS32%/lib
    >copy/Y %PBBUILD32%\freetype-2.9\objs\Win32\Release\freetype.dll         %PBLIBS32%\lib
    >copy/Y %PBBUILD32%\openjpeg-2.3.0\build\bin\Release\openjp2.dll         %PBLIBS32%\lib
    >copy/Y %PBBUILD32%\poppler-0.62.0\build\qt5\src\Release\poppler-qt5.lib %PBLIBS32%\lib
    >copy/Y %PBBUILD32%\poppler-0.62.0\build\qt5\src\Release\poppler-qt5.dll %PBLIBS32%\lib
    >copy/Y %PBBUILD32%\poppler-0.62.0\build\Release\poppler.lib             %PBLIBS32%\lib
    >copy/Y %PBBUILD32%\zlib-1.2.8\build\Release\zlib.dll                    %PBLIBS32%\lib
16. To build with poppler, make sure PBBUILD32 and PBLIB are defined then use the following lines:
    >set FFTW_HOME=%PBBUILD32%\..\unzip_fftw\lib32
    >set LOG4CPP_HOME=%PBBUILD32%\..\unzip_log4cpp\lib32
    >set POPPLER_INCLUDE=%PBLIBS32%\include
    >set POPPLER_LIB=%PBLIBS32%\lib
    >vcvarsall x86
    >cd <root>\engauge-digitizer         (after replacing <root>)
    >qmake CONFIG+=pdf engauge.pro
    >nmake
    >copy/Y %PBLIBS32%\lib\*.dll bin   (if this step is skipped then the error 
