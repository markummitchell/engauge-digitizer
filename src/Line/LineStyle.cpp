...
const ColorPalette DEFAULT_LINE_COLOR_AXES =
    COLOR_PALETTE_TRANSPARENT; // Same default color as used for PointStyle axes
                               // curve default
const ColorPalette DEFAULT_LINE_COLOR_GRAPH =
    COLOR_PALETTE_BLUE; // Same default color as used for PointStyle axes curve
                        // default
...
LineStyle::LineStyle() {
  ...
  m_width = settings.value(SETTINGS_CURVE_LINE_WIDTH, DEFAULT_LINE_WIDTH_AXES)
                .toInt();
  m_paletteColor = (ColorPalette)settings.value(SETTINGS_CURVE_LINE_COLOR,
                                                DEFAULT_LINE_COLOR_AXES)
                       .toInt();
  m_curveConnectAs =
      (CurveConnectAs)settings.value(SETTINGS_CURVE_LINE_CONNECT_AS,
                                     DEFAULT_LINE_CONNECT_AS_AXES)
          .toInt();
}

LineStyle::LineStyle(unsigned int width, ColorPalette paletteColor,
                     CurveConnectAs curveConnectAs)
    : m_width(width), m_paletteColor(paletteColor),
      m_curveConnectAs(curveConnectAs) {}

LineStyle::LineStyle(const LineStyle &other)
    : m_width(other.width()), m_paletteColor(other.paletteColor()),
...
CurveConnectAs LineStyle::curveConnectAs() const { return m_curveConnectAs; }

LineStyle LineStyle::defaultAxesCurve() {
  QSettings settings(SETTINGS_ENGAUGE, SETTINGS_DIGITIZER);
  settings.beginGroup(SETTINGS_GROUP_CURVE_AXES);
  int width = settings.value(SETTINGS_CURVE_LINE_WIDTH, DEFAULT_LINE_WIDTH_AXES)
                  .toInt();
  ...
  return LineStyle(width, color, connectAs);
}
...
LineStyle LineStyle::defaultGraphCurve(int index) {
  ...
  int width =
      settings.value(SETTINGS_CURVE_LINE_WIDTH, DEFAULT_LINE_WIDTH_GRAPH)
          .toInt();
  ColorPalette color = (ColorPalette)settings.value(SETTINGS_CURVE_LINE_COLOR,
                                                    DEFAULT_LINE_COLOR_GRAPH)
                           .toInt();
  CurveConnectAs connectAs =
      (CurveConnectAs)settings.value(SETTINGS_CURVE_LINE_CONNECT_AS,
                                     DEFAULT_LINE_CONNECT_AS_GRAPH)
          .toInt();
...
void LineStyle::loadXml(QXmlStreamReader &reader) {
  ...
  if (attributes.hasAttribute(DOCUMENT_SERIALIZE_LINE_STYLE_WIDTH) &&
    ...
    setPaletteColor(
        (ColorPalette)attributes.value(DOCUMENT_SERIALIZE_LINE_STYLE_COLOR)
            .toInt());
    setCurveConnectAs(
        (CurveConnectAs)attributes.value(
                                      DOCUMENT_SERIALIZE_LINE_STYLE_CONNECT_AS)
            .toInt());
...
ColorPalette LineStyle::paletteColor() const { return m_paletteColor; }

void LineStyle::printStream(QString indentation, QTextStream &str) const {
...
void LineStyle::saveXml(QXmlStreamWriter &writer) const {
  ...
  writer.writeAttribute(DOCUMENT_SERIALIZE_LINE_STYLE_WIDTH,
                        QString::number(m_width));
  writer.writeAttribute(DOCUMENT_SERIALIZE_LINE_STYLE_COLOR,
                        QString::number(m_paletteColor));
  writer.writeAttribute(DOCUMENT_SERIALIZE_LINE_STYLE_COLOR_STRING,
                        colorPaletteToString(m_paletteColor));
  writer.writeAttribute(DOCUMENT_SERIALIZE_LINE_STYLE_CONNECT_AS,
                        QString::number(m_curveConnectAs));
  writer.writeAttribute(DOCUMENT_SERIALIZE_LINE_STYLE_CONNECT_AS_STRING,
                        curveConnectAsToString(m_curveConnectAs));
...
void LineStyle::setWidth(int width) { m_width = width; }

unsigned int LineStyle::width() const { return m_width; }
