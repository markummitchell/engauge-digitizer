...
/// Utility class for generating ordinal numbers. For axis points, the ordinals
/// are arbitrary but
/// must be unique. For curve points, point is inserted according to its
/// position and the CurveStyle settings
class OrdinalGenerator {
public:
  OrdinalGenerator();

  /// Select ordinal just for uniqueness, since there is never a curve drawn
  /// through the axis points
