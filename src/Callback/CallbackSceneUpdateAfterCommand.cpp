...
CallbackSceneUpdateAfterCommand::CallbackSceneUpdateAfterCommand(
    ...
    : m_graphicsLinesForCurves(graphicsLinesForCurves), m_scene(scene),
      m_document(document) {}

CallbackSearchReturn
CallbackSceneUpdateAfterCommand::callback(const QString &curveName,
                                          const Point &point) {
  CallbackSearchReturn rtn = CALLBACK_SEARCH_RETURN_CONTINUE;

  m_graphicsLinesForCurves.updateAfterCommand(
      m_scene, m_document.modelCurveStyles(), curveName, point);
