...
CallbackGatherXThetaValuesFunctions::CallbackGatherXThetaValuesFunctions(
  ...
  for (itr = curvesIncluded.begin(); itr != curvesIncluded.end(); itr++) {
    ...
    if (firstCurveForGatherXTheta) {
      ...
      // We only want points belonging to the first included curve so exit this
      // loop
      break;
    }
  }
...
CallbackGatherXThetaValuesFunctions::callback(const QString &curveName,
                                              const Point &point) {
  ...
  if (m_curveNamesIncluded.contains(curveName)) {
    ...
    QPointF posGraph;
    m_transformation.transformScreenToRawGraph(point.posScreen(), posGraph);
