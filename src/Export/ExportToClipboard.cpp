...
void ExportToClipboard::exportToClipboard(
    const QStringList &selected, const Transformation &transformation,
    QTextStream &strCsv, QTextStream &strHtml, const Curve &curveAxis,
  ...
  // List of curve names. Although we do not want axis points to be exported to
  // the real
  // clipboard, leaving out the axis curve would result in axis points not
  // getting
  ...
  for (itrC = curveNames.begin(); itrC != curveNames.end(); itrC++) {
    ...
    if (curveName == AXIS_CURVE_NAME) {
      curveAxis.exportToClipboard(selectedHash, transformation, strCsv, strHtml,
                                  curvesGraphsSelected);
    } else {
      const Curve *curve = curvesGraphsAll.curveForCurveName(curveName);
      ENGAUGE_CHECK_PTR(curve);
      curve->exportToClipboard(selectedHash, transformation, strCsv, strHtml,
                               curvesGraphsSelected);
    }
  }
}
