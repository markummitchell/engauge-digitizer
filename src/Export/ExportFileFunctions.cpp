...
void ExportFileFunctions::exportAllPerLineXThetaValuesMerged(
  ...
  QVector<QVector<QString *>> yRadiusValues(curveCount,
                                            QVector<QString *>(xThetaCount));
  initializeYRadiusValues(curvesIncluded, xThetaValues, yRadiusValues);
  loadYRadiusValues(modelExportOverride, document, modelMainWindow,
                    curvesIncluded, transformation, xThetaValues,
                    yRadiusValues);

  outputXThetaYRadiusValues(modelExportOverride, document.modelCoords(),
                            modelMainWindow, curvesIncluded, xThetaValues,
                            transformation, yRadiusValues, delimiter, str);
  destroy2DArray(yRadiusValues);
}

void ExportFileFunctions::exportOnePerLineXThetaValuesMerged(
    const DocumentModelExportFormat &modelExportOverride,
    const Document &document, const MainWindowModel &modelMainWindow,
    const QStringList &curvesIncluded, const ExportValuesXOrY &xThetaValues,
    const QString &delimiter, const Transformation &transformation,
  ...
  for (itr = curvesIncluded.begin(); itr != curvesIncluded.end(); itr++) {
    ...
    insertLineSeparator(isFirst, modelExportOverride.header(), str);
    ...
    int xThetaCount = xThetaValues.count();
    QVector<QVector<QString *>> yRadiusValues(CURVE_COUNT,
                                              QVector<QString *>(xThetaCount));
    initializeYRadiusValues(curvesIncluded, xThetaValues, yRadiusValues);
    loadYRadiusValues(modelExportOverride, document, modelMainWindow,
                      curvesIncluded, transformation, xThetaValues,
...
void ExportFileFunctions::exportToFile(
  ...
  QStringList curvesIncluded = curvesToInclude(
      ...
      CONNECT_AS_FUNCTION_SMOOTH, CONNECT_AS_FUNCTION_STRAIGHT);
  ...
  CallbackGatherXThetaValuesFunctions ftor(modelExportOverride, curvesIncluded,
                                           transformation);
  Functor2wRet<const QString &, const Point &, CallbackSearchReturn>
      ftorWithCallback =
          functor_ret(ftor, &CallbackGatherXThetaValuesFunctions::callback);
  document.iterateThroughCurvesPointsGraphs(ftorWithCallback);

  ExportXThetaValuesMergedFunctions exportXTheta(
      modelExportOverride, ftor.xThetaValuesRaw(), transformation);
...
void ExportFileFunctions::initializeYRadiusValues(
...
double ExportFileFunctions::linearlyInterpolate(
    const Points &points, double xThetaValue,
  ...
  for (int ip = 0; ip < points.count(); ip++) {
    ...
    if (xThetaValue <= posGraph.x()) {
      ...
      if (ip == 0) {
        ...
        double s = (xThetaValue - posGraphBefore.x()) /
                   (posGraph.x() - posGraphBefore.x());
...
void ExportFileFunctions::loadYRadiusValues(
    const DocumentModelExportFormat &modelExportOverride,
    const Document &document, const MainWindowModel &modelMainWindow,
  ...
  for (int col = 0; col < curveCount; col++) {
    ...
    if (modelExportOverride.pointsSelectionFunctions() ==
        EXPORT_POINTS_SELECTION_FUNCTIONS_RAW) {

      // No interpolation. Raw points
      loadYRadiusValuesForCurveRaw(document.modelCoords(), modelMainWindow,
                                   points, xThetaValues, transformation,
                                   yRadiusValues[col]);
    } else {
      ...
      if (curve->curveStyle().lineStyle().curveConnectAs() ==
        ...
        loadYRadiusValuesForCurveInterpolatedSmooth(
            document.modelCoords(), modelMainWindow, points, xThetaValues,
            transformation, yRadiusValues[col]);
...
void ExportFileFunctions::loadYRadiusValuesForCurveInterpolatedSmooth(
    const DocumentModelCoords &modelCoords,
    const MainWindowModel &modelMainWindow, const Points &points,
    const ExportValuesXOrY &xThetaValues, const Transformation &transformation,
    QVector<QString *> &yRadiusValues) const {
  LOG4CPP_INFO_S((*mainCat))
      << "ExportFileFunctions::loadYRadiusValuesForCurveInterpolatedSmooth";

  // Convert screen coordinates to graph coordinates, in vectors suitable for
  // spline fitting
  vector<double> t;
  ...
  if (points.count() == 0) {
  ...
  } else if (points.count() == 1 || points.count() == 2) {
    ...
    for (int row = 0; row < xThetaValues.count(); row++) {
      ...
      format.unformattedToFormatted(xTheta, yRadius, modelCoords,
                                    modelMainWindow, dummyXThetaOut,
                                    *(yRadiusValues[row]), transformation);
    }

  } else {

    // Iteration accuracy versus number of iterations 8->256, 10->1024,
    // 12->4096. Single pixel accuracy out of
    // typical image size of 1024x1024 means around 10 iterations gives decent
    // accuracy for numbers much bigger
    // than 1. A value of 12 gave some differences in the least significant
    // figures of numbers like 10^-3 in
    // the regression tests. Toggling between 30 and 32 made no difference in
    ...
    for (int row = 0; row < xThetaValues.count(); row++) {
      ...
      double xTheta = xThetaValues.at(row);
      SplinePair splinePairFound =
          spline.findSplinePairForFunctionX(xTheta, MAX_ITERATIONS);
      double yRadius = splinePairFound.y();

      // Save y/radius value for this row into yRadiusValues, after appropriate
      // formatting
      QString dummyXThetaOut;
      format.unformattedToFormatted(xTheta, yRadius, modelCoords,
                                    modelMainWindow, dummyXThetaOut,
...
void ExportFileFunctions::loadYRadiusValuesForCurveInterpolatedStraight(
    const DocumentModelCoords &modelCoords,
    const MainWindowModel &modelMainWindow, const Points &points,
    const ExportValuesXOrY &xThetaValues, const Transformation &transformation,
    QVector<QString *> &yRadiusValues) const {
  ...
  for (int row = 0; row < xThetaValues.count(); row++) {
    ...
    double yRadius = linearlyInterpolate(points, xThetaValue, transformation);

    // Save y/radius value for this row into yRadiusValues, after appropriate
    // formatting
    QString dummyXThetaOut;
    format.unformattedToFormatted(xThetaValue, yRadius, modelCoords,
                                  modelMainWindow, dummyXThetaOut,
                                  *(yRadiusValues[row]), transformation);
  }
}

void ExportFileFunctions::loadYRadiusValuesForCurveRaw(
    const DocumentModelCoords &modelCoords,
    const MainWindowModel &modelMainWindow, const Points &points,
    const ExportValuesXOrY &xThetaValues, const Transformation &transformation,
    QVector<QString *> &yRadiusValues) const {
  ...
  // Since the curve points may be a subset of xThetaValues (in which case the
  // non-applicable xThetaValues will have
  ...
  for (int pt = 0; pt < points.count(); pt++) {
    ...
    // Find the closest point in xThetaValues. This is probably an N-squared
    // algorithm, which is less than optimial,
    ...
    for (int row = 0; row < xThetaValues.count(); row++) {
      ...
      if ((row == 0) || (separation < closestSeparation)) {
    ...
    format.unformattedToFormatted(posGraph.x(), posGraph.y(), modelCoords,
                                  modelMainWindow, dummyXThetaOut,
                                  *(yRadiusValues[rowClosest]), transformation);
  }
}

void ExportFileFunctions::outputXThetaYRadiusValues(
    const DocumentModelExportFormat &modelExportOverride,
    const DocumentModelCoords &modelCoords,
  ...
  if (modelExportOverride.header() != EXPORT_HEADER_NONE) {
    ...
    for (itrHeader = curvesIncluded.begin(); itrHeader != curvesIncluded.end();
         itrHeader++) {
  ...
  for (int row = 0; row < xThetaValuesMerged.count(); row++) {
    ...
    if (rowHasAtLeastOneYRadiusEntry(yRadiusValues, row)) {

      double xTheta = xThetaValuesMerged.at(row);

      // Output x/theta value for this row
      QString xThetaString, yRadiusString;
      format.unformattedToFormatted(xTheta, DUMMY_Y_RADIUS, modelCoords,
                                    modelMainWindow, xThetaString,
                                    yRadiusString, transformation);
      str << xThetaString;
...
bool ExportFileFunctions::rowHasAtLeastOneYRadiusEntry(
    const QVector<QVector<QString *>> &yRadiusValues, int row) const {
  bool hasEntry = false;
