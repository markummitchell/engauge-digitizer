...
ExportXThetaValuesMergedFunctions::ExportXThetaValuesMergedFunctions(
    const DocumentModelExportFormat &modelExport,
    const ValuesVectorXOrY &xThetaValuesRaw,
    const Transformation &transformation)
    : m_modelExport(modelExport), m_xThetaValuesRaw(xThetaValuesRaw),
      m_transformation(transformation) {}

void ExportXThetaValuesMergedFunctions::firstSimplestNumberLinear(
    double &xThetaFirstSimplestNumber, double &xThetaMin,
  ...
  ExportAlignLinear alignLinear(xThetaMin, xThetaMax);

  xThetaFirstSimplestNumber = alignLinear.firstSimplestNumber();
}

void ExportXThetaValuesMergedFunctions::firstSimplestNumberLog(
    double &xThetaFirstSimplestNumber, double &xThetaMin,
    double &xThetaMax) const {
  LOG4CPP_INFO_S((*mainCat))
  ...
  // Compute offset that gives the simplest numbers
  ExportAlignLog alignLog(xThetaMin, xThetaMax);
...
ExportValuesXOrY ExportXThetaValuesMergedFunctions::periodicLinear() const {
  ...
  if (m_xThetaValuesRaw.count() > 0) {
    ...
    firstSimplestNumberLinear(xThetaFirstSimplestNumber, xThetaMin, xThetaMax);

    // Assuming user picks an appropriate interval increment, numbering starting
    // at xThetaFirstSimplestNumber
    // will give nice x/theta numbers
    if (m_modelExport.pointsIntervalUnitsFunctions() ==
        EXPORT_POINTS_INTERVAL_UNITS_GRAPH) {
      return periodicLinearGraph(xThetaFirstSimplestNumber, xThetaMin,
                                 xThetaMax);
    } else {
      return periodicLinearScreen(xThetaMin, xThetaMax);
    }
  } else {
...
ExportValuesXOrY ExportXThetaValuesMergedFunctions::periodicLinearGraph(
    double xThetaFirstSimplestNumber, double xThetaMin,
    double xThetaMax) const {
  LOG4CPP_INFO_S((*mainCat))
  ...
  while (xTheta > xThetaMin) {
    xTheta -= m_modelExport.pointsIntervalFunctions(); // Go backwards until
                                                       // reaching or passing
                                                       // minimum
  }
  if (xTheta < xThetaMin) {
    values[xThetaMin] =
        true; // We passed minimum so insert point right at xThetaMin
  }

  xTheta += m_modelExport.pointsIntervalFunctions();
  while (xTheta <= xThetaMax) {
    values[xTheta] = true;
    xTheta += m_modelExport
                  .pointsIntervalFunctions(); // Insert point at a simple number
  }

  if (xTheta > xThetaMax) {
    values[xThetaMax] =
        true; // We passed maximum so insert point right at xThetaMax
  }

  return values.keys();
}

ExportValuesXOrY ExportXThetaValuesMergedFunctions::periodicLinearScreen(
    double xThetaMin, double xThetaMax) const {
  LOG4CPP_INFO_S((*mainCat))
  ...
  m_transformation.transformRawGraphToScreen(QPointF(xThetaMin, ARBITRARY_Y),
                                             posScreenFirst);
  m_transformation.transformRawGraphToScreen(QPointF(xThetaMax, ARBITRARY_Y),
                                             posScreenLast);
  double deltaScreenX = posScreenLast.x() - posScreenFirst.x();
  double deltaScreenY = posScreenLast.y() - posScreenFirst.y();
  double deltaScreen =
      qSqrt(deltaScreenX * deltaScreenX + deltaScreenY * deltaScreenY);
  ...
  if ((interval > 0) && (interval < deltaScreen)) {
    s = interval / deltaScreen;
  }

  // Example: xThetaMin=0.1 and xThetaMax=100 (points are 0.1, 1, 10, 100) with
  // s=1/3 so scale should be 10
  // which multiples 0.1 to get 1. This uses
  // s=(log(xNext)-log(xMin))/(log(xMax)-log(xMin))
...
ExportValuesXOrY ExportXThetaValuesMergedFunctions::periodicLog() const {
  ...
  double xThetaFirstSimplestNumber, xThetaMin, xThetaMax;
  firstSimplestNumberLog(xThetaFirstSimplestNumber, xThetaMin, xThetaMax);
  ...
  if (m_modelExport.pointsIntervalUnitsFunctions() ==
      EXPORT_POINTS_INTERVAL_UNITS_GRAPH) {
    return periodicLogGraph(xThetaFirstSimplestNumber, xThetaMin, xThetaMax);
  } else {
    return periodicLogScreen(xThetaMin, xThetaMax);
  }
}
...
ExportValuesXOrY
ExportXThetaValuesMergedFunctions::periodicLogScreen(double xThetaMin,
                                                     double xThetaMax) const {
  LOG4CPP_INFO_S((*mainCat))
...
ExportValuesXOrY ExportXThetaValuesMergedFunctions::xThetaValues() const {
  ...
  if (m_modelExport.pointsSelectionFunctions() ==
    ...
    if (m_modelExport.pointsIntervalFunctions() == 0) {
      ...
      bool isLinear = (m_transformation.modelCoords().coordScaleXTheta() ==
                       COORD_SCALE_LINEAR);
      if (isLinear) {
