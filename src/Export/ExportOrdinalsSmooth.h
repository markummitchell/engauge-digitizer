...
/// curve with fitted spline
class ExportOrdinalsSmooth {
public:
  /// Single constructor
  ExportOrdinalsSmooth();

  /// Load t (=ordinal) and xy (=screen position) spline pairs, without any
  /// conversion to graph coordinates
  void loadSplinePairsWithoutTransformation(const Points &points,
                                            std::vector<double> &t,
                                            std::vector<SplinePair> &xy) const;

  /// Load t (=ordinal) and xy (=screen position) spline pairs, converting
  ...
  ExportValuesOrdinal
  ordinalsAtIntervalsGraph(const std::vector<double> &t,
