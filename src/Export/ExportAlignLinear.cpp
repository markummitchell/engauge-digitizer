...
ExportAlignLinear::ExportAlignLinear(double xMin, double xMax) {
  ...
  double digitsCurrent =
      firstDigit * powerOf10; // May or may not be less than xMax
  while (digitsCurrent > xMin) {
    digitsCurrent -= powerOf10; // Go backwards until less than xMin. Required
                                // only if xMin < 0
  }
  double digitsHighestUnderXMin = digitsCurrent;
  do {
    digitsCurrent = digitsHighestUnderXMin; // Go back to highest value so far
                                            // less than xMin
    while (digitsCurrent < xMin) {
...
double ExportAlignLinear::log10(double in) const { return qLn(in) / qLn(10.0); }
