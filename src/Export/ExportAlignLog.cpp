...
ExportAlignLog::ExportAlignLog(double xMin, double xMax) {
  // Convert log numbers to linear numbers
  double xMinLog = log10(xMin);
  double xMaxLog = log10(xMax);

  ExportAlignLinear alignLinear(xMinLog, xMaxLog);
...
double ExportAlignLog::log10(double in) const { return qLn(in) / qLn(10.0); }
