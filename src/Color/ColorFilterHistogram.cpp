...
int ColorFilterHistogram::binFromPixel(const ColorFilter &filter,
  ...
  // map it to bin=1 through HISTOGRAM_BINS-2 so first and last bin are zero.
  // The
  // result is a peak at the start or end is complete and easier to read
  double s =
      filter.pixelToZeroToOneOrMinusOne(colorFilterMode, pixel, rgbBackground);
  ...
  if (s >= 0) {
    ...
    bin = FIRST_NON_EMPTY_BIN_AT_START() +
          s * (LAST_NON_EMPTY_BIN_AT_END() - FIRST_NON_EMPTY_BIN_AT_START());
  }
...
void ColorFilterHistogram::generate(const ColorFilter &filter,
  ...
  for (int x = 0; x < image.width(); x++) {
    for (int y = 0; y < image.height(); y++) {
      ...
      int bin = binFromPixel(filter, colorFilterMode, pixel, rgbBackground);
      if (bin >= 0) {
...
int ColorFilterHistogram::valueFromBin(const ColorFilter &filter,
  ...
  double s =
      (double)(bin - FIRST_NON_EMPTY_BIN_AT_START()) /
      (double)(LAST_NON_EMPTY_BIN_AT_END() - FIRST_NON_EMPTY_BIN_AT_START());
  s = qMin(qMax(s, 0.0), 1.0);

  return filter.zeroToOneToValue(colorFilterMode, s);
}
