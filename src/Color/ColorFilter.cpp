...
bool ColorFilter::colorCompare(QRgb rgb1, QRgb rgb2) const {
...
void ColorFilter::filterImage(const QImage &imageOriginal,
                              ...
                              ColorFilterMode colorFilterMode, double low,
                              double high, QRgb rgbBackground) {
  ...
  for (int x = 0; x < imageOriginal.width(); x++) {
    for (int y = 0; y < imageOriginal.height(); y++) {
      ...
      if (pixel.rgb() != rgbBackground) {
        ...
        isOn = pixelUnfilteredIsOn(colorFilterMode, pixel, rgbBackground, low,
                                   high);
      }

      imageFiltered.setPixel(
          x, y, (isOn ? QColor(Qt::black).rgb() : QColor(Qt::white).rgb()));
    }
  }
}
...
QRgb ColorFilter::marginColor(const QImage *image) const {
  ...
  for (int x = 0; x < image->width(); x++) {
    ...
    mergePixelIntoColorCounts(image->pixel(x, image->height() - 1),
                              colorCounts);
  }
  ...
  entryMax.count = 0;
  for (ColorList::const_iterator itr = colorCounts.begin();
       itr != colorCounts.end(); itr++) {
...
void ColorFilter::mergePixelIntoColorCounts(QRgb pixel,
  ...
  bool found = false;
  for (ColorList::iterator itr = colorCounts.begin(); itr != colorCounts.end();
       itr++) {
    if (colorCompare(entry.color.rgb(), (*itr).color.rgb())) {
      found = true;
      ++(*itr).count;
      break;
    }
...
bool ColorFilter::pixelFilteredIsOn(const QImage &image, int x, int y) const {
  bool rtn = false;

  if ((0 <= x) && (0 <= y) && (x < image.width()) && (y < image.height())) {

    // Pixel is on if it is closer to black than white in gray scale. This test
    // must be performed
    // on little endian and big endian systems, with or without alpha bits
    // (which are typically high bits);
    const int BLACK_WHITE_THRESHOLD =
        255 / 2; // Put threshold in middle of range
...
bool ColorFilter::pixelUnfilteredIsOn(ColorFilterMode colorFilterMode,
                                      const QColor &pixel, QRgb rgbBackground,
                                      double low0To1, double high0To1) const {
  bool rtn = false;

  double s = pixelToZeroToOneOrMinusOne(colorFilterMode, pixel, rgbBackground);
  if (s >= 0.0) {
    if (low0To1 <= high0To1) {
...
double ColorFilter::pixelToZeroToOneOrMinusOne(ColorFilterMode colorFilterMode,
  ...
  case COLOR_FILTER_MODE_FOREGROUND: {
    double distance =
        qSqrt(pow((double)pixel.red() - qRed(rgbBackground), 2) +
  ...
  case COLOR_FILTER_MODE_INTENSITY: {
    double distance =
        qSqrt(pow((double)pixel.red(), 2) + pow((double)pixel.green(), 2) +
