...
ColorFilterSettings::ColorFilterSettings()
    : m_colorFilterMode(COLOR_FILTER_MODE_INTENSITY),
      ...
      m_foregroundHigh(FOREGROUND_HIGH_DEFAULT), m_hueLow(HUE_LOW_DEFAULT),
      m_hueHigh(HUE_HIGH_DEFAULT), m_saturationLow(SATURATION_LOW_DEFAULT),
      m_saturationHigh(SATURATION_HIGH_DEFAULT), m_valueLow(VALUE_LOW_DEFAULT),
...
ColorFilterSettings::ColorFilterSettings(ColorFilterMode colorFilterMode,
                                         ...
                                         int foregroundLow, int foregroundHigh,
                                         int hueLow, int hueHigh,
                                         int saturationLow, int saturationHigh,
                                         int valueLow, int valueHigh)
    : m_colorFilterMode(colorFilterMode), m_intensityLow(intensityLow),
      m_intensityHigh(intensityHigh), m_foregroundLow(foregroundLow),
      m_foregroundHigh(foregroundHigh), m_hueLow(hueLow), m_hueHigh(hueHigh),
      m_saturationLow(saturationLow), m_saturationHigh(saturationHigh),
      m_valueLow(valueLow), m_valueHigh(valueHigh) {}
...
ColorFilterSettings::ColorFilterSettings(const ColorFilterSettings &other)
    : m_colorFilterMode(other.colorFilterMode()),
      ...
      m_foregroundHigh(other.foregroundHigh()), m_hueLow(other.hueLow()),
      m_hueHigh(other.hueHigh()), m_saturationLow(other.saturationLow()),
      m_saturationHigh(other.saturationHigh()), m_valueLow(other.valueLow()),
...
ColorFilterSettings &ColorFilterSettings::
operator=(const ColorFilterSettings &other) {
  m_colorFilterMode = other.colorFilterMode();
  m_intensityLow = other.intensityLow();
  m_intensityHigh = other.intensityHigh();
  m_foregroundLow = other.foregroundLow();
...
int ColorFilterSettings::foregroundHigh() const { return m_foregroundHigh; }

int ColorFilterSettings::foregroundLow() const { return m_foregroundLow; }
...
double ColorFilterSettings::high() const {
  ...
  case COLOR_FILTER_MODE_HUE:
    return (double)(m_hueHigh - HUE_MIN) / ((double)HUE_MAX - HUE_MIN);
  ...
  case COLOR_FILTER_MODE_VALUE:
    return (double)(m_valueHigh - VALUE_MIN) / (double)(VALUE_MAX - VALUE_MIN);

  default:
    ENGAUGE_ASSERT(false);
    return (double)(m_intensityHigh - INTENSITY_MIN) /
           (double)(INTENSITY_MAX -
                    INTENSITY_MIN); // Default when asserts are disabled
  }
}

int ColorFilterSettings::hueHigh() const { return m_hueHigh; }

int ColorFilterSettings::hueLow() const { return m_hueLow; }

int ColorFilterSettings::intensityHigh() const { return m_intensityHigh; }

int ColorFilterSettings::intensityLow() const { return m_intensityLow; }
...
void ColorFilterSettings::loadXml(QXmlStreamReader &reader) {
  ...
  if (attributes.hasAttribute(DOCUMENT_SERIALIZE_COLOR_FILTER_MODE) &&
      ...
      attributes.hasAttribute(
          DOCUMENT_SERIALIZE_COLOR_FILTER_FOREGROUND_HIGH) &&
      ...
      attributes.hasAttribute(
          DOCUMENT_SERIALIZE_COLOR_FILTER_SATURATION_HIGH) &&
    ...
    setColorFilterMode(
        (ColorFilterMode)attributes.value(DOCUMENT_SERIALIZE_COLOR_FILTER_MODE)
            .toInt());
    setIntensityLow(
        attributes.value(DOCUMENT_SERIALIZE_COLOR_FILTER_INTENSITY_LOW)
            .toInt());
    setIntensityHigh((GridCoordDisable)attributes
                         .value(DOCUMENT_SERIALIZE_COLOR_FILTER_INTENSITY_HIGH)
                         .toInt());
    setForegroundLow(
        attributes.value(DOCUMENT_SERIALIZE_COLOR_FILTER_FOREGROUND_LOW)
            .toInt());
    setForegroundHigh(
        attributes.value(DOCUMENT_SERIALIZE_COLOR_FILTER_FOREGROUND_HIGH)
            .toInt());
    setHueLow(
        attributes.value(DOCUMENT_SERIALIZE_COLOR_FILTER_HUE_LOW).toInt());
    setHueHigh(
        attributes.value(DOCUMENT_SERIALIZE_COLOR_FILTER_HUE_HIGH).toInt());
    setSaturationLow((GridCoordDisable)attributes
                         .value(DOCUMENT_SERIALIZE_COLOR_FILTER_SATURATION_LOW)
                         .toInt());
    setSaturationHigh(
        attributes.value(DOCUMENT_SERIALIZE_COLOR_FILTER_SATURATION_HIGH)
            .toInt());
    setValueLow(
        attributes.value(DOCUMENT_SERIALIZE_COLOR_FILTER_VALUE_LOW).toInt());
    setValueHigh(
        attributes.value(DOCUMENT_SERIALIZE_COLOR_FILTER_VALUE_HIGH).toInt());
...
double ColorFilterSettings::low() const {
  ...
  case COLOR_FILTER_MODE_HUE:
    return (double)(m_hueLow - HUE_MIN) / ((double)HUE_MAX - HUE_MIN);
  ...
  case COLOR_FILTER_MODE_VALUE:
    return (double)(m_valueLow - VALUE_MIN) / (double)(VALUE_MAX - VALUE_MIN);
...
int ColorFilterSettings::saturationHigh() const { return m_saturationHigh; }

int ColorFilterSettings::saturationLow() const { return m_saturationLow; }
...
void ColorFilterSettings::saveXml(QXmlStreamWriter &writer,
  ...
  writer.writeAttribute(DOCUMENT_SERIALIZE_COLOR_FILTER_MODE,
                        QString::number(m_colorFilterMode));
  writer.writeAttribute(DOCUMENT_SERIALIZE_COLOR_FILTER_MODE_STRING,
                        colorFilterModeToString(m_colorFilterMode));
  writer.writeAttribute(DOCUMENT_SERIALIZE_COLOR_FILTER_INTENSITY_LOW,
                        QString::number(m_intensityLow));
  writer.writeAttribute(DOCUMENT_SERIALIZE_COLOR_FILTER_INTENSITY_HIGH,
                        QString::number(m_intensityHigh));
  writer.writeAttribute(DOCUMENT_SERIALIZE_COLOR_FILTER_FOREGROUND_LOW,
                        QString::number(m_foregroundLow));
  writer.writeAttribute(DOCUMENT_SERIALIZE_COLOR_FILTER_FOREGROUND_HIGH,
                        QString::number(m_foregroundHigh));
  writer.writeAttribute(DOCUMENT_SERIALIZE_COLOR_FILTER_HUE_LOW,
                        QString::number(m_hueLow));
  writer.writeAttribute(DOCUMENT_SERIALIZE_COLOR_FILTER_HUE_HIGH,
                        QString::number(m_hueHigh));
  writer.writeAttribute(DOCUMENT_SERIALIZE_COLOR_FILTER_SATURATION_LOW,
                        QString::number(m_saturationLow));
  writer.writeAttribute(DOCUMENT_SERIALIZE_COLOR_FILTER_SATURATION_HIGH,
                        QString::number(m_saturationHigh));
  writer.writeAttribute(DOCUMENT_SERIALIZE_COLOR_FILTER_VALUE_LOW,
                        QString::number(m_valueLow));
  writer.writeAttribute(DOCUMENT_SERIALIZE_COLOR_FILTER_VALUE_HIGH,
                        QString::number(m_valueHigh));
...
void ColorFilterSettings::setForegroundHigh(int foregroundHigh) {
  ENGAUGE_ASSERT(FOREGROUND_MIN <= foregroundHigh &&
                 foregroundHigh <= FOREGROUND_MAX);
  m_foregroundHigh = foregroundHigh;
}

void ColorFilterSettings::setForegroundLow(int foregroundLow) {
  ENGAUGE_ASSERT(FOREGROUND_MIN <= foregroundLow &&
                 foregroundLow <= FOREGROUND_MAX);
...
void ColorFilterSettings::setHigh(double s0To1) {
  switch (m_colorFilterMode) {
  case COLOR_FILTER_MODE_FOREGROUND:
    setForegroundHigh(FOREGROUND_MIN +
                      s0To1 * (FOREGROUND_MAX - FOREGROUND_MIN));
    break;
  ...
  case COLOR_FILTER_MODE_SATURATION:
    setSaturationHigh(SATURATION_MIN +
                      s0To1 * (SATURATION_MAX - SATURATION_MIN));
    break;
...
void ColorFilterSettings::setIntensityHigh(int intensityHigh) {
  ENGAUGE_ASSERT(INTENSITY_MIN <= intensityHigh &&
                 intensityHigh <= INTENSITY_MAX);
  m_intensityHigh = intensityHigh;
}

void ColorFilterSettings::setIntensityLow(int intensityLow) {
  ENGAUGE_ASSERT(INTENSITY_MIN <= intensityLow &&
                 intensityLow <= INTENSITY_MAX);
...
void ColorFilterSettings::setLow(double s0To1) {
  switch (m_colorFilterMode) {
  case COLOR_FILTER_MODE_FOREGROUND:
    setForegroundLow(FOREGROUND_MIN +
                     s0To1 * (FOREGROUND_MAX - FOREGROUND_MIN));
    break;
  ...
  case COLOR_FILTER_MODE_SATURATION:
    setSaturationLow(SATURATION_MIN +
                     s0To1 * (SATURATION_MAX - SATURATION_MIN));
    break;
...
void ColorFilterSettings::setSaturationHigh(int saturationHigh) {
  ENGAUGE_ASSERT(SATURATION_MIN <= saturationHigh &&
                 saturationHigh <= SATURATION_MAX);
  m_saturationHigh = saturationHigh;
}

void ColorFilterSettings::setSaturationLow(int saturationLow) {
  ENGAUGE_ASSERT(SATURATION_MIN <= saturationLow &&
                 saturationLow <= SATURATION_MAX);
...
int ColorFilterSettings::valueHigh() const { return m_valueHigh; }

int ColorFilterSettings::valueLow() const { return m_valueLow; }
