...
public:
  ...
  int binFromPixel(const ColorFilter &filter, ColorFilterMode colorFilterMode,
                   const QColor &pixel, const QRgb &rgbBackground) const;

  /// Generate the histogram. The resolution is coarse since
  /// -# finer resolution is not needed
  /// -# this smooths out the curve
  void generate(const ColorFilter &filter, double histogramBins[],
                ColorFilterMode colorFilterMode, const QImage &image,
  ...
  int valueFromBin(const ColorFilter &filter, ColorFilterMode colorFilterMode,
                   int bin);

private:
  static int FIRST_NON_EMPTY_BIN_AT_START() { return 1; }
  static int LAST_NON_EMPTY_BIN_AT_END() {
    return ColorFilterHistogram::HISTOGRAM_BINS() - 2;
  }
};
