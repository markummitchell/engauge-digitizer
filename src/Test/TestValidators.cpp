...
TestValidators::TestValidators(QObject *parent) : QObject(parent) {}
...
bool TestValidators::stateDateTime(const QString &string,
  ...
  return (validator.validate(stringLocal, pos) == expectedState);
}

bool TestValidators::stateDegreesMinutesSeconds(
    const QString &string, QValidator::State expectedState) {
  int pos;
...
bool TestValidators::stateNumber(const QString &string,
  ...
  DlgValidatorNumber validator(COORD_SCALE_LOG, locale);
