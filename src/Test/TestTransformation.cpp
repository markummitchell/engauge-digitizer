...
TestTransformation::TestTransformation(QObject *parent) : QObject(parent) {}
...
void TestTransformation::initTransformation(
    const QPointF &s0, const QPointF &s1, const QPointF &s2, const QPointF &g0,
    const QPointF &g1, const QPointF &g2,
    const DocumentModelCoords &modelCoords) {
  QTransform matrixScreen(s0.x(), s1.x(), s2.x(), s0.y(), s1.y(), s2.y(), 1.0,
                          1.0, 1.0);
  QTransform matrixGraph(g0.x(), g1.x(), g2.x(), g0.y(), g1.y(), g2.y(), 1.0,
                         1.0, 1.0);

  Transformation t;
  MainWindowModel mainWindowModel;
  t.setModelCoords(modelCoords, mainWindowModel);
  t.updateTransformFromMatrices(matrixScreen, matrixGraph);

  t.transformScreenToRawGraph(s0, m_g0Transformed);
  t.transformScreenToRawGraph(s1, m_g1Transformed);
  t.transformScreenToRawGraph(s2, m_g2Transformed);
  t.transformRawGraphToScreen(g0, m_s0Transformed);
  t.transformRawGraphToScreen(g1, m_s1Transformed);
  t.transformRawGraphToScreen(g2, m_s2Transformed);
}
...
void TestTransformation::testCartesianLinearLinear() {
  ...
  initTransformation(s0, s1, s2, g0, g1, g2, modelCoordsDefault());
