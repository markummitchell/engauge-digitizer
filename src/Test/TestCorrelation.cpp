...
TestCorrelation::TestCorrelation(QObject *parent) : QObject(parent) {}
...
void TestCorrelation::loadSinusoid(double function[], int n, int center) const {
  for (int i = 0; i < n; i++) {
    int x = i - center;
    if (x == 0) {
      function[i] = 1.0;
...
void TestCorrelation::loadThreeTriangles(double function[], int n,
                                         int center) const {
...
void TestCorrelation::testShiftSinusoidNonPowerOf2() {
  ...
  correlation.correlateWithShift(N, function1, function2, binStartMax, corrMax,
                                 correlations);
