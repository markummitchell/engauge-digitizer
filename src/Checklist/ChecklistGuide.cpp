...
ChecklistGuide::ChecklistGuide(QWidget *parent)
    : QDockWidget(parent), m_browser(0) {
  setVisible(false);
  ...
  setWhatsThis(tr("Checklist Guide\n\n"
                  "This box contains a checklist of step suggested by the "
                  "Checklist Guide Wizard. Following "
                  "these steps should produce a set of digitized points in an "
                  "output file.\n\n"
                  "To run the Checklist Guide Wizard when an image file is "
                  "imported, select the "
...
void ChecklistGuide::setTemplateHtml(const QString &html,
  ...
  m_browser->setTemplateHtml(html, curveNames);
}
...
void ChecklistGuide::update(const CmdMediator &cmdMediator,
  ...
  m_browser->update(cmdMediator, documentIsExported);
}
