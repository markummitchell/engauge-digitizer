...
ChecklistGuidePage::ChecklistGuidePage(const QString &title)
    : m_row(0), m_checklineLineEditContainer(0), m_checklineLineEditLayout(0) {
  setTitle(title);
...
QRadioButton *
ChecklistGuidePage::addLabelAndRadioButton(const QString &label,
...
void ChecklistGuidePage::addLineEdit(ChecklistLineEdit *edit,
  ...
  if (m_checklineLineEditContainer == 0) {
    ...
    // This is the first ChecklistLineEdit, so we create a container for it and
    // any more that get added
    m_checklineLineEditLayout = new QVBoxLayout;
    m_checklineLineEditLayout->setSpacing(0); // This is the whole reason we
                                              // wrap the ChecklineLineEdits in
                                              // a container

    m_checklineLineEditContainer = new QWidget;
    m_checklineLineEditContainer->setLayout(m_checklineLineEditLayout);
    m_layout->addWidget(m_checklineLineEditContainer, m_row++, 0, 1, 2,
                        Qt::AlignTop);
  }
