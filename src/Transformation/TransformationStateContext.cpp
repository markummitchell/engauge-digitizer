...
TransformationStateContext::TransformationStateContext(QGraphicsScene &scene,
  ...
  m_states.insert(TRANSFORMATION_STATE_DEFINED,
                  new TransformationStateDefined(*this, scene));
  m_states.insert(TRANSFORMATION_STATE_UNDEFINED,
                  new TransformationStateUndefined(*this, scene));
  ENGAUGE_ASSERT(m_states.size() == NUM_TRANSFORMATION_STATES);

  m_currentState =
      NUM_TRANSFORMATION_STATES; // Value that forces a transition right away
}

TransformationStateContext::~TransformationStateContext() {}

bool TransformationStateContext::isGnuplot() const { return m_isGnuplot; }
...
void TransformationStateContext::triggerStateTransition(
    TransformationState transformationState, CmdMediator &cmdMediator,
    const Transformation &transformation, const QString &selectedGraphCurve) {
  ...
  if (transformationState != m_currentState) {
    ...
    if (m_currentState != NUM_TRANSFORMATION_STATES) {
      m_states[m_currentState]->end(cmdMediator, transformation);
    }

    m_currentState = transformationState;

    // Start the requested state
    m_states[m_currentState]->begin(cmdMediator, transformation,
                                    selectedGraphCurve);
  }
}

void TransformationStateContext::updateAxesChecker(
  ...
  if (m_currentState < NUM_TRANSFORMATION_STATES) {
    ...
    m_states[m_currentState]->updateAxesChecker(cmdMediator, transformation);
  }
}
