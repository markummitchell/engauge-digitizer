...
PointMatchAlgorithm::PointMatchAlgorithm(const QList<QPoint> &samplePointPixels,
                                         ...
                                         const QList<QPoint> &pointsExisting)
{
  ...
  for (itr = listCreated.begin(); itr != listCreated.end(); itr++) {
    ...
    pointsCreated.push_back (triplet);
...
double PointMatchAlgorithm::correlation(double* image,
                                        ...
                                        int sampleYExtent)
{
  ENGAUGE_ASSERT(image != 0);
  ENGAUGE_ASSERT(sample != 0);
