...
/// Algorithm returning a list of points that match the specified point. This
/// returns a list of matches, from best to worst.
...
public:
  ...
  /// Find points that match the specified sample point pixels. They are sorted
  /// by best-to-worst match
...
private:
  ...
  void allocateMemory(double **array, fftw_complex **arrayPrime, int width,
                      int height);
  ...
  void assembleLocalMaxima(double *convolution, PointMatchList &listCreated,
                           int width, int height);

  // Compute convolution in image space from phase space image and sample arrays
  void computeConvolution(fftw_complex *imagePrime, fftw_complex *samplePrime,
                          int width, int height, double **convolution,
  ...
  void dumpToGnuplot(double *convolution, int width, int height,
                     const QString &filename) const;

  // Load image and imagePrime arrays
  void loadImage(const QImage &imageProcessed,
                 const DocumentModelPointMatch &modelPointMatch,
                 const Points &pointsExisting, int width, int height,
                 double **image, fftw_complex **imagePrime);

  // Load sample and samplePrime arrays, and compute center location and extent
  void loadSample(const QList<PointMatchPixel> &samplePointPixels, int width,
                  int height, double **sample, fftw_complex **samplePrime,
                  int *sampleXCenter, int *sampleYCenter, int *sampleXExtent,
                  int *sampleYExtent);

  // Multiply corresponding elements of two matrices into a third matrix
  void multiplyMatrices(int width, int height, fftw_complex *in1,
                        fftw_complex *in2, fftw_complex *out);

  // Given an original array length, this method returns an array length that
  // includes enough padding so that the
  // array length equals 2^a * 3^b * 5^c * 7^d, which optimizes the fft
  // performance. Typical memory penalties are
  // less than 6% to get a cpu performance increase of 0% to roughly 100% or
  // 200%
  int optimizeLengthForFft(int originalLength);

  // Populate image array with processed image
  void populateImageArray(const QImage &imageProcessed, int width, int height,
                          double **image);

  // Populate sample array with sample image
  void populateSampleArray(const QList<PointMatchPixel> &samplePointPixels,
                           int width, int height, double **sample,
                           int *sampleXCenter, int *sampleYCenter,
  ...
  // Prevent duplication of existing points. this function returns the number of
  // pixels removed
  void removePixelsNearExistingPoints(double *image, int imageWidth,
                                      int imageHeight,
                                      const Points &pointsExisting,
                                      int pointSeparation);
  ...
  void scanImage(bool *sampleMaskArray, int sampleMaskWidth,
                 int sampleMaskHeight, int sampleXCenter, int sampleYCenter,
                 const DocumentModelPointMatch &modelPointMatch,
