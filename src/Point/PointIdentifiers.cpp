...
int PointIdentifiers::count() const { return m_pointIdentifiers.count(); }
...
void PointIdentifiers::loadXml(QXmlStreamReader &reader) {
  ...
  // Read through each DOCUMENT_SERIALIZE_POINT_IDENTIFIER until end of
  // DOCUMENT_SERIALIZE_POINT_IDENTIFIERS is encountered
  while (loadNextFromReader(reader)) {
    ...
    if ((reader.tokenType() == QXmlStreamReader::StartElement) &&
      ...
      if (attributes.hasAttribute(DOCUMENT_SERIALIZE_POINT_IDENTIFIER_NAME) &&
        ...
        QString valueStr =
            attributes.value(DOCUMENT_SERIALIZE_POINT_IDENTIFIER_VALUE)
                .toString();

        QString identifier =
            attributes.value(DOCUMENT_SERIALIZE_POINT_IDENTIFIER_NAME)
                .toString();
...
void PointIdentifiers::saveXml(QXmlStreamWriter &writer) const {
  ...
  for (itr = m_pointIdentifiers.begin(); itr != m_pointIdentifiers.end();
       itr++) {
    ...
    writer.writeAttribute(DOCUMENT_SERIALIZE_POINT_IDENTIFIER_VALUE,
                          value ? DOCUMENT_SERIALIZE_BOOL_TRUE
...
void PointIdentifiers::setKeyValue(const QString &pointIdentifier, bool value) {
