...
PointStyle::PointStyle(PointShape shape, unsigned int radius, int lineWidth,
                       ColorPalette paletteColor)
    : m_shape(shape), m_radius(radius), m_lineWidth(lineWidth),
      m_paletteColor(paletteColor) {}

PointStyle::PointStyle(const PointStyle &other)
    : m_shape(other.shape()), m_radius(other.radius()),
      m_lineWidth(other.lineWidth()), m_paletteColor(other.paletteColor()) {}
...
PointStyle PointStyle::defaultAxesCurve() {
  ...
  PointShape shape = (PointShape)settings.value(SETTINGS_CURVE_POINT_SHAPE,
                                                DEFAULT_POINT_SHAPE_AXIS)
                         .toInt();
  int radius =
      settings.value(SETTINGS_CURVE_POINT_RADIUS, DEFAULT_POINT_RADIUS).toInt();
  int pointLineWidth =
      settings.value(SETTINGS_CURVE_POINT_LINE_WIDTH, DEFAULT_POINT_LINE_WIDTH)
          .toInt();
  ColorPalette pointColor =
      (ColorPalette)settings.value(SETTINGS_CURVE_POINT_COLOR,
                                   DEFAULT_POINT_COLOR_AXES)
          .toInt();
  settings.endGroup();

  return PointStyle(shape, radius, pointLineWidth, pointColor);
}

PointStyle PointStyle::defaultGraphCurve(int index) {
  // Shape is always computed on the fly
  PointShape shape = POINT_SHAPE_CROSS;
  static PointShape pointShapes[] = {POINT_SHAPE_CROSS, POINT_SHAPE_X,
                                     POINT_SHAPE_DIAMOND, POINT_SHAPE_SQUARE};
...
bool PointStyle::isCircle() const { return m_shape == POINT_SHAPE_CIRCLE; }

int PointStyle::lineWidth() const { return m_lineWidth; }
...
void PointStyle::loadXml(QXmlStreamReader &reader) {
  ...
  if (attributes.hasAttribute(DOCUMENT_SERIALIZE_POINT_STYLE_RADIUS) &&
    ...
    setLineWidth(
        attributes.value(DOCUMENT_SERIALIZE_POINT_STYLE_LINE_WIDTH).toInt());
    setPaletteColor(
        (ColorPalette)attributes.value(DOCUMENT_SERIALIZE_POINT_STYLE_COLOR)
            .toInt());
    setShape((PointShape)attributes.value(DOCUMENT_SERIALIZE_POINT_STYLE_SHAPE)
                 .toInt());
...
ColorPalette PointStyle::paletteColor() const { return m_paletteColor; }
...
void PointStyle::printStream(QString indentation, QTextStream &str) const {
...
int PointStyle::radius() const { return m_radius; }
...
void PointStyle::saveXml(QXmlStreamWriter &writer) const {
  ...
  writer.writeAttribute(DOCUMENT_SERIALIZE_POINT_STYLE_RADIUS,
                        QString::number(m_radius));
  writer.writeAttribute(DOCUMENT_SERIALIZE_POINT_STYLE_LINE_WIDTH,
                        QString::number(m_lineWidth));
  writer.writeAttribute(DOCUMENT_SERIALIZE_POINT_STYLE_COLOR,
                        QString::number(m_paletteColor));
  writer.writeAttribute(DOCUMENT_SERIALIZE_POINT_STYLE_COLOR_STRING,
                        colorPaletteToString(m_paletteColor));
  writer.writeAttribute(DOCUMENT_SERIALIZE_POINT_STYLE_SHAPE,
                        QString::number(m_shape));
  writer.writeAttribute(DOCUMENT_SERIALIZE_POINT_STYLE_SHAPE_STRING,
                        pointShapeToString(m_shape));
  writer.writeEndElement();
}

void PointStyle::setLineWidth(int width) { m_lineWidth = width; }

void PointStyle::setPaletteColor(ColorPalette paletteColor) {
  m_paletteColor = paletteColor;
}

void PointStyle::setRadius(int radius) { m_radius = radius; }

void PointStyle::setShape(PointShape shape) { m_shape = shape; }

PointShape PointStyle::shape() const { return m_shape; }
