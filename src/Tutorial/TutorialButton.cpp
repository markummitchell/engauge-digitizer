...
TutorialButton::TutorialButton(const QString &text, QGraphicsScene &scene) {
  createRect(scene);
  createText(text);
}
...
void TutorialButton::createText(const QString &text) {
  // Create text. There is no need to call QGraphicsScene::addItem since it gets
  // added automatically as the
  // child of m_rect
  m_text = new TutorialButtonText(*this, text, m_rect);
  m_text->show();
}

QSize TutorialButton::size() const {
  // The size of the rectangle is not updated until later so we use the size of
  // the text
...
void TutorialButton::setGeometry(const QPoint &pos) {
  // Size the rectangle to fit the text, now that the extent of the text is
  // known, with padding on the four sides
  m_rect->setRect(pos.x(), pos.y(),
  ...
  m_text->setPos(pos.x() + m_rect->boundingRect().width() / 2.0 -
                     m_text->boundingRect().width() / 2.0,
                 pos.y() + m_rect->boundingRect().height() / 2.0 -
                     m_text->boundingRect().height() / 2.0);
}
