...
/// Context class for tutorial state machine. Each state represents one panel in
/// the tutorial
...
public:
  ...
  /// Request a transition to the specified state from the current state. A
  /// timer is used. This assumes TutorialStateContext
  /// is NOT on the stack - probably since an external event (mouse click, ...)
  /// resulted in a callback to the current state
  void requestDelayedStateTransition(TutorialState tutorialState);

  /// Request a transition to the specified state from the current state. The
  /// transition is delayed until the current state
  /// is off the stack to prevent stack corruption errors. This assumes
  /// TutorialStateContext is on the stack to finish
...
private:
  ...
  TutorialState m_requestedState; // Same as m_currentState until
