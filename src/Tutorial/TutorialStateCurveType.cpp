...
void TutorialStateCurveType::begin() {
  ...
  m_background =
      createPixmapItem(":/engauge/img/panel_lines_points.png", QPoint(0, 0));
  ...
  m_previous =
      new TutorialButton(tr("Previous"), context().tutorialDlg().scene());
  m_previous->setGeometry(
      QPoint(buttonMargin(), backgroundSize.height() - buttonMargin() -
                                 m_previous->size().height()));
  connect(m_previous, SIGNAL(signalTriggered()), this, SLOT(slotPrevious()));

  m_nextLines =
      new TutorialButton(tr("Next (Lines)"), context().tutorialDlg().scene());
  m_nextLines->setGeometry(QPoint(
      backgroundSize.width() - buttonMargin() - m_nextLines->size().width(),
      backgroundSize.height() / 3.0 - m_nextLines->size().height() / 2.0));
  connect(m_nextLines, SIGNAL(signalTriggered()), this, SLOT(slotNextCurves()));

  m_nextPoints =
      new TutorialButton(tr("Next (Points)"), context().tutorialDlg().scene());
  m_nextPoints->setGeometry(QPoint(backgroundSize.width() - buttonMargin() -
                                       m_nextPoints->size().width(),
                                   backgroundSize.height() * 2.0 / 3.0 -
                                       m_nextPoints->size().height() / 2.0));
