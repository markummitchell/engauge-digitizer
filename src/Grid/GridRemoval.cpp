...
QPointF GridRemoval::clipX(const QPointF &posUnprojected, double xBoundary,
                           const QPointF &posOther) const {
  double s =
      (xBoundary - posUnprojected.x()) / (posOther.x() - posUnprojected.x());
...
QPointF GridRemoval::clipY(const QPointF &posUnprojected, double yBoundary,
                           const QPointF &posOther) const {
  double s =
      (yBoundary - posUnprojected.y()) / (posOther.y() - posUnprojected.y());
  ENGAUGE_ASSERT((-1.0 * EPSILON < s) && (s < 1.0 + EPSILON));
...
QPixmap GridRemoval::remove(const Transformation &transformation,
  ...
  LOG4CPP_INFO_S((*mainCat))
      << "GridRemoval::remove"
      << " transformationIsDefined="
      << (transformation.transformIsDefined() ? "true" : "false")
      << " removeDefinedGridLines="
      << (modelGridRemoval.removeDefinedGridLines() ? "true" : "false");

  QImage image = imageBefore;

  // Make sure grid line removal is wanted, and possible. Otherwise all
  // processing is skipped
  if (modelGridRemoval.removeDefinedGridLines() &&
      transformation.transformIsDefined()) {

    GridHealer gridHealer(imageBefore, modelGridRemoval);
    ...
    for (int i = 0; i < modelGridRemoval.countX(); i++) {
      ...
      // Convert line between graph coordinates (xGraph,yGraphMin) and
      // (xGraph,yGraphMax) to screen coordinates
      QPointF posScreenMin, posScreenMax;
      transformation.transformRawGraphToScreen(QPointF(xGraph, yGraphMin),
                                               posScreenMin);
      transformation.transformRawGraphToScreen(QPointF(xGraph, yGraphMax),
                                               posScreenMax);

      removeLine(posScreenMin, posScreenMax, image, gridHealer);
    }
    ...
    for (int j = 0; j < modelGridRemoval.countY(); j++) {
      ...
      // Convert line between graph coordinates (xGraphMin,yGraph) and
      // (xGraphMax,yGraph) to screen coordinates
      QPointF posScreenMin, posScreenMax;
      transformation.transformRawGraphToScreen(QPointF(xGraphMin, yGraph),
                                               posScreenMin);
      transformation.transformRawGraphToScreen(QPointF(xGraphMax, yGraph),
                                               posScreenMax);
...
void GridRemoval::removeLine(const QPointF &posMin, const QPointF &posMax,
                             QImage &image, GridHealer &gridHealer) {
  ...
  // Throw away all lines that are entirely above or below or left or right to
  // the screen, since
  ...
  if (!onLeft && !onTop && !onRight && !onBottom) {
    ...
    if (pos1.x() < 0) {
      pos1 = clipX(pos1, 0, pos2);
    }
    if (pos2.x() < 0) {
      pos2 = clipX(pos2, 0, pos1);
    }
    if (pos1.y() < 0) {
      pos1 = clipY(pos1, 0, pos2);
    }
    if (pos2.y() < 0) {
      pos2 = clipY(pos2, 0, pos1);
    }
    if (pos1.x() > w) {
      pos1 = clipX(pos1, w, pos2);
    }
    if (pos2.x() > w) {
      pos2 = clipX(pos2, w, pos1);
    }
    if (pos1.y() > h) {
      pos1 = clipY(pos1, h, pos2);
    }
    if (pos2.y() > h) {
      pos2 = clipY(pos2, h, pos1);
    }
