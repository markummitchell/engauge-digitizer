...
int GridClassifier::MIN_STEP_PIXELS =
    4 * GridClassifier::PEAK_HALF_WIDTH; // Step includes down ramp, flat part,
                                         // up ramp
const QString GNUPLOT_DELIMITER("\t");

// We set up the picket fence with binStart arbitrarily set close to zero. Peak
// is
...
int GridClassifier::binFromCoordinate(double coord, double coordMin,
  ...
  int bin =
      0.5 +
      (m_numHistogramBins - 1.0) * (coord - coordMin) / (coordMax - coordMin);

  return bin;
}

void GridClassifier::classify(bool isGnuplot, const QPixmap &originalPixmap,
                              const Transformation &transformation, int &countX,
                              double &startX, double &stepX, int &countY,
                              double &startY, double &stepY) {
  ...
  computeGraphCoordinateLimits(image, transformation, xMin, xMax, yMin, yMax);
  initializeHistogramBins();
  populateHistogramBins(image, transformation, xMin, xMax, yMin, yMax);
  searchStartStepSpace(isGnuplot, m_binsX, "x", xMin, xMax, startX, stepX,
                       binStartX, binStepX);
  searchStartStepSpace(isGnuplot, m_binsY, "y", yMin, yMax, startY, stepY,
                       binStartY, binStepY);
  searchCountSpace(m_binsX, binStartX, binStepX, countX);
  searchCountSpace(m_binsY, binStartY, binStepY, countY);

  delete m_binsX;
  delete m_binsY;
}

void GridClassifier::computeGraphCoordinateLimits(
    const QImage &image, const Transformation &transformation, double &xMin,
    double &xMax, double &yMin, double &yMax) {
  LOG4CPP_INFO_S((*mainCat)) << "GridClassifier::computeGraphCoordinateLimits";

  // Project every pixel onto the x axis, and onto the y axis. One set of
  // histogram bins will be
  // set up along each of the axes. The range of bins will encompass every pixel
  // in the image, and no more.

  QPointF posGraphTL, posGraphTR, posGraphBL, posGraphBR;
  transformation.transformScreenToRawGraph(QPointF(0, 0), posGraphTL);
  transformation.transformScreenToRawGraph(QPointF(image.width(), 0),
                                           posGraphTR);
  transformation.transformScreenToRawGraph(QPointF(0, image.height()),
                                           posGraphBL);
  transformation.transformScreenToRawGraph(
      QPointF(image.width(), image.height()), posGraphBR);

  // Compute x and y ranges for setting up the histogram bins
  if (transformation.modelCoords().coordsType() == COORDS_TYPE_CARTESIAN) {

    // For affine cartesian coordinates, we only need to look at the screen
    // corners
    xMin = qMin(qMin(qMin(posGraphTL.x(), posGraphTR.x()), posGraphBL.x()),
                posGraphBR.x());
    xMax = qMax(qMax(qMax(posGraphTL.x(), posGraphTR.x()), posGraphBL.x()),
                posGraphBR.x());
    yMin = qMin(qMin(qMin(posGraphTL.y(), posGraphTR.y()), posGraphBL.y()),
                posGraphBR.y());
    yMax = qMax(qMax(qMax(posGraphTL.y(), posGraphTR.y()), posGraphBL.y()),
                posGraphBR.y());

  } else {

    // For affine polar coordinates, easiest approach is to assume the full
    // circle
    xMin = 0.0;
...
double GridClassifier::coordinateFromBin(int bin, double coordMin,
                                         double coordMax) const {
  ENGAUGE_ASSERT(bin < m_numHistogramBins);
  ENGAUGE_ASSERT(coordMin < coordMax);

  return coordMin +
         (coordMax - coordMin) * (double)bin /
...
void GridClassifier::dumpGnuplotCoordinate(const QString &coordinateLabel,
                                           double corr, const double *bins,
                                           double coordinateMin,
                                           double coordinateMax, int binStart,
                                           int binStep) const {
  QString filename =
      QString("gridclassifier_%1_corr%2_startMax%3_stepMax%4.gnuplot")
          .arg(coordinateLabel)
          .arg(corr, 8, 'f', 3, '0')
          .arg(binStart)
          .arg(binStep);
  ...
  for (bin = 0; bin < m_numHistogramBins; bin++) {
    if (bins[bin] > binCountMax) {
      binCountMax = qMax((double)binCountMax, bins[bin]);
    }
  }

  // Get picket fence
  double *picketFence = new double[m_numHistogramBins];
  loadPicketFence(picketFence, binStart, binStep, 0, false);

  // Header
  strDump << "bin" << GNUPLOT_DELIMITER << "coordinate" << GNUPLOT_DELIMITER
          << "binCount" << GNUPLOT_DELIMITER << "startStep" << GNUPLOT_DELIMITER
          << "picketFence"
          << "\n";
  ...
  for (bin = 0; bin < m_numHistogramBins; bin++) {
    ...
    double coordinate = coordinateFromBin(bin, coordinateMin, coordinateMax);
    double startStepValue(((bin - binStart) % binStep == 0) ? 1 : 0);
    strDump << bin << GNUPLOT_DELIMITER << coordinate << GNUPLOT_DELIMITER
            << bins[bin] << GNUPLOT_DELIMITER << binCountMax * startStepValue
...
void GridClassifier::dumpGnuplotCorrelations(const QString &coordinateLabel,
  ...
  QString filename =
      QString("gridclassifier_%1_correlations.gnuplot").arg(coordinateLabel);
  ...
  for (int bin = 0; bin < m_numHistogramBins; bin++) {
    ...
    strDump << coordinateFromBin(bin, valueMin, valueMax) << GNUPLOT_DELIMITER
            << signalA[bin] / signalAMax << GNUPLOT_DELIMITER
            << signalB[bin] / signalBMax << GNUPLOT_DELIMITER
            << correlations[bin] / correlationsMax << "\n";
  }
}
...
void GridClassifier::loadPicketFence(double picketFence[], int binStart,
                                     int binStep, int count,
                                     bool isCount) const {
  const double PEAK_HEIGHT = 1.0; // Arbitrary height, since normalization will
                                  // counteract any change to this value

  // Count argument is optional. Note that binStart already excludes
  // PEAK_HALF_WIDTH bins at start,
  ...
  int binStopPlusHalfWidth =
      (binStart + (count - 1) * binStep) + PEAK_HALF_WIDTH;
  ...
  for (int bin = 0; bin < m_numHistogramBins; bin++) {
    ...
    // Outside of the peaks, bin is small negative so correlation with unit
    // function is zero. In other
    // words, the function is normalized
    picketFence[bin] = normalizationOffset;

    if ((binStartMinusHalfWidth <= bin) && (bin <= binStopPlusHalfWidth)) {
      ...
      if (distanceToClosestPeak < PEAK_HALF_WIDTH) {
        ...
        picketFence[bin] = 1.0 -
                           (double)distanceToClosestPeak / PEAK_HALF_WIDTH +
                           normalizationOffset;
      }
    }
  }
    ...
    for (int y = 0; y < image.height(); y++) {
      ...
      if (!filter.colorCompare(rgbBackground, pixel.rgb())) {
        ...
        if (transformation.modelCoords().coordsType() == COORDS_TYPE_POLAR) {
          ...
          // If out of the 0 to period range, the theta value must shifted by
          // the period to get into that range
          while (posGraph.x() < xMin) {
            posGraph.setX(posGraph.x() +
                          transformation.modelCoords().thetaPeriod());
          }
...
void GridClassifier::searchCountSpace(double bins[], double binStart,
                                      double binStep, int &countMax) {
  LOG4CPP_INFO_S((*mainCat)) << "GridClassifier::searchCountSpace"
                             << " start=" << binStart << " step=" << binStep;
  ...
  for (int count = 2; count <= countStop; count++) {
    ...
    loadPicketFence(picketFence, binStart, binStep, count, true);

    correlation.correlateWithoutShift(m_numHistogramBins, bins, picketFence,
                                      corr);
...
void GridClassifier::searchStartStepSpace(bool isGnuplot, double bins[],
  ...
  // Loop though the space of possible gridlines using the independent variables
  // (start,step).
  Correlation correlation(m_numHistogramBins);
  double *picketFence = new double[m_numHistogramBins];
  int binStart;
  ...
  // We do not explicitly search(=loop) through binStart here, since
  // Correlation::correlateWithShift will take
  // care of that for us

  // Step search starts out small, and stops at value that gives count
  // substantially greater than 2. Freakishly small
  // images need to have MIN_STEP_PIXELS overridden so the loop iterates at
  // least once
  for (int binStep = qMin(MIN_STEP_PIXELS, m_numHistogramBins / 8);
       binStep < m_numHistogramBins / 4; binStep++) {

    loadPicketFence(picketFence, BIN_START_UNSHIFTED, binStep, PEAK_HALF_WIDTH,
                    false);

    correlation.correlateWithShift(m_numHistogramBins, bins, picketFence,
                                   binStart, corr, correlations);
    if (isFirst || (corr > corrMax)) {

      int binStartMaxNext =
          binStart + BIN_START_UNSHIFTED +
          1; // Compensate for the shift performed inside loadPicketFence
      ...
      if (binStartMaxNext < m_numHistogramBins) {
        ...
        // Output a gnuplot file. We should see the correlation values
        // consistently increasing
        if (isGnuplot) {

          dumpGnuplotCoordinate(coordinateLabel, corr, bins, valueMin, valueMax,
                                binStart, binStep);
        }
      }
    }
  ...
  start = coordinateFromBin(binStartMax, valueMin, valueMax);
  double next = coordinateFromBin(binStartMax + binStepMax, valueMin, valueMax);
  step = next - start;

  if (isGnuplot) {
    dumpGnuplotCorrelations(coordinateLabel, valueMin, valueMax, signalA,
                            signalB, correlationsMax);
  }
