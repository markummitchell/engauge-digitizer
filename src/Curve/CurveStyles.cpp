...
CurveStyles::CurveStyles(const CoordSystem &coordSystem) {
  ...
  m_curveStyles[AXIS_CURVE_NAME].setLineStyle(
      curveAxes.curveStyle().lineStyle());
  m_curveStyles[AXIS_CURVE_NAME].setPointStyle(
      curveAxes.curveStyle().pointStyle());
  ...
  for (itr = graphCurveNames.begin(); itr != graphCurveNames.end(); itr++) {
    ...
    m_curveStyles[graphCurveName].setLineStyle(
        graphCurve->curveStyle().lineStyle());
    m_curveStyles[graphCurveName].setPointStyle(
        graphCurve->curveStyle().pointStyle());
  }
}
...
void CurveStyles::loadXml(QXmlStreamReader &reader) {
  ...
  // Read through each DOCUMENT_SERIALIZE_LINE_STYLE and
  // DOCUMENT_SERIALIZE_POINT_STYLE until end of DOCUMENT_SERIALIZE_CURVE_STYLES
  // is encountered
...
void CurveStyles::saveXml(QXmlStreamWriter &writer) const {
  ...
  for (itr = m_curveStyles.begin(); itr != m_curveStyles.end(); itr++) {
    ...
    curveStyle.saveXml(writer, curveName);
  }
...
void CurveStyles::setLineWidth(const QString &curveName, int width) {
  ENGAUGE_ASSERT(m_curveStyles.contains(curveName));
  CurveStyle &curveStyle = m_curveStyles[curveName];
...
void CurveStyles::setPointLineWidth(const QString &curveName, int width) {
  ENGAUGE_ASSERT(m_curveStyles.contains(curveName));
  CurveStyle &curveStyle = m_curveStyles[curveName];
  curveStyle.setPointLineWidth(width);
}

void CurveStyles::setPointRadius(const QString &curveName, int radius) {
  ENGAUGE_ASSERT(m_curveStyles.contains(curveName));
  CurveStyle &curveStyle = m_curveStyles[curveName];
  curveStyle.setPointRadius(radius);
}

void CurveStyles::setPointShape(const QString &curveName, PointShape shape) {
  ENGAUGE_ASSERT(m_curveStyles.contains(curveName));
  CurveStyle &curveStyle = m_curveStyles[curveName];
