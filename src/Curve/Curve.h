...
public:
  ...
  /// Copy constructor. Copying a Curve only helps for making a copy, since
  /// access to any Points inside must be via functor.
  ...
  /// Edit the graph coordinates of an axis point. This method does not apply to
  /// a graph point
  ...
  /// Determine if specified point has just x coordinate. Otherwise has just y
  /// coordinate, or both x and y coordinates
  bool isXOnly(const QString &pointIdentifier) const;

  /// Apply functor to Points on Curve.
  void iterateThroughCurvePoints(
      const Functor2wRet<const QString &, const Point &, CallbackSearchReturn>
          &ftorWithCallback) const;

  /// Apply functor to successive Points, as line segments, on Curve. This could
  /// be a bit slow
  ...
  /// See CurveGraphs::updatePointOrdinals. Same algorithm as
  /// GraphicsLineForCurve::updatePointOrdinalsAfterDrag, although
  /// graph coordinates of points have been updated before this is called so the
  /// graph coordinates are not updated by this method
