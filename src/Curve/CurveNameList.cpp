...
bool CurveNameList::containsCurveNameCurrent(const QString &curveName) const {
  ...
  for (itr = m_modelCurvesEntries.begin(); itr != m_modelCurvesEntries.end();
       itr++) {
...
bool CurveNameList::curveNamesWillBeUnique(const QString &value,
  ...
  for (int row1 = 0; row1 < m_modelCurvesEntries.count(); row1++) {
    ...
    CurveNameListEntry curvesEntry1(
        m_modelCurvesEntries[row1]); // Retrieve entry
    QString curveNameCurrent1 =
        (row1 == row ? value : curvesEntry1.curveNameCurrent());

    for (int row2 = row1 + 1; row2 < m_modelCurvesEntries.count(); row2++) {

      // Use table entry except for the one row that gets overridden
      CurveNameListEntry curvesEntry2(
          m_modelCurvesEntries[row2]); // Retrieve entry
      QString curveNameCurrent2 =
          (row2 == row ? value : curvesEntry2.curveNameCurrent());
...
QVariant CurveNameList::data(const QModelIndex &index, int role) const {
...
Qt::ItemFlags CurveNameList::flags(const QModelIndex &index) const {
  // Only the root item can accept drops, or else dragging one entry onto
  // another
...
bool CurveNameList::insertRows(int row, int count, const QModelIndex &parent) {
  ...
  m_modelCurvesEntries.insert(row, emptyCurvesEntry.toString());
...
bool CurveNameList::removeRows(int row, int count, const QModelIndex &parent) {
  bool skip = (count != 1 || row < 0 || row > rowCount() || parent.isValid());
...
bool CurveNameList::setData(const QModelIndex &index, const QVariant &value,
                            int role) {
  ...
  if (row < m_modelCurvesEntries.count()) {
    ...
    success = true; // This method will be successful except in the rare case
                    // when a curve name is a duplicate
    ...
    if (!value.isValid() && (role == Qt::EditRole)) {
      ...
      // Modify the entry
      CurveNameListEntry curvesEntry(
          m_modelCurvesEntries[row]); // Retrieve entry

      if (index.column() == 0) {
        curvesEntry.setCurveNameCurrent(value.toString());
        success = curveNamesWillBeUnique(value.toString(), row);
      } else if (index.column() == 1) {
