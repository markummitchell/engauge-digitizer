...
QPixmap
FilterImage::filter(const QImage &imageUnfiltered,
  ...
  // Filtered image
  ColorFilter filter;
  QImage imageFiltered(imageUnfiltered.width(), imageUnfiltered.height(),
                       QImage::Format_RGB32);
  QRgb rgbBackground = filter.marginColor(&imageUnfiltered);
  filter.filterImage(imageUnfiltered, imageFiltered,
                     modelColorFilter.colorFilterMode(curveSelected),
                     modelColorFilter.low(curveSelected),
                     modelColorFilter.high(curveSelected), rgbBackground);

  GridRemoval gridRemoval;
  QPixmap pixmapFiltered =
      gridRemoval.remove(transformation, modelGridRemoval, imageFiltered);
