...
CmdEditPointAxis::CmdEditPointAxis(MainWindow &mainWindow, Document &document,
                                   const QString &pointIdentifier,
                                   const QPointF &posGraphBefore,
                                   const QPointF &posGraphAfter, bool isXOnly)
    : CmdAbstract(mainWindow, document, CMD_DESCRIPTION),
      m_pointIdentifier(pointIdentifier), m_posGraphBefore(posGraphBefore),
      m_posGraphAfter(posGraphAfter), m_isXOnly(isXOnly) {
...
CmdEditPointAxis::CmdEditPointAxis(MainWindow &mainWindow, Document &document,
  ...
  if (!attributes.hasAttribute(DOCUMENT_SERIALIZE_GRAPH_X_BEFORE) ||
    ...
    xmlExitWithError(reader, QString("%1 %2, %3, %4, %5, %6 %7 %8")
  ...
  // Boolean values
  QString isXOnlyValue =
      attributes.value(DOCUMENT_SERIALIZE_POINT_IS_X_ONLY).toString();

  m_posGraphBefore.setX(
      attributes.value(DOCUMENT_SERIALIZE_GRAPH_X_BEFORE).toDouble());
  m_posGraphBefore.setY(
      attributes.value(DOCUMENT_SERIALIZE_GRAPH_Y_BEFORE).toDouble());
  m_posGraphAfter.setX(
      attributes.value(DOCUMENT_SERIALIZE_GRAPH_X_AFTER).toDouble());
  m_posGraphAfter.setY(
      attributes.value(DOCUMENT_SERIALIZE_GRAPH_Y_AFTER).toDouble());
  m_pointIdentifier =
...
void CmdEditPointAxis::cmdRedo() {
  ...
  document().editPointAxis(m_posGraphAfter, m_pointIdentifier);
...
void CmdEditPointAxis::cmdUndo() {
  ...
  document().editPointAxis(m_posGraphBefore, m_pointIdentifier);
  document().updatePointOrdinals(mainWindow().transformation());
  mainWindow().updateAfterCommand();
}
...
void CmdEditPointAxis::saveXml(QXmlStreamWriter &writer) const {
  ...
  writer.writeAttribute(DOCUMENT_SERIALIZE_CMD_TYPE,
                        DOCUMENT_SERIALIZE_CMD_EDIT_POINT_AXIS);
  ...
  writer.writeAttribute(DOCUMENT_SERIALIZE_GRAPH_X_BEFORE,
                        QString::number(m_posGraphBefore.x()));
  writer.writeAttribute(DOCUMENT_SERIALIZE_GRAPH_Y_BEFORE,
                        QString::number(m_posGraphBefore.y()));
  writer.writeAttribute(DOCUMENT_SERIALIZE_GRAPH_X_AFTER,
                        QString::number(m_posGraphAfter.x()));
  writer.writeAttribute(DOCUMENT_SERIALIZE_GRAPH_Y_AFTER,
                        QString::number(m_posGraphAfter.y()));
