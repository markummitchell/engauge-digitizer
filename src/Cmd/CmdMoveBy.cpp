...
CmdMoveBy::CmdMoveBy(MainWindow &mainWindow, Document &document,
                     const QPointF &deltaScreen, const QString &moveText,
                     const QStringList &selectedPointIdentifiers)
    : CmdAbstract(mainWindow, document, moveText), m_deltaScreen(deltaScreen) {
  QStringList selected; // For debug
  QStringList::const_iterator itr;
  for (itr = selectedPointIdentifiers.begin();
       itr != selectedPointIdentifiers.end(); itr++) {
...
CmdMoveBy::CmdMoveBy(MainWindow &mainWindow, Document &document,
  ...
  if (!attributes.hasAttribute(DOCUMENT_SERIALIZE_SCREEN_X_DELTA) ||
    ...
    xmlExitWithError(reader, QString("%1 %2 %3 %4")
  ...
  m_deltaScreen.setX(
      attributes.value(DOCUMENT_SERIALIZE_SCREEN_X_DELTA).toDouble());
  m_deltaScreen.setY(
      attributes.value(DOCUMENT_SERIALIZE_SCREEN_Y_DELTA).toDouble());
...
void CmdMoveBy::saveXml(QXmlStreamWriter &writer) const {
  ...
  writer.writeAttribute(DOCUMENT_SERIALIZE_CMD_TYPE,
                        DOCUMENT_SERIALIZE_CMD_MOVE_BY);
  writer.writeAttribute(DOCUMENT_SERIALIZE_CMD_DESCRIPTION,
                        QUndoCommand::text());
  writer.writeAttribute(DOCUMENT_SERIALIZE_SCREEN_X_DELTA,
                        QString::number(m_deltaScreen.x()));
  writer.writeAttribute(DOCUMENT_SERIALIZE_SCREEN_Y_DELTA,
                        QString::number(m_deltaScreen.y()));
