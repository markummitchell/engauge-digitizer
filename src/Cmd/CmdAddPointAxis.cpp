...
CmdAddPointAxis::CmdAddPointAxis(MainWindow &mainWindow, Document &document,
                                 const QPointF &posScreen,
                                 const QPointF &posGraph, double ordinal,
                                 bool isXOnly)
    : CmdAbstract(mainWindow, document, CMD_DESCRIPTION),
      m_posScreen(posScreen), m_posGraph(posGraph), m_ordinal(ordinal),
      m_isXOnly(isXOnly) {
...
CmdAddPointAxis::CmdAddPointAxis(MainWindow &mainWindow, Document &document,
  ...
  if (!attributes.hasAttribute(DOCUMENT_SERIALIZE_SCREEN_X) ||
    ...
    xmlExitWithError(
        reader, QString("Missing attribute(s) %1, %2, %3, %4, %5, %6 and/or %7")
...
void CmdAddPointAxis::cmdRedo() {
  ...
  document().addPointAxisWithGeneratedIdentifier(
      m_posScreen, m_posGraph, m_identifierAdded, m_ordinal, m_isXOnly);
...
void CmdAddPointAxis::saveXml(QXmlStreamWriter &writer) const {
  ...
  writer.writeAttribute(DOCUMENT_SERIALIZE_CMD_TYPE,
                        DOCUMENT_SERIALIZE_CMD_ADD_POINT_AXIS);
  ...
  writer.writeAttribute(DOCUMENT_SERIALIZE_GRAPH_X,
                        QString::number(m_posGraph.x()));
  writer.writeAttribute(DOCUMENT_SERIALIZE_GRAPH_Y,
