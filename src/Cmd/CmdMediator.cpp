...
CmdMediator::CmdMediator(MainWindow &mainWindow, const QImage &image)
    : m_document(image) {
  LOG4CPP_INFO_S((*mainCat))
      << "CmdMediator::CmdMediator image=" << image.width() << "x"
      << image.height();

  connectSignals(mainWindow);
}

CmdMediator::CmdMediator(MainWindow &mainWindow, const QString &fileName)
    : m_document(fileName) {
  LOG4CPP_INFO_S((*mainCat)) << "CmdMediator::CmdMediator filename="
...
void CmdMediator::connectSignals(MainWindow &mainWindow) {
  connect(this, SIGNAL(cleanChanged(bool)), &mainWindow,
...
const Curve &CmdMediator::curveAxes() const { return m_document.curveAxes(); }
...
Document &CmdMediator::document() { return m_document; }

const Document &CmdMediator::document() const { return m_document; }

bool CmdMediator::isModified() const { return !isClean(); }

void CmdMediator::iterateThroughCurvePointsAxes(
...
void CmdMediator::iterateThroughCurvesPointsGraphs(
    const Functor2wRet<const QString &, const Point &, CallbackSearchReturn>
        &ftorWithCallback) {
...
bool CmdMediator::successfulRead() const { return m_document.successfulRead(); }
