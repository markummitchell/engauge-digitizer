...
void Ghosts::captureGraphicsItems(QGraphicsScene &scene) {
  ...
  for (itr = items.begin(); itr != items.end(); itr++) {
    ...
    if (itemEllipse != 0) {
      ...
      GhostEllipse ghost(itemEllipse->boundingRect(), itemEllipse->pen(),
                         itemEllipse->brush());
      ...
      if (itemPath != 0) {
        ...
        GhostPath ghost(itemPath->path(), itemPath->pen(), itemPath->brush());
        ...
        if (itemPolygon != 0) {
          ...
          GhostPolygon ghost(polygon, itemPolygon->pen(), itemPolygon->brush());
...
void Ghosts::createGhosts(QGraphicsScene &scene) {
  ...
  for (i = 0; i < m_paths.count(); i++) {
    ...
    QGraphicsPathItem *item =
        scene.addPath(ghost.path(), ghost.pen(), ghost.brush());
  ...
  for (i = 0; i < m_polygons.count(); i++) {
    ...
    QGraphicsPolygonItem *item =
        scene.addPolygon(ghost.polygon(), ghost.pen(), ghost.brush());
