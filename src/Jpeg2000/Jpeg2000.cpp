...
int Jpeg2000::getFileFormat(const char *filename) const {
  ...
  static const int format[] = {PGX_DFMT, PXM_DFMT, PXM_DFMT, PXM_DFMT,
                               BMP_DFMT, TIF_DFMT, RAW_DFMT, RAWL_DFMT,
                               TGA_DFMT, PNG_DFMT, J2K_CFMT, JP2_CFMT,
...
bool Jpeg2000::invalidFileExtension(const QString &filename) const {
  ...
  // Look for file extension in approved list. A complication is that we
  // probably want this
  ...
  for (itr = extensions.begin(); itr != extensions.end(); itr++) {
    ...
    if (QString::compare(extensionGot, extensionWanted, Qt::CaseInsensitive)) {
...
bool Jpeg2000::load(const QString &filename, QImage &imageResult) const {
  ...
  opj_stream_t *inStream =
      opj_stream_create_default_file_stream(filename.toLatin1().data(), 1);
  if (!inStream) {
    LOG4CPP_ERROR_S((*mainCat))
  ...
  if (!opj_setup_decoder(inCodec, &parameters)) {
    LOG4CPP_ERROR_S((*mainCat))
  ...
  opj_image_t *image;
  if (!opj_read_header(inStream, inCodec, &image)) {
    LOG4CPP_ERROR_S((*mainCat))
  ...
  if (!(opj_decode(inCodec, inStream, image) &&
        opj_end_decompress(inCodec, inStream))) {
...
QStringList Jpeg2000::supportedFileExtensions() const {
  ...
  // Entries from openjpeg source code, and may not be correct. Order is
  // unimportant since they are sorted later
  extensions << "j2k"
             << "jp2"
             << "jpc"
             << "jpt";
