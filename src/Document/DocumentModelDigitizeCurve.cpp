...
DocumentModelDigitizeCurve::DocumentModelDigitizeCurve(const Document &document)
    : m_cursorStandardCross(
          document.modelDigitizeCurve().cursorStandardCross()),
...
DocumentModelDigitizeCurve::DocumentModelDigitizeCurve(
    const DocumentModelDigitizeCurve &other)
...
void DocumentModelDigitizeCurve::loadXml(QXmlStreamReader &reader) {
  ...
  if (attributes.hasAttribute(
    ...
    // Boolean values
    QString standardCrossValue =
        attributes.value(
                      DOCUMENT_SERIALIZE_DIGITIZE_CURVE_CURSOR_STANDARD_CROSS)
            .toString();

    setCursorInnerRadius(
        attributes.value(DOCUMENT_SERIALIZE_DIGITIZE_CURVE_CURSOR_INNER_RADIUS)
            .toInt());
    setCursorLineWidth(
        attributes.value(DOCUMENT_SERIALIZE_DIGITIZE_CURVE_CURSOR_LINE_WIDTH)
            .toInt());
    setCursorSize(
        (CursorSize)attributes.value(
...
void DocumentModelDigitizeCurve::printStream(QString indentation,
  ...
  str << indentation
      << "cursorStandardCross=" << (m_cursorStandardCross ? "true" : "false")
      << "\n";
...
void DocumentModelDigitizeCurve::saveXml(QXmlStreamWriter &writer) const {
  ...
  writer.writeAttribute(DOCUMENT_SERIALIZE_DIGITIZE_CURVE_CURSOR_INNER_RADIUS,
                        QString::number(m_cursorInnerRadius));
  writer.writeAttribute(DOCUMENT_SERIALIZE_DIGITIZE_CURVE_CURSOR_LINE_WIDTH,
                        QString::number(m_cursorLineWidth));
  writer.writeAttribute(DOCUMENT_SERIALIZE_DIGITIZE_CURVE_CURSOR_SIZE,
                        QString::number(m_cursorSize));
  writer.writeAttribute(DOCUMENT_SERIALIZE_DIGITIZE_CURVE_CURSOR_STANDARD_CROSS,
                        m_cursorStandardCross ? DOCUMENT_SERIALIZE_BOOL_TRUE
...
void DocumentModelDigitizeCurve::setCursorStandardCross(
    bool cursorStandardCross) {
