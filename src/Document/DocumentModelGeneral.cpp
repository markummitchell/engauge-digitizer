...
DocumentModelGeneral &DocumentModelGeneral::
operator=(const DocumentModelGeneral &other) {
  m_cursorSize = other.cursorSize();
  m_extraPrecision = other.extraPrecision();
...
int DocumentModelGeneral::cursorSize() const { return m_cursorSize; }

int DocumentModelGeneral::extraPrecision() const { return m_extraPrecision; }
...
void DocumentModelGeneral::loadXml(QXmlStreamReader &reader) {
  ...
  if (attributes.hasAttribute(DOCUMENT_SERIALIZE_GENERAL_CURSOR_SIZE) &&
    ...
    setCursorSize(
        attributes.value(DOCUMENT_SERIALIZE_GENERAL_CURSOR_SIZE).toInt());
    setExtraPrecision(
        attributes.value(DOCUMENT_SERIALIZE_GENERAL_EXTRA_PRECISION).toInt());
...
void DocumentModelGeneral::saveXml(QXmlStreamWriter &writer) const {
  ...
  writer.writeAttribute(DOCUMENT_SERIALIZE_GENERAL_CURSOR_SIZE,
                        QString::number(m_cursorSize));
  writer.writeAttribute(DOCUMENT_SERIALIZE_GENERAL_EXTRA_PRECISION,
