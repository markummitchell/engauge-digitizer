...
const double DEFAULT_POINTS_INTERVAL_FUNCTIONS =
    10; // Consistent with DEFAULT_POINTS_INTERVAL_UNITS_FUNCTIONS
const double DEFAULT_POINTS_INTERVAL_RELATIONS =
    10; // Consistent with DEFAULT_POINTS_INTERVAL_UNITS_RELATIONS
const QString DEFAULT_X_LABEL("x");
const ExportPointsIntervalUnits DEFAULT_POINTS_INTERVAL_UNITS_FUNCTIONS =
    EXPORT_POINTS_INTERVAL_UNITS_SCREEN; // Consistent with
...
DocumentModelExportFormat::DocumentModelExportFormat() {
  ...
  m_curveNamesNotExported =
      settings.value(SETTINGS_EXPORT_CURVE_NAMES_NOT_EXPORTED,
                     QVariant(DEFAULT_CURVE_NAMES_NOT_EXPORTED))
          .toStringList();
  m_delimiter =
      (ExportDelimiter)settings.value(SETTINGS_EXPORT_DELIMITER,
                                      QVariant(EXPORT_DELIMITER_COMMA))
          .toInt();
  m_header = (ExportHeader)settings.value(SETTINGS_EXPORT_HEADER,
                                          QVariant(EXPORT_HEADER_SIMPLE))
                 .toInt();
  m_layoutFunctions =
      (ExportLayoutFunctions)settings.value(
                                         SETTINGS_EXPORT_LAYOUT_FUNCTIONS,
                                         QVariant(EXPORT_LAYOUT_ALL_PER_LINE))
          .toInt();
  m_pointsIntervalFunctions =
      settings.value(SETTINGS_EXPORT_POINTS_INTERVAL_FUNCTIONS,
                     QVariant(DEFAULT_POINTS_INTERVAL_FUNCTIONS))
          .toDouble();
  m_pointsIntervalRelations =
      settings.value(SETTINGS_EXPORT_POINTS_INTERVAL_RELATIONS,
                     QVariant(DEFAULT_POINTS_INTERVAL_RELATIONS))
          .toDouble();
  m_pointsIntervalUnitsFunctions =
      (ExportPointsIntervalUnits)
          settings.value(SETTINGS_EXPORT_POINTS_INTERVAL_UNITS_FUNCTIONS,
                         QVariant(DEFAULT_POINTS_INTERVAL_UNITS_FUNCTIONS))
              .toInt();
  m_pointsIntervalUnitsRelations =
      (ExportPointsIntervalUnits)
          settings.value(SETTINGS_EXPORT_POINTS_INTERVAL_UNITS_RELATIONS,
                         QVariant(DEFAULT_POINTS_INTERVAL_UNITS_RELATIONS))
              .toInt();
  m_pointsSelectionFunctions =
      (ExportPointsSelectionFunctions)settings
          .value(SETTINGS_EXPORT_POINTS_SELECTION_FUNCTIONS,
                 QVariant(
                     EXPORT_POINTS_SELECTION_FUNCTIONS_INTERPOLATE_ALL_CURVES))
          .toInt();
  m_pointsSelectionRelations =
      (ExportPointsSelectionRelations)settings
          .value(SETTINGS_EXPORT_POINTS_SELECTION_RELATIONS,
                 QVariant(EXPORT_POINTS_SELECTION_RELATIONS_INTERPOLATE))
          .toInt();
  m_xLabel = settings.value(SETTINGS_EXPORT_X_LABEL, QVariant(DEFAULT_X_LABEL))
                 .toString();
}
...
DocumentModelExportFormat::DocumentModelExportFormat(const Document &document)
    : m_curveNamesNotExported(document.modelExport().curveNamesNotExported()),
      m_pointsSelectionFunctions(
          document.modelExport().pointsSelectionFunctions()),
      m_pointsIntervalFunctions(
          document.modelExport().pointsIntervalFunctions()),
      m_pointsIntervalUnitsFunctions(
          document.modelExport().pointsIntervalUnitsFunctions()),
      m_pointsSelectionRelations(
          document.modelExport().pointsSelectionRelations()),
      m_pointsIntervalRelations(
          document.modelExport().pointsIntervalRelations()),
      m_pointsIntervalUnitsRelations(
          document.modelExport().pointsIntervalUnitsRelations()),
...
DocumentModelExportFormat::DocumentModelExportFormat(
    const DocumentModelExportFormat &other)
    : m_curveNamesNotExported(other.curveNamesNotExported()),
      ...
      m_delimiter(other.delimiter()), m_header(other.header()),
      m_xLabel(other.xLabel()) {}

DocumentModelExportFormat &DocumentModelExportFormat::
operator=(const DocumentModelExportFormat &other) {
  m_curveNamesNotExported = other.curveNamesNotExported();
  m_pointsSelectionFunctions = other.pointsSelectionFunctions();
  m_pointsIntervalFunctions = other.pointsIntervalFunctions();
  m_pointsIntervalUnitsFunctions = other.pointsIntervalUnitsFunctions();
...
ExportHeader DocumentModelExportFormat::header() const { return m_header; }
...
void DocumentModelExportFormat::loadXml(QXmlStreamReader &reader) {
  ...
  if (attributes.hasAttribute(
    ...
    setPointsSelectionFunctions(
        (ExportPointsSelectionFunctions)attributes
            .value(DOCUMENT_SERIALIZE_EXPORT_POINTS_SELECTION_FUNCTIONS)
            .toInt());
    setPointsIntervalFunctions(
        attributes.value(DOCUMENT_SERIALIZE_EXPORT_POINTS_INTERVAL_FUNCTIONS)
            .toDouble());
    setPointsIntervalUnitsFunctions(
        (ExportPointsIntervalUnits)attributes
            .value(DOCUMENT_SERIALIZE_EXPORT_POINTS_INTERVAL_UNITS_FUNCTIONS)
            .toInt());
    setPointsSelectionRelations(
        (ExportPointsSelectionRelations)
            attributes.value(DOCUMENT_SERIALIZE_COORDS_SCALE_Y_RADIUS)
                .toInt());
    setPointsIntervalRelations(
        attributes.value(DOCUMENT_SERIALIZE_EXPORT_POINTS_INTERVAL_RELATIONS)
            .toDouble());
    setPointsIntervalUnitsRelations(
        (ExportPointsIntervalUnits)attributes
            .value(DOCUMENT_SERIALIZE_EXPORT_POINTS_INTERVAL_UNITS_RELATIONS)
            .toInt());
    setLayoutFunctions((ExportLayoutFunctions)attributes
                           .value(DOCUMENT_SERIALIZE_EXPORT_LAYOUT_FUNCTIONS)
                           .toInt());
    setDelimiter(
        (ExportDelimiter)attributes.value(DOCUMENT_SERIALIZE_EXPORT_DELIMITER)
            .toInt());
    setHeader((ExportHeader)attributes.value(DOCUMENT_SERIALIZE_EXPORT_HEADER)
                  .toInt());
...
ExportPointsIntervalUnits
DocumentModelExportFormat::pointsIntervalUnitsFunctions() const {
  return m_pointsIntervalUnitsFunctions;
}

ExportPointsIntervalUnits
DocumentModelExportFormat::pointsIntervalUnitsRelations() const {
  return m_pointsIntervalUnitsRelations;
}

ExportPointsSelectionFunctions
DocumentModelExportFormat::pointsSelectionFunctions() const {
  return m_pointsSelectionFunctions;
}

ExportPointsSelectionRelations
DocumentModelExportFormat::pointsSelectionRelations() const {
...
void DocumentModelExportFormat::printStream(QString indentation,
  ...
  for (itr = m_curveNamesNotExported.begin();
       itr != m_curveNamesNotExported.end(); itr++) {
  ...
  str << indentation << "pointsIntervalUnitsRelations="
      ...
      << "\n";
  str << indentation << "exportLayoutFunctions="
      << exportLayoutFunctionsToString(m_layoutFunctions) << "\n";
  str << indentation
...
void DocumentModelExportFormat::saveXml(QXmlStreamWriter &writer) const {
  ...
  writer.writeAttribute(DOCUMENT_SERIALIZE_EXPORT_POINTS_SELECTION_FUNCTIONS,
                        QString::number(m_pointsSelectionFunctions));
  writer.writeAttribute(
      DOCUMENT_SERIALIZE_EXPORT_POINTS_SELECTION_FUNCTIONS_STRING,
      exportPointsSelectionFunctionsToString(m_pointsSelectionFunctions));
  writer.writeAttribute(DOCUMENT_SERIALIZE_EXPORT_POINTS_INTERVAL_FUNCTIONS,
                        QString::number(m_pointsIntervalFunctions));
  writer.writeAttribute(
      DOCUMENT_SERIALIZE_EXPORT_POINTS_INTERVAL_UNITS_FUNCTIONS,
      QString::number(m_pointsIntervalUnitsFunctions));
  writer.writeAttribute(DOCUMENT_SERIALIZE_EXPORT_POINTS_SELECTION_RELATIONS,
                        QString::number(m_pointsSelectionRelations));
  writer.writeAttribute(
  ...
  writer.writeAttribute(DOCUMENT_SERIALIZE_EXPORT_POINTS_INTERVAL_RELATIONS,
                        QString::number(m_pointsIntervalRelations));
  writer.writeAttribute(DOCUMENT_SERIALIZE_EXPORT_LAYOUT_FUNCTIONS,
                        QString::number(m_layoutFunctions));
  writer.writeAttribute(DOCUMENT_SERIALIZE_EXPORT_LAYOUT_FUNCTIONS_STRING,
                        exportLayoutFunctionsToString(m_layoutFunctions));
  writer.writeAttribute(DOCUMENT_SERIALIZE_EXPORT_DELIMITER,
                        QString::number(m_delimiter));
  writer.writeAttribute(DOCUMENT_SERIALIZE_EXPORT_DELIMITER_STRING,
                        exportDelimiterToString(m_delimiter));
  writer.writeAttribute(DOCUMENT_SERIALIZE_EXPORT_HEADER,
                        QString::number(m_header));
  writer.writeAttribute(DOCUMENT_SERIALIZE_EXPORT_HEADER_STRING,
                        exportHeaderToString(m_header));
...
void DocumentModelExportFormat::setCurveNamesNotExported(
    const QStringList &curveNamesNotExported) {
...
QString DocumentModelExportFormat::xLabel() const { return m_xLabel; }
