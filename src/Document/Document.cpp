...
Document::Document(const QImage &image)
  ...
  m_successfulRead =
      true; // Reading from QImage always succeeds, resulting in empty Document
    ...
    if (bytesIndicatePreVersion6(bytesStart)) {
      ...
      if (file->open(QIODevice::ReadOnly)) {
        ...
        m_reasonForUnsuccessfulRead =
            QObject::tr("Operating system says file is not readable");
      }
    } else {
      ...
      if (file->open(QIODevice::ReadOnly | QIODevice::Text)) {
        ...
        default:
          m_successfulRead = false;
          m_reasonForUnsuccessfulRead =
              QString("Engauge %1 %2 %3 %4 Engauge")
                  .arg(VERSION_NUMBER)
...
void Document::addPointAxisWithGeneratedIdentifier(const QPointF &posScreen,
  ...
  m_coordSystemContext.addPointAxisWithGeneratedIdentifier(
      posScreen, posGraph, identifier, ordinal, isXOnly);
}
...
void Document::addPointAxisWithSpecifiedIdentifier(const QPointF &posScreen,
  ...
  m_coordSystemContext.addPointAxisWithSpecifiedIdentifier(
      posScreen, posGraph, identifier, ordinal, isXOnly);
}
...
void Document::addPointGraphWithGeneratedIdentifier(const QString &curveName,
  ...
  m_coordSystemContext.addPointGraphWithGeneratedIdentifier(
      curveName, posScreen, identifier, ordinal);
}
...
void Document::addPointGraphWithSpecifiedIdentifier(const QString &curveName,
  ...
  m_coordSystemContext.addPointGraphWithSpecifiedIdentifier(
      curveName, posScreen, identifier, ordinal);
}
...
void Document::checkAddPointAxis(const QPointF &posScreen,
  ...
  m_coordSystemContext.checkAddPointAxis(posScreen, posGraph, isError,
                                         errorMessage, isXOnly);
}
...
void Document::checkEditPointAxis(const QString &pointIdentifier,
  ...
  m_coordSystemContext.checkEditPointAxis(pointIdentifier, posScreen, posGraph,
...
void Document::editPointAxis(const QPointF &posGraph,
  ...
  m_coordSystemContext.editPointAxis(posGraph, identifier);
}
...
void Document::iterateThroughCurvePointsAxes(
...
void Document::iterateThroughCurveSegments(
  ...
  m_coordSystemContext.iterateThroughCurveSegments(curveName, ftorWithCallback);
}

void Document::iterateThroughCurvesPointsGraphs(
    const Functor2wRet<const QString &, const Point &, CallbackSearchReturn>
        &ftorWithCallback) {
...
void Document::loadImage(QXmlStreamReader &reader) {
  ...
  if (reader.isCDATA()) {
    ...
    // 1) File is broken
    // 2) Bad character is in text, and NetworkClient::cleanXml did not do its
    // job
...
void Document::loadPreVersion6(QDataStream &str) {
  ...
  m_coordSystemContext.loadPreVersion6(str, version);
}
...
void Document::loadVersion6(QFile *file) {
  ...
  // If this is purely a serialized Document then we process every node under
  // the root. However, if this is an error report file
  // then we need to skip the non-Document stuff. The common solution is to skip
  // nodes outside the Document subtree using this flag
  ...
  while (!reader.atEnd() && !reader.hasError()) {
    ...
    // Special processing of DOCUMENT_SERIALIZE_IMAGE outside
    // DOCUMENT_SERIALIZE_DOCUMENT, for an error report file
    ...
    // Branching to skip non-Document nodes, with the exception of any
    // DOCUMENT_SERIALIZE_IMAGE outside DOCUMENT_SERIALIZE_DOCUMENT
    ...
    if (inDocumentSubtree) {
      ...
      if (tokenType == QXmlStreamReader::StartElement) {
        ...
        if (tag == DOCUMENT_SERIALIZE_IMAGE) {
          // A standard Document file has DOCUMENT_SERIALIZE_IMAGE inside
          // DOCUMENT_SERIALIZE_DOCUMENT, versus an error report file
          loadImage(reader);

          // Now that we have the image at the DOCUMENT_SERIALIZE_DOCUMENT
          // level, we read the rest at this level into CoordSystem
          m_coordSystemContext.loadVersion6(reader);

          // Reading of DOCUMENT_SERIALIZE_DOCUMENT has just finished, so the
  ...
  // There are already one axes curve and at least one graph curve so we do not
  // need to add any more graph curves
}
      ...
      if (attributes.hasAttribute(DOCUMENT_SERIALIZE_AXES_POINTS_REQUIRED)) {
        m_documentAxesPointsRequired =
            (DocumentAxesPointsRequired)
                attributes.value(DOCUMENT_SERIALIZE_AXES_POINTS_REQUIRED)
                    .toInt();
      } else {
        m_documentAxesPointsRequired = DOCUMENT_AXES_POINTS_REQUIRED_3;
      }
...
void Document::movePoint(const QString &pointIdentifier,
  ...
  m_coordSystemContext.movePoint(pointIdentifier, deltaScreen);
}
...
QPixmap Document::pixmap() const { return m_pixmap; }
...
void Document::printStream(QString indentation, QTextStream &str) const {
  ...
  m_coordSystemContext.printStream(indentation, str);
}
...
void Document::saveXml(QXmlStreamWriter &writer) const {
  ...
  // Version number is tacked onto DOCUMENT_SERIALIZE_DOCUMENT since the
  // alternative  (creating a new start element)
  ...
  writer.writeAttribute(DOCUMENT_SERIALIZE_AXES_POINTS_REQUIRED,
                        QString::number(m_documentAxesPointsRequired));
  ...
  // Image width and height are explicitly inserted for error reports, since the
  // CDATA is removed
  // but we still want the image size for reconstructing the error(s)
  writer.writeAttribute(DOCUMENT_SERIALIZE_IMAGE_WIDTH,
                        QString::number(img.width()));
  writer.writeAttribute(DOCUMENT_SERIALIZE_IMAGE_HEIGHT,
...
void Document::setCurvesGraphs(CoordSystemIndex coordSystemIndex,
  ...
  m_coordSystemContext.setCurvesGraphs(coordSystemIndex, curvesGraphs);
}
...
void Document::setModelAxesChecker(
...
void Document::setModelColorFilter(
...
void Document::setModelDigitizeCurve(
...
void Document::setModelGridRemoval(
...
void Document::setModelPointMatch(
...
bool Document::successfulRead() const { return m_successfulRead; }
  ...
  if (doc.setContent(file)) {
    ...
    if (nodes.count() > 0) {
      ...
      if (attributes.contains(DOCUMENT_SERIALIZE_APPLICATION_VERSION_NUMBER)) {
        ...
        version =
            (int)elem.attribute(DOCUMENT_SERIALIZE_APPLICATION_VERSION_NUMBER)
                .toDouble();
      }
    }
  }
