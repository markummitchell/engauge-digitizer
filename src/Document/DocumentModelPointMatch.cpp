...
DocumentModelPointMatch::DocumentModelPointMatch(const Document &document)
    : m_maxPointSize(document.modelPointMatch().maxPointSize()),
      ...
      m_paletteColorCandidate(
          document.modelPointMatch().paletteColorCandidate()),
      m_paletteColorRejected(
          document.modelPointMatch().paletteColorRejected()) {}

DocumentModelPointMatch::DocumentModelPointMatch(
    const DocumentModelPointMatch &other)
...
DocumentModelPointMatch &DocumentModelPointMatch::
operator=(const DocumentModelPointMatch &other) {
  m_maxPointSize = other.maxPointSize();
  m_paletteColorAccepted = other.paletteColorAccepted();
  m_paletteColorCandidate = other.paletteColorCandidate();
  m_paletteColorRejected = other.paletteColorRejected();
...
void DocumentModelPointMatch::loadXml(QXmlStreamReader &reader) {
  ...
  if (attributes.hasAttribute(DOCUMENT_SERIALIZE_POINT_MATCH_POINT_SIZE) &&
    ...
    setMaxPointSize(
        attributes.value(DOCUMENT_SERIALIZE_POINT_MATCH_POINT_SIZE).toDouble());
    setPaletteColorAccepted(
        (ColorPalette)
            attributes.value(DOCUMENT_SERIALIZE_POINT_MATCH_COLOR_ACCEPTED)
                .toInt());
    setPaletteColorCandidate(
        (ColorPalette)
            attributes.value(DOCUMENT_SERIALIZE_POINT_MATCH_COLOR_CANDIDATE)
                .toInt());
    setPaletteColorRejected(
        (ColorPalette)
            attributes.value(DOCUMENT_SERIALIZE_POINT_MATCH_COLOR_REJECTED)
                .toInt());
...
double DocumentModelPointMatch::maxPointSize() const { return m_maxPointSize; }
...
void DocumentModelPointMatch::saveXml(QXmlStreamWriter &writer) const {
  ...
  writer.writeAttribute(DOCUMENT_SERIALIZE_POINT_MATCH_POINT_SIZE,
                        QString::number(m_maxPointSize));
  writer.writeAttribute(DOCUMENT_SERIALIZE_POINT_MATCH_COLOR_ACCEPTED,
                        QString::number(m_paletteColorAccepted));
  writer.writeAttribute(DOCUMENT_SERIALIZE_POINT_MATCH_COLOR_ACCEPTED_STRING,
                        colorPaletteToString(m_paletteColorAccepted));
  writer.writeAttribute(DOCUMENT_SERIALIZE_POINT_MATCH_COLOR_CANDIDATE,
                        QString::number(m_paletteColorCandidate));
  writer.writeAttribute(DOCUMENT_SERIALIZE_POINT_MATCH_COLOR_CANDIDATE_STRING,
                        colorPaletteToString(m_paletteColorCandidate));
  writer.writeAttribute(DOCUMENT_SERIALIZE_POINT_MATCH_COLOR_REJECTED,
                        QString::number(m_paletteColorRejected));
  writer.writeAttribute(DOCUMENT_SERIALIZE_POINT_MATCH_COLOR_REJECTED_STRING,
...
void DocumentModelPointMatch::setPaletteColorAccepted(
    ColorPalette paletteColorAccepted) {
  m_paletteColorAccepted = paletteColorAccepted;
}

void DocumentModelPointMatch::setPaletteColorCandidate(
    ColorPalette paletteColorCandidate) {
  m_paletteColorCandidate = paletteColorCandidate;
}

void DocumentModelPointMatch::setPaletteColorRejected(
    ColorPalette paletteColorRejected) {
