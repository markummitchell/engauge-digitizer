...
CoordSystem::CoordSystem(DocumentAxesPointsRequired documentAxesPointsRequired)
  ...
  m_curvesGraphs.addGraphCurveAtEnd(Curve(
      curveName, ColorFilterSettings::defaultFilter(),
...
void CoordSystem::addPointAxisWithGeneratedIdentifier(const QPointF &posScreen,
                                                      ...
                                                      double ordinal,
                                                      bool isXOnly) {
  Point point(AXIS_CURVE_NAME, posScreen, posGraph, ordinal, isXOnly);
...
void CoordSystem::addPointAxisWithSpecifiedIdentifier(const QPointF &posScreen,
                                                      ...
                                                      double ordinal,
                                                      bool isXOnly) {
  Point point(AXIS_CURVE_NAME, identifier, posScreen, posGraph, ordinal,
              isXOnly);
  m_curveAxes->addPoint(point);
...
void CoordSystem::addPointGraphWithGeneratedIdentifier(const QString &curveName,
                                                       ...
                                                       QString &identifier,
                                                       double ordinal) {
  Point point(curveName, posScreen, ordinal);
...
void CoordSystem::addPointGraphWithSpecifiedIdentifier(
    const QString &curveName, const QPointF &posScreen,
    const QString &identifier, double ordinal) {
  Point point(curveName, identifier, posScreen, ordinal);
  m_curvesGraphs.addPoint(point);
...
void CoordSystem::addPointsInCurvesGraphs(CurvesGraphs &curvesGraphs) {
  ...
  Functor2wRet<const QString &, const Point &, CallbackSearchReturn>
      ftorWithCallback =
          functor_ret(ftor, &CallbackAddPointsInCurvesGraphs::callback);
...
void CoordSystem::checkAddPointAxis(const QPointF &posScreen,
  ...
  CallbackCheckAddPointAxis ftor(m_modelCoords, posScreen, posGraph,
                                 m_documentAxesPointsRequired, isXOnly);

  Functor2wRet<const QString &, const Point &, CallbackSearchReturn>
      ftorWithCallback =
          functor_ret(ftor, &CallbackCheckAddPointAxis::callback);
...
void CoordSystem::checkEditPointAxis(const QString &pointIdentifier,
  ...
  CallbackCheckEditPointAxis ftor(m_modelCoords, pointIdentifier, posScreen,
                                  posGraph, m_documentAxesPointsRequired);

  Functor2wRet<const QString &, const Point &, CallbackSearchReturn>
      ftorWithCallback =
          functor_ret(ftor, &CallbackCheckEditPointAxis::callback);
  m_curveAxes->iterateThroughCurvePoints(ftorWithCallback);
...
const CurvesGraphs &CoordSystem::curvesGraphs() const { return m_curvesGraphs; }
...
void CoordSystem::editPointAxis(const QPointF &posGraph,
  ...
  LOG4CPP_INFO_S((*mainCat)) << "CoordSystem::editPointAxis posGraph=("
                             << posGraph.x() << ", " << posGraph.y()
                             << ") identifier=" << identifier.toLatin1().data();

  m_curveAxes->editPoint(posGraph, identifier);
}
...
void CoordSystem::iterateThroughCurvePointsAxes(
...
void CoordSystem::iterateThroughCurveSegments(
  ...
  if (curveName == AXIS_CURVE_NAME) {
    ...
    m_curvesGraphs.iterateThroughCurveSegments(curveName, ftorWithCallback);
  }
}

void CoordSystem::iterateThroughCurvesPointsGraphs(
    const Functor2wRet<const QString &, const Point &, CallbackSearchReturn>
        &ftorWithCallback) {
  ENGAUGE_CHECK_PTR(m_curveAxes);
...
void CoordSystem::loadPreVersion6(QDataStream &str, double version) {
  ...
  str >> int32;
  m_modelExport.setPointsSelectionFunctions(
      (ExportPointsSelectionFunctions)int32);
  m_modelExport.setPointsIntervalUnitsRelations(
      (ExportPointsIntervalUnits)int32);
  str >> int32;
  ...
  // Information from curves and points can affect some data structures that
  // were (mostly) set earlier
...
void CoordSystem::loadVersion6(QXmlStreamReader &reader) {
  ...
  // Import from xml. Loop to end of data or error condition occurs, whichever
  // is first
  while (!reader.atEnd() && !reader.hasError()) {
    ...
    if (tokenType == QXmlStreamReader::StartElement) {
      ...
      if (tag == DOCUMENT_SERIALIZE_AXES_CHECKER) {
      ...
      } else if (tag == DOCUMENT_SERIALIZE_GENERAL ||
                 tag == DOCUMENT_SERIALIZE_COMMON) {
        ...
        ENGAUGE_ASSERT(false); // The image should have been read before this
                               // method was called
  ...
  while (!reader.atEnd() && !reader.hasError()) {
    ...
    if (tokenType == QXmlStreamReader::StartElement) {
      ...
      if (tag == DOCUMENT_SERIALIZE_AXES_CHECKER) {
        ...
        m_successfulRead = false;
        m_reasonForUnsuccessfulRead =
            QString("Unexpected xml token '%1' encountered").arg(tag);
        break;
      }
    }
  }
...
DocumentModelCoords CoordSystem::modelCoords() const { return m_modelCoords; }
...
void CoordSystem::movePoint(const QString &pointIdentifier,
  ...
  curve->movePoint(pointIdentifier, deltaScreen);
}
...
int CoordSystem::nextOrdinalForCurve(const QString &curveName) const {
  ...
  Functor2wRet<const QString &, const Point &, CallbackSearchReturn>
      ftorWithCallback = functor_ret(ftor, &CallbackNextOrdinal::callback);
...
void CoordSystem::printStream(QString indentation, QTextStream &str) const {
  str << indentation << "Graph\n";

  indentation += INDENTATION_DELTA;

  //  str << indentation << "name=" << m_name << "\n";
  //  str << indentation << "pixmap=" << m_pixmap.width() << "x" <<
  //  m_pixmap.height() << "\n";

  m_curveAxes->printStream(indentation, str);
  m_curvesGraphs.printStream(indentation, str);

  m_modelAxesChecker.printStream(indentation, str);
  m_modelCoords.printStream(indentation, str);
  m_modelDigitizeCurve.printStream(indentation, str);
  m_modelExport.printStream(indentation, str);
  m_modelGeneral.printStream(indentation, str);
  m_modelGridRemoval.printStream(indentation, str);
  m_modelPointMatch.printStream(indentation, str);
  m_modelSegments.printStream(indentation, str);
}
...
void CoordSystem::removePointAxis(const QString &identifier) {
  LOG4CPP_INFO_S((*mainCat)) << "CoordSystem::removePointAxis identifier="
...
void CoordSystem::removePointGraph(const QString &identifier) {
  LOG4CPP_INFO_S((*mainCat)) << "CoordSystem::removePointGraph identifier="
                             << identifier.toLatin1().data();
...
void CoordSystem::removePointsInCurvesGraphs(CurvesGraphs &curvesGraphs) {
  ...
  Functor2wRet<const QString &, const Point &, CallbackSearchReturn>
      ftorWithCallback =
          functor_ret(ftor, &CallbackRemovePointsInCurvesGraphs::callback);
...
void CoordSystem::setModelColorFilter(
  ...
  for (itr = modelColorFilter.colorFilterSettingsList().constBegin();
       itr != modelColorFilter.colorFilterSettingsList().constEnd(); itr++) {
...
void CoordSystem::setModelDigitizeCurve(
...
void CoordSystem::setModelGridRemoval(
    const DocumentModelGridRemoval &modelGridRemoval) {
  m_modelGridRemoval = modelGridRemoval;
}

void CoordSystem::setModelPointMatch(
...
bool CoordSystem::successfulRead() const { return m_successfulRead; }
...
void CoordSystem::updatePointOrdinals(const Transformation &transformation) {
  ...
  // The graph coordinates of all points in m_curvesGraphs must have already
  // been updated at this point. See applyTransformation
