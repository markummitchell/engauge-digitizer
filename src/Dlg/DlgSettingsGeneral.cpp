...
DlgSettingsGeneral::DlgSettingsGeneral(MainWindow &mainWindow)
    : DlgSettingsAbstractBase("General", "DlgSettingsGeneral", mainWindow),
      m_modelGeneralBefore(0), m_modelGeneralAfter(0) {
...
void DlgSettingsGeneral::createControls(QGridLayout *layout, int &row) {
  ...
  m_spinCursorSize->setWhatsThis(
      tr("Effective Cursor Size\n\n"
         "This is the effective width and height of the cursor when clicking "
         "on a pixel that is "
         "not part of the background.\n\n"
         "This parameter is used in the Color Picker and Point Match modes"));
  connect(m_spinCursorSize, SIGNAL(valueChanged(int)), this,
          SLOT(slotCursorSize(int)));
  ...
  m_spinExtraPrecision->setWhatsThis(
      tr("Extra Digits of Precision\n\n"
         "This is the number of additional digits of precision appended after "
         "the significant "
         "digits determined by the digitization accuracy at that point. The "
         "digitization accuracy "
         "at any point equals the change in graph coordinates from moving one "
         "pixel in each direction. "
         "Appending extra digits does not improve the accuracy of the numbers. "
         "More information can "
         "be found in discussions of accuracy versus precision.\n\n"
         "This parameter is used on the coordinates in the Status Bar and "
         "during Export"));
  connect(m_spinExtraPrecision, SIGNAL(valueChanged(int)), this,
          SLOT(slotExtraPrecision(int)));
...
void DlgSettingsGeneral::createOptionalSaveDefault(QHBoxLayout *layout) {
  ...
  m_btnSaveDefault->setWhatsThis(
      tr("Save the settings for use as future defaults, according to the curve "
         "name selection."));
...
void DlgSettingsGeneral::handleOk() {
  ...
  CmdSettingsGeneral *cmd =
      new CmdSettingsGeneral(mainWindow(), cmdMediator().document(),
                             *m_modelGeneralBefore, *m_modelGeneralAfter);
...
void DlgSettingsGeneral::updateControls() { enableOk(true); }
