...
const int STEPS_PER_CYCLE = 4;   // Repeat STEPS_PER_CYLE-1 unhighlighted steps
                                 // plus 1 highlighted step in each cycle
const int STEPS_CYCLE_COUNT = 4; // Repeat one highlighted step +
                                 // STEPS_UNHIGHLIGHTED_PER_HIGHLIGHTED steps
                                 // this many times
...
const double CARTESIAN_COORD_STEP =
    (CARTESIAN_COORD_MAX - CARTESIAN_COORD_MIN) / (NUM_COORD_STEPS - 1.0);
...
const double POLAR_THETA_STEP =
    (POLAR_THETA_MAX - POLAR_THETA_MIN) / (NUM_COORD_STEPS - 1.0);
...
const double POWER_FOR_LOG = 10.0; // Need a larger power (certainly more than
                                   // e) to make log gradient obvious

DlgSettingsCoords::DlgSettingsCoords(MainWindow &mainWindow)
    : DlgSettingsAbstractBase("Coordinates", "DlgSettingsCoords", mainWindow),
      m_btnCartesian(0), m_btnPolar(0), m_validatorOriginRadius(0),
      m_cmbDate(0), m_cmbTime(0), m_scenePreview(0), m_viewPreview(0),
      m_modelCoordsBefore(0), m_modelCoordsAfter(0) {
...
void DlgSettingsCoords::annotateAngles(const QFont &defaultFont) {
  ...
  for (int direction = 0; direction < 4; direction++) {
    ...
    QString angle;
    CoordUnitsPolarTheta thetaUnits =
        (CoordUnitsPolarTheta)m_cmbXThetaUnits->currentData().toInt();
...
void DlgSettingsCoords::annotateRadiusAtOrigin(const QFont &defaultFont) {
  ...
  QGraphicsTextItem *textRadius =
      m_scenePreview->addText(m_editOriginRadius->text());
...
QRectF DlgSettingsCoords::boundingRectGraph(CmdMediator &cmdMediator,
  ...
  // There may or may one, two or three axis points. Even if all three are not
  // defined (so
  // transformation is not defined), we can still get coordinates if there are
  // one or two
  cmdMediator.iterateThroughCurvePointsAxes(ftorWithCallback);

  // If the transformation is not defined, then there are no graph coordinates
  // to extract
...
void DlgSettingsCoords::createDateTime(QGridLayout *layout, int &row) {
  ...
  m_cmbDate->setWhatsThis(tr("Date format to be used for date values, and date "
                             "portion of mixed date/time values, "
                             "during input and output.\n\n"
                             "Setting the format to an empty value results in "
                             "just the time portion appearing in output."));
  connect(m_cmbDate, SIGNAL(activated(const QString &)), this,
          SLOT(slotDate(const QString &)));
  layoutCombos->addWidget(m_cmbDate);

  m_cmbTime = new QComboBox;
  m_cmbTime->setWhatsThis(tr("Time format to be used for time values, and time "
                             "portion of mixed date/time values, "
                             "during input and output.\n\n"
                             "Setting the format to an empty value results in "
                             "just the date portion appearing in output."));
  connect(m_cmbTime, SIGNAL(activated(const QString &)), this,
          SLOT(slotTime(const QString &)));
  layoutCombos->addWidget(m_cmbTime);
}

void DlgSettingsCoords::createGroupCoordsType(QGridLayout *layout, int &row) {
  ...
  QString polarButtonText =
      QString(tr("Polar") + " (") + THETA + QString(", " + tr("R") + ")");

  m_btnCartesian = new QRadioButton(tr("Cartesian (X, Y)"), m_boxCoordsType);
  m_btnCartesian->setWhatsThis(
      QString(tr("Select cartesian coordinates.\n\n"
                 "The X and Y coordinates will be used")));
  connect(m_btnCartesian, SIGNAL(toggled(bool)), this,
          SLOT(slotCartesianPolar(bool)));
  layoutGroup->addWidget(m_btnCartesian);

  m_btnPolar = new QRadioButton(polarButtonText, m_boxCoordsType);
  m_btnPolar->setWhatsThis(QString(
      tr("Select polar coordinates.\n\n"
         "The Theta and R coordinates will be used.\n\n"
         "Polar coordinates are not allowed with log scale for Theta")));
  connect(m_btnPolar, SIGNAL(toggled(bool)), this,
          SLOT(slotCartesianPolar(bool)));
  layoutGroup->addWidget(m_btnPolar);
}

void DlgSettingsCoords::createGroupXTheta(QGridLayout *layout, int &row) {
  ...
  m_xThetaLinear->setWhatsThis(
      QString(tr("Specifies linear scale for the X or Theta coordinate")));
  ...
  m_xThetaLog->setWhatsThis(QString(
      tr("Specifies logarithmic scale for the X or Theta coordinate.\n\n"
  ...
  connect(m_cmbXThetaUnits, SIGNAL(activated(const QString &)), this,
          SLOT(slotUnitsXTheta(
              const QString &))); // activated() ignores code changes
  layoutXTheta->addWidget(m_cmbXThetaUnits, rowGroup++, COLUMN_0, 1, 2);
}

void DlgSettingsCoords::createGroupYRadius(QGridLayout *layout, int &row) {
  ...
  m_yRadiusLinear->setWhatsThis(
      QString(tr("Specifies linear scale for the Y or R coordinate")));
  ...
  m_yRadiusLog->setWhatsThis(QString(
      tr("Specifies logarithmic scale for the Y or R coordinate\n\n"
         "Log scale is not allowed if there are negative coordinates.")));
  ...
  connect(m_cmbYRadiusUnits, SIGNAL(activated(const QString &)), this,
          SLOT(slotUnitsYRadius(
              const QString &))); // activated() ignores code changes
  ...
  m_editOriginRadius->setWhatsThis(
      QString(tr("Specify radius value at origin.\n\n"
                 "Normally the radius at the origin is 0, but a nonzero value "
                 "may be applied in other cases "
                 "(like when the radial units are decibels).")));
  connect(m_editOriginRadius, SIGNAL(textChanged(const QString &)), this,
          SLOT(slotPolarOriginRadius(const QString &)));
...
void DlgSettingsCoords::createPreview(QGridLayout *layout, int &row) {
  ...
  m_viewPreview->setWhatsThis(tr("Preview window that shows how current "
                                 "settings affect the coordinate system."));
...
void DlgSettingsCoords::drawCartesianLinearX() {
  ...
  for (int step = 0; step < NUM_COORD_STEPS; step++) {
    ...
    QGraphicsLineItem *line =
        m_scenePreview->addLine(x, CARTESIAN_COORD_MIN, x, CARTESIAN_COORD_MAX);
    ...
    if (isAxis) {
      line = m_scenePreview->addLine(x, CARTESIAN_COORD_MIN, x,
                                     CARTESIAN_COORD_MAX);
      line->setPen(QPen(QBrush(Qt::black), LINE_WIDTH_THICK));
    }
    isAxis = false;
  }
}
...
void DlgSettingsCoords::drawCartesianLinearY() {
  ...
  for (int step = NUM_COORD_STEPS - 1; step >= 0; step--) {
    ...
    QGraphicsLineItem *line =
        m_scenePreview->addLine(CARTESIAN_COORD_MIN, y, CARTESIAN_COORD_MAX, y);
    bool isHighlighted = (step % STEPS_PER_CYCLE == 0);
    line->setPen(QPen(QBrush(isHighlighted ? Qt::gray : Qt::lightGray),
                      LINE_WIDTH_THIN,
                      (isHighlighted ? Qt::SolidLine : Qt::DashLine)));
    if (isAxis) {
      line = m_scenePreview->addLine(CARTESIAN_COORD_MIN, y,
                                     CARTESIAN_COORD_MAX, y);
      line->setPen(QPen(QBrush(Qt::black), LINE_WIDTH_THICK));
    }
    isAxis = false;
  }
...
void DlgSettingsCoords::drawCartesianLogX() {
  ...
  bool isAxis = true;
  for (int step = 0; step < NUM_COORD_STEPS; step++) {
    double s = (exp(step / (NUM_COORD_STEPS - 1.0)) - 1.0) / (exp(1.0) - 1.0);
...
void DlgSettingsCoords::drawCartesianLogY() {
  ...
  for (int step = 0; step < NUM_COORD_STEPS; step++) {
    ...
    double y = (1.0 - s) * CARTESIAN_COORD_MAX +
               s * CARTESIAN_COORD_MIN; // Invert y coordinate (min<->max)
    QGraphicsLineItem *line =
        m_scenePreview->addLine(CARTESIAN_COORD_MIN, y, CARTESIAN_COORD_MAX, y);
    bool isHighlighted = (step % STEPS_PER_CYCLE == 0);
    line->setPen(QPen(QBrush(isHighlighted ? Qt::gray : Qt::lightGray),
...
void DlgSettingsCoords::drawPolarLinearRadius() {
  ...
  for (int step = 0; step < NUM_COORD_STEPS; step++) {
    ...
    QGraphicsEllipseItem *line = m_scenePreview->addEllipse(
        XCENTER - radius, YCENTER - radius, 2.0 * radius, 2.0 * radius);
    bool isHighlighted = (step % STEPS_PER_CYCLE == 0);
    line->setPen(QPen(QBrush(isHighlighted ? Qt::gray : Qt::lightGray),
                      LINE_WIDTH_THIN,
                      (isHighlighted ? Qt::SolidLine : Qt::DashLine)));
...
void DlgSettingsCoords::drawPolarTheta() {
  ...
  for (int step = 0; step < NUM_COORD_STEPS; step++) {
    ...
    QGraphicsLineItem *line =
        m_scenePreview->addLine(XCENTER, YCENTER, XCENTER + x, YCENTER + y);
    bool isHighlighted = (step % STEPS_PER_CYCLE == 0);
    line->setPen(QPen(QBrush(isHighlighted ? Qt::gray : Qt::lightGray),
                      LINE_WIDTH_THIN,
                      (isHighlighted ? Qt::SolidLine : Qt::DashLine)));
...
void DlgSettingsCoords::handleOk() {
  ...
  CmdSettingsCoords *cmd =
      new CmdSettingsCoords(mainWindow(), cmdMediator().document(),
                            *m_modelCoordsBefore, *m_modelCoordsAfter);
...
void DlgSettingsCoords::load(CmdMediator &cmdMediator) {
  ...
  bool isEmpty;
  QRectF rectGraph = boundingRectGraph(cmdMediator, isEmpty);
  ...
  // Save new data
  m_modelCoordsBefore =
      new DocumentModelCoords(cmdMediator.document().modelCoords());
  m_modelCoordsAfter =
      new DocumentModelCoords(cmdMediator.document().modelCoords());

  // Populate controls
  DlgValidatorFactory dlgValidatorFactory;
  m_validatorOriginRadius = dlgValidatorFactory.createWithNonPolar(
  ...
  m_editOriginRadius->setValidator(m_validatorOriginRadius); // Set before call
                                                             // to setText so
                                                             // validator is
                                                             // defined in
                                                             // updateControls
  m_editOriginRadius->setText(
      QString::number(m_modelCoordsAfter->originRadius()));
  ...
  m_xThetaLinear->setChecked(m_modelCoordsAfter->coordScaleXTheta() ==
                             COORD_SCALE_LINEAR);
  m_xThetaLog->setChecked(m_modelCoordsAfter->coordScaleXTheta() ==
                          COORD_SCALE_LOG);
  m_yRadiusLinear->setChecked(m_modelCoordsAfter->coordScaleYRadius() ==
                              COORD_SCALE_LINEAR);
  m_yRadiusLog->setChecked(m_modelCoordsAfter->coordScaleYRadius() ==
                           COORD_SCALE_LOG);
...
void DlgSettingsCoords::loadComboBoxDate() {
  ...
  int index =
      m_cmbDate->findData(QVariant(m_modelCoordsAfter->coordUnitsDate()));
...
void DlgSettingsCoords::loadComboBoxTime() {
  ...
  m_cmbTime->addItem(
      coordUnitsTimeToString(COORD_UNITS_TIME_HOUR_MINUTE_SECOND),
      QVariant(COORD_UNITS_TIME_HOUR_MINUTE_SECOND));

  ENGAUGE_ASSERT(m_cmbTime->count() == NUM_COORD_UNITS_TIME);

  int index =
      m_cmbTime->findData(QVariant(m_modelCoordsAfter->coordUnitsTime()));
  m_cmbTime->setCurrentIndex(index);
}

void DlgSettingsCoords::loadComboBoxUnitsNonPolar(
    QComboBox &cmb, CoordUnitsNonPolarTheta coordUnits) {
  LOG4CPP_INFO_S((*mainCat)) << "DlgSettingsCoords::loadComboBoxUnitsNonPolar";

  cmb.clear();

  cmb.addItem(
      coordUnitsNonPolarThetaToString(COORD_UNITS_NON_POLAR_THETA_NUMBER),
      QVariant(COORD_UNITS_NON_POLAR_THETA_NUMBER));
  cmb.addItem(
      coordUnitsNonPolarThetaToString(COORD_UNITS_NON_POLAR_THETA_DATE_TIME),
      QVariant(COORD_UNITS_NON_POLAR_THETA_DATE_TIME));
  cmb.addItem(coordUnitsNonPolarThetaToString(
  ...
  cmb.setWhatsThis(QString(
      tr("Numbers have the simplest and most general format.\n\n"
         "Date and time values have date and/or time components.\n\n"
         "Degrees Minutes Seconds (DDD MM SS.S) format uses two integer number "
         "for degrees and minutes, and a real number for "
         "seconds. There are 60 seconds per minute. During input, spaces must "
         "be inserted between the three numbers.")));

  int index = cmb.findData(coordUnits);
  cmb.setCurrentIndex(index);
}

void DlgSettingsCoords::loadComboBoxUnitsPolar(
    QComboBox &cmb, CoordUnitsPolarTheta coordUnits) {
  ...
  cmb.addItem(
      coordUnitsPolarThetaToString(COORD_UNITS_POLAR_THETA_DEGREES_MINUTES),
      QVariant(COORD_UNITS_POLAR_THETA_DEGREES_MINUTES));
  cmb.addItem(coordUnitsPolarThetaToString(
  ...
  cmb.setWhatsThis(QString(
      tr("Degrees (DDD.DDDDD) format uses a single real number. One complete "
         "revolution is 360 degrees.\n\n"
         "Degrees Minutes (DDD MM.MMM) format uses one integer number for "
         "degrees, and a real number for minutes. There are "
         "60 minutes per degree. During input, a space must be inserted "
         "between the two numbers.\n\n"
         "Degrees Minutes Seconds (DDD MM SS.S) format uses two integer number "
         "for degrees and minutes, and a real number for "
         "seconds. There are 60 seconds per minute. During input, spaces must "
         "be inserted between the three numbers.\n\n"
         "Gradians format uses a single real number. One complete revolution "
         "is 400 gradians.\n\n"
         "Radians format uses a single real number. One complete revolution is "
         "2*pi radians.\n\n"
         "Turns format uses a single real number. One complete revolution is "
         "one turn.")));
...
void DlgSettingsCoords::slotUnitsXTheta(const QString &) {
  ...
  if (m_modelCoordsAfter->coordsType() == COORDS_TYPE_CARTESIAN) {
    CoordUnitsNonPolarTheta coordUnits =
        (CoordUnitsNonPolarTheta)m_cmbXThetaUnits->currentData().toInt();
...
void DlgSettingsCoords::slotUnitsYRadius(const QString &) {
  ...
  CoordUnitsNonPolarTheta coordUnits =
      (CoordUnitsNonPolarTheta)m_cmbYRadiusUnits->currentData().toInt();
  if (m_modelCoordsAfter->coordsType() == COORDS_TYPE_CARTESIAN) {
...
void DlgSettingsCoords::slotYRadiusLinear() {
  ...
  DlgValidatorFactory dlgValidatorFactory;
  m_validatorOriginRadius = dlgValidatorFactory.createWithNonPolar(
      COORD_SCALE_LINEAR, m_modelCoordsAfter->coordUnitsRadius(),
      m_modelCoordsAfter->coordUnitsDate(),
      m_modelCoordsAfter->coordUnitsTime(),
      mainWindow().modelMainWindow().locale());
...
void DlgSettingsCoords::slotYRadiusLog() {
  ...
  DlgValidatorFactory dlgValidatorFactory;
  m_validatorOriginRadius = dlgValidatorFactory.createWithNonPolar(
      COORD_SCALE_LOG, m_modelCoordsAfter->coordUnitsRadius(),
      m_modelCoordsAfter->coordUnitsDate(),
      m_modelCoordsAfter->coordUnitsTime(),
      mainWindow().modelMainWindow().locale());
  m_editOriginRadius->setValidator(m_validatorOriginRadius);
...
void DlgSettingsCoords::updateControls() {
  ...
  bool goodOriginRadius =
      true; // Cartesian coordinates do not use origin radius
  if (m_editOriginRadius->isEnabled()) {

    // Origin radius must be greater than zero
    goodOriginRadius =
        (m_validatorOriginRadius->validate(textOriginRadius, posOriginRadius) ==
  ...
  if (m_btnCartesian->isChecked()) {
    ...
    // Use temporary validator to see if current origin radius would be correct
    // in OTHER linear/log mode
    DlgValidatorFactory dlgValidatorFactory;
    DlgValidatorAbstract *dlg = dlgValidatorFactory.createWithNonPolar(
    ...
    bool goodOriginRadiusOther =
        (dlg->validate(textOriginRadius, posOriginRadiusOther) ==
         QValidator::Acceptable);
  ...
  QString captionXTheta =
      (m_btnCartesian->isChecked() ? QString(tr("X")) : THETA) +
      QString(" %1").arg(tr("Coordinates"));
  QString captionYRadius =
      (m_btnCartesian->isChecked() ? QString(tr("Y")) : QString(tr("R"))) +
      QString(" %1").arg(tr("Coordinates"));
...
void DlgSettingsCoords::updatePreview() {
  ...
  // General approach
  // 1) Axis lines are extra thick, but since they sometimes disappear as the
  // preview window is rescaled, we keep the
  //    constant-pixel line under each axis line
  // 2) Every STEPS_UNHIGHLIGHTED_PER_HIGHLIGHTED out of
  // STEPS_UNHIGHLIGHTED_PER_HIGHLIGHTED+1 lines are dashed to make
  //    them more subtle
