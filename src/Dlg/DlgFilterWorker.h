...
/// http://blog.debao.me/2013/08/how-to-use-qworker-in-the-right-way-part-1/
...
public slots:
  /// Start processing with a new set of parameters. Any ongoing processing is
  /// interrupted when m_filterMode changes.
  void slotNewParameters(ColorFilterMode colorFilterMode, double low,
                         double high);
...
private:
  ...
  QImage m_imageOriginal; // Use QImage rather than QPixmap so we can access
                          // pixel by pixel
  ...
  int m_xLeft;
  QTimer m_restartTimer; // Decouple slotRestartProcessing from the processing
                         // that this class performs
};
