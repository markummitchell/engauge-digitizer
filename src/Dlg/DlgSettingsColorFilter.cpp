...
DlgSettingsColorFilter::DlgSettingsColorFilter(MainWindow &mainWindow)
    : DlgSettingsAbstractBase("Filter", "DlgSettingsColorFilter", mainWindow),
      m_scenePreview(0), m_viewPreview(0), m_filterThread(0),
      m_modelColorFilterBefore(0), m_modelColorFilterAfter(0) {
...
void DlgSettingsColorFilter::createControls(QGridLayout *layout, int &row) {
  ...
  m_btnIntensity->setWhatsThis(tr(
      "Filter the original image into black and white pixels using the "
      "Intensity parameter, "
  ...
  m_btnForeground->setWhatsThis(tr(
      "Filter the original image into black and white pixels by isolating the "
      "foreground from the background, "
      "to hide unimportant information and emphasize important information.\n\n"
      "The background color is shown on the left side of the scale bar.\n\n"
      "The distance of any color (R, G, B) from the background color (Rb, Gb, "
      "Bb) is computed as "
      "F = squareroot ((R - Rb) * (R - Rb) + (G - Gb) * (G - Gb) + (B - Bb)). "
      "On the left end of the "
      "scale, the foreground distance value is zero, and it increases linearly "
      "to the maximum on the far right."));
  ...
  m_btnHue->setWhatsThis(tr(
      "Filter the original image into black and white pixels using the Hue "
      "component of the "
  ...
  m_btnSaturation->setWhatsThis(tr(
      "Filter the original image into black and white pixels using the "
  ...
  m_btnValue->setWhatsThis(tr(
      "Filter the original image into black and white pixels using the Value "
      "component of the "
      "Hue, Saturation and Value (HSV) color components, "
      "to hide unimportant information and emphasize important information.\n\n"
...
void DlgSettingsColorFilter::createOptionalSaveDefault(
    QHBoxLayout * /* layout */) {}
...
void DlgSettingsColorFilter::createPreview(QGridLayout *layout, int &row) {
  ...
  m_viewPreview->setWhatsThis(
      tr("Preview window that shows how current settings affect the filtering "
...
void DlgSettingsColorFilter::createProfileAndScale(QGridLayout *layout,
                                                   int &row) {
  ...
  m_sceneProfile->setSceneRect(0, 0, PROFILE_SCENE_WIDTH(),
                               PROFILE_SCENE_HEIGHT());

  m_viewProfile = new ViewProfile(m_sceneProfile, MINIMUM_VIEW_PROFILE_WIDTH);
  m_viewProfile->setWhatsThis(
      tr("Histogram profile of the selected filter parameter. The two Dividers "
         "can be moved back and forth to adjust "
         "the range of filter parameter values that will be included in the "
         "filtered image. The clear portion will "
  ...
  m_scale->setWhatsThis(
      tr("This read-only box displays a graphical representation of the "
         "horizontal axis in the histogram profile above."));
...
QWidget *DlgSettingsColorFilter::createSubPanel() {
  ...
  layout->setColumnStretch(2, 0); // Empty column to put some space between
                                  // previous and next columns, so they are not
                                  // too close
...
QRgb DlgSettingsColorFilter::createThread() {
  ...
  if (m_filterThread == 0) {
    ...
    m_filterThread = new DlgFilterThread(cmdMediator().document().pixmap(),
                                         rgbBackground, *this);
    m_filterThread
        ->start(); // Now that thread is started, we can use signalApplyFilter
  }
...
void DlgSettingsColorFilter::handleOk() {
  ...
  CmdSettingsColorFilter *cmd = new CmdSettingsColorFilter(
      mainWindow(), cmdMediator().document(), *m_modelColorFilterBefore,
...
void DlgSettingsColorFilter::load(CmdMediator &cmdMediator) {
  ...
  m_modelColorFilterBefore =
      new DocumentModelColorFilter(cmdMediator.document().modelColorFilter());
  m_modelColorFilterAfter =
      new DocumentModelColorFilter(cmdMediator.document().modelColorFilter());
...
void DlgSettingsColorFilter::slotTransferPiece(int xLeft, QImage image) {
  // Overwrite one piece of the processed image. This approach is a bit slow
  // because the entire QPixmap
  // in the QGraphicsScene gets exchanged as part of each update, but that seems
  // to be the only possible
  // approach when using QGraphicsScene. If not fast enough or there is ugly
  // flicker, we may replace
  // QGraphicsScene by a simple QWidget and override the paint function - but
  // that approach may get
  ...
  // Save new pixmap. Only visible change should be the area covered by the
  // pixels in image
  m_scenePreview->addPixmap(QPixmap::fromImage(m_imagePreview));
}
...
void DlgSettingsColorFilter::updateHistogram() {
  ...
  const double PEN_WIDTH =
      0.0; // Zero value gives one-pixel width at all scales

  QString curveName = m_cmbCurveName->currentText();

  m_sceneProfile->clear();

  m_scale->setColorFilterMode(
      m_modelColorFilterAfter->colorFilterMode(curveName));
  ...
  int maxBinCount;
  filterHistogram.generate(filter, histogramBins,
                           m_modelColorFilterAfter->colorFilterMode(curveName),
                           image, maxBinCount);

  // Draw histogram, normalizing so highest peak exactly fills the vertical
  // range. Log scale is used
  ...
  for (int bin = 1; bin < ColorFilterHistogram::HISTOGRAM_BINS(); bin++) {
    ...
    double x0 = PROFILE_SCENE_WIDTH() * (bin - 1.0) /
                (ColorFilterHistogram::HISTOGRAM_BINS() - 1.0);

    // Map logPixelCount through 0 to 0 through PROFILE_SCENE_HEIGHT-1, using
    // log scale
    double count0 = 1.0 + histogramBins[bin - 1];
    double y0 =
        (PROFILE_SCENE_HEIGHT() - 1.0) * (1.0 - qLn(count0) / logMaxBinCount);

    double x1 = PROFILE_SCENE_WIDTH() * (bin - 0.0) /
                (ColorFilterHistogram::HISTOGRAM_BINS() - 1.0);
    ...
    double y1 =
        (PROFILE_SCENE_HEIGHT() - 1.0) * (1.0 - qLn(count1) / logMaxBinCount);
  ...
  m_dividerLow = new ViewProfileDivider(
      *m_sceneProfile, *m_viewProfile, PROFILE_SCENE_WIDTH(),
      PROFILE_SCENE_HEIGHT(), PROFILE_SCENE_HEIGHT() * 2.0 / 3.0, true);
  m_dividerHigh = new ViewProfileDivider(
      *m_sceneProfile, *m_viewProfile, PROFILE_SCENE_HEIGHT(),
      PROFILE_SCENE_WIDTH(), PROFILE_SCENE_HEIGHT() / 3.0, false);

  // Connect the dividers to each other since the shaded areas depend on both
  // divides when low divider is
  // moved to the right of the high divider
  connect(m_dividerLow, SIGNAL(signalMovedLow(double)), m_dividerHigh,
          SLOT(slotOtherMoved(double)));
  connect(m_dividerHigh, SIGNAL(signalMovedHigh(double)), m_dividerLow,
          SLOT(slotOtherMoved(double)));

  // Update preview when the dividers move
  connect(m_dividerLow, SIGNAL(signalMovedLow(double)), this,
          SLOT(slotDividerLow(double)));
  connect(m_dividerHigh, SIGNAL(signalMovedHigh(double)), this,
          SLOT(slotDividerHigh(double)));

  if (m_btnForeground->isChecked()) {

    // Foreground
    m_dividerLow->setX(m_modelColorFilterAfter->foregroundLow(curveName),
                       FOREGROUND_MIN, FOREGROUND_MAX);
    m_dividerHigh->setX(m_modelColorFilterAfter->foregroundHigh(curveName),
                        FOREGROUND_MIN, FOREGROUND_MAX);

  } else if (m_btnIntensity->isChecked()) {

    // Intensity
    m_dividerLow->setX(m_modelColorFilterAfter->intensityLow(curveName),
                       INTENSITY_MIN, INTENSITY_MAX);
    m_dividerHigh->setX(m_modelColorFilterAfter->intensityHigh(curveName),
                        INTENSITY_MIN, INTENSITY_MAX);

  } else if (m_btnHue->isChecked()) {

    // Hue
    m_dividerLow->setX(m_modelColorFilterAfter->hueLow(curveName), HUE_MIN,
                       HUE_MAX);
    m_dividerHigh->setX(m_modelColorFilterAfter->hueHigh(curveName), HUE_MIN,
                        HUE_MAX);

  } else if (m_btnSaturation->isChecked()) {

    // Saturation
    m_dividerLow->setX(m_modelColorFilterAfter->saturationLow(curveName),
                       SATURATION_MIN, SATURATION_MAX);
    m_dividerHigh->setX(m_modelColorFilterAfter->saturationHigh(curveName),
                        SATURATION_MIN, SATURATION_MAX);

  } else if (m_btnValue->isChecked()) {

    // Value
    m_dividerLow->setX(m_modelColorFilterAfter->valueLow(curveName), VALUE_MIN,
                       VALUE_MAX);
    m_dividerHigh->setX(m_modelColorFilterAfter->valueHigh(curveName),
                        VALUE_MIN, VALUE_MAX);
