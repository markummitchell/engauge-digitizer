...
DlgSettingsGridRemoval::DlgSettingsGridRemoval(MainWindow &mainWindow)
    : DlgSettingsAbstractBase("Grid Removal", "DlgSettingsGridRemoval",
                              mainWindow),
      m_scenePreview(0), m_viewPreview(0), m_modelGridRemovalBefore(0),
...
void DlgSettingsGridRemoval::createOptionalSaveDefault(
    QHBoxLayout * /* layout */) {}
...
void DlgSettingsGridRemoval::createPreview(QGridLayout *layout, int &row) {
  ...
  m_viewPreview->setWhatsThis(
      tr("Preview window that shows how current settings affect grid removal"));
...
void DlgSettingsGridRemoval::createRemoveGridLines(QGridLayout *layout,
                                                   int &row) {
  LOG4CPP_INFO_S((*mainCat)) << "DlgSettingsGridRemoval::createRemoveGridLines";

  m_chkRemoveGridLines =
      new QCheckBox(tr("Remove pixels close to defined grid lines"));
  m_chkRemoveGridLines->setWhatsThis(
      tr("Check this box to have pixels close to regularly spaced gridlines "
         "removed.\n\n"
         "This option is only available when the axis points have all been "
         "defined."));
  connect(m_chkRemoveGridLines, SIGNAL(stateChanged(int)), this,
          SLOT(slotRemoveGridLines(int)));
  ...
  m_editCloseDistance->setWhatsThis(
      tr("Set closeness distance in pixels.\n\n"
         "Pixels that are closer to the regularly spaced gridlines, than this "
         "distance, "
         "will be removed.\n\n"
         "This value cannot be negative. A zero value disables this feature. "
         "Decimal values are allowed"));
  m_validatorCloseDistance = new QDoubleValidator(
      CLOSE_DISTANCE_MIN, CLOSE_DISTANCE_MAX, CLOSE_DECIMALS);
  m_editCloseDistance->setValidator(m_validatorCloseDistance);
  connect(m_editCloseDistance, SIGNAL(textChanged(const QString &)), this,
          SLOT(slotCloseDistance(const QString &)));
...
void DlgSettingsGridRemoval::createRemoveGridLinesX(QGridLayout *layout,
                                                    int &row) {
  LOG4CPP_INFO_S((*mainCat))
  ...
  m_cmbDisableX->setWhatsThis(
      tr("Disabled value.\n\n"
         "The X grid lines are specified using only three values at a time. "
         "For flexibility, four values "
         "are offered so you must chose which value is disabled. Once "
         "disabled, that value is simply "
  ...
  connect(
      m_cmbDisableX, SIGNAL(activated(const QString &)), this,
      SLOT(slotDisableX(const QString &))); // activated() ignores code changes
  ...
  connect(m_editCountX, SIGNAL(textChanged(const QString &)), this,
          SLOT(slotCountX(const QString &)));
  ...
  m_editStartX->setWhatsThis(
      tr("Value of the first X grid line.\n\n"
  ...
  connect(m_editStartX, SIGNAL(textChanged(const QString &)), this,
          SLOT(slotStartX(const QString &)));
  ...
  m_editStepX->setWhatsThis(
      tr("Difference in value between two successive X grid lines.\n\n"
         "The step value must be greater than zero"));
  m_validatorStepX = new QDoubleValidator;
  m_editStepX->setValidator(m_validatorStepX);
  connect(m_editStepX, SIGNAL(textChanged(const QString &)), this,
          SLOT(slotStepX(const QString &)));
  ...
  m_editStopX->setWhatsThis(
      tr("Value of the last X grid line.\n\n"
  ...
  connect(m_editStopX, SIGNAL(textChanged(const QString &)), this,
          SLOT(slotStopX(const QString &)));
  layoutGroup->addWidget(m_editStopX, 4, 1);
}

void DlgSettingsGridRemoval::createRemoveGridLinesY(QGridLayout *layout,
                                                    int &row) {
  LOG4CPP_INFO_S((*mainCat))
  ...
  m_cmbDisableY->setWhatsThis(
      tr("Disabled value.\n\n"
         "The Y grid lines are specified using only three values at a time. "
         "For flexibility, four values "
         "are offered so you must chose which value is disabled. Once "
         "disabled, that value is simply "
         "updated as the other values change"));
  ...
  m_cmbDisableY->addItem(gridCoordDisableToString(GRID_COORD_DISABLE_STOP),
                         QVariant(GRID_COORD_DISABLE_STOP));
  connect(
      m_cmbDisableY, SIGNAL(activated(const QString &)), this,
      SLOT(slotDisableY(const QString &))); // activated() ignores code changes
  ...
  connect(m_editCountY, SIGNAL(textChanged(const QString &)), this,
          SLOT(slotCountY(const QString &)));
  ...
  m_editStartY->setWhatsThis(
      tr("Value of the first Y grid line.\n\n"
         "The start value cannot be greater than the stop value"));
  m_validatorStartY = new QDoubleValidator;
  m_editStartY->setValidator(m_validatorStartY);
  connect(m_editStartY, SIGNAL(textChanged(const QString &)), this,
          SLOT(slotStartY(const QString &)));
  ...
  m_editStepY->setWhatsThis(
      tr("Difference in value between two successive Y grid lines.\n\n"
         "The step value must be greater than zero"));
  m_validatorStepY = new QDoubleValidator;
  m_editStepY->setValidator(m_validatorStepY);
  connect(m_editStepY, SIGNAL(textChanged(const QString &)), this,
          SLOT(slotStepY(const QString &)));
  ...
  m_editStopY->setWhatsThis(
      tr("Value of the last Y grid line.\n\n"
         "The stop value cannot be less than the start value"));
  m_validatorStopY = new QDoubleValidator;
  m_editStopY->setValidator(m_validatorStopY);
  connect(m_editStopY, SIGNAL(textChanged(const QString &)), this,
          SLOT(slotStopY(const QString &)));
...
QWidget *DlgSettingsGridRemoval::createSubPanel() {
  ...
  layout->setColumnStretch(1, 0); // Checkbox part of "section" checkboxes. In
                                  // other rows this has empty space as
                                  // indentation
...
void DlgSettingsGridRemoval::handleOk() {
  ...
  CmdSettingsGridRemoval *cmd = new CmdSettingsGridRemoval(
      mainWindow(), cmdMediator().document(), *m_modelGridRemovalBefore,
...
void DlgSettingsGridRemoval::load(CmdMediator &cmdMediator) {
  ...
  m_modelGridRemovalBefore =
      new DocumentModelGridRemoval(cmdMediator.document());
  m_modelGridRemovalAfter =
      new DocumentModelGridRemoval(cmdMediator.document());

  // Sanity checks. Incoming defaults must be acceptable to the local limits
  ENGAUGE_ASSERT(CLOSE_DISTANCE_MIN <=
                 m_modelGridRemovalAfter->closeDistance());
  ENGAUGE_ASSERT(CLOSE_DISTANCE_MAX >=
                 m_modelGridRemovalAfter->closeDistance());

  // Populate controls
  m_chkRemoveGridLines->setChecked(
      m_modelGridRemovalAfter->removeDefinedGridLines());

  m_editCloseDistance->setText(
      QString::number(m_modelGridRemovalAfter->closeDistance()));

  int indexDisableX = m_cmbDisableX->findData(
      QVariant(m_modelGridRemovalAfter->gridCoordDisableX()));
  ...
  int indexDisableY = m_cmbDisableX->findData(
      QVariant(m_modelGridRemovalAfter->gridCoordDisableY()));
...
void DlgSettingsGridRemoval::slotCloseDistance(const QString &) {
  ...
  m_modelGridRemovalAfter->setCloseDistance(
      m_editCloseDistance->text().toDouble());
  updateControls();
  updatePreview();
}
...
void DlgSettingsGridRemoval::slotDisableX(const QString &) {
  ...
  GridCoordDisable gridCoordDisable =
      (GridCoordDisable)m_cmbDisableX->currentData().toInt();
...
void DlgSettingsGridRemoval::slotDisableY(const QString &) {
  ...
  GridCoordDisable gridCoordDisable =
      (GridCoordDisable)m_cmbDisableY->currentData().toInt();
...
void DlgSettingsGridRemoval::updateControls() {
  ...
  int pos;
  bool isOk =
      (m_validatorCloseDistance->validate(textCloseDistance, pos) ==
       QValidator::Acceptable) &&
      (m_validatorCountX->validate(textCountX, pos) ==
       QValidator::Acceptable) &&
      (m_validatorStartX->validate(textStartX, pos) ==
       QValidator::Acceptable) &&
      ...
      (m_validatorCountY->validate(textCountY, pos) ==
       QValidator::Acceptable) &&
      (m_validatorStartY->validate(textStartY, pos) ==
       QValidator::Acceptable) &&
