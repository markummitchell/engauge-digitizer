...
DlgSettingsCurveAddRemove::DlgSettingsCurveAddRemove(MainWindow &mainWindow)
    : DlgSettingsAbstractBase("Curve Add/Remove", "DlgSettingsCurveAddRemove",
                              mainWindow) {
...
void DlgSettingsCurveAddRemove::appendCurveName(
    const QString &curveNameNew, const QString &curveNameOriginal,
    int numPoints) {
  ENGAUGE_CHECK_PTR(m_curveNameList);

  int row = m_curveNameList->rowCount();
  insertCurveName(row, curveNameNew, curveNameOriginal, numPoints);
}

void DlgSettingsCurveAddRemove::createButtons(QGridLayout *layout, int &row) {
  LOG4CPP_INFO_S((*mainCat)) << "DlgSettingsCurveAddRemove::createButtons";

  m_btnAdd = new QPushButton(tr("Add..."));
  m_btnAdd->setWhatsThis(tr("Adds a new curve to the curve list. The curve "
                            "name can be edited in the curve name list.\n\n"
  ...
  m_btnRemove->setWhatsThis(
      tr("Removes the currently selected curve from the curve list.\n\n"
...
void DlgSettingsCurveAddRemove::createListCurves(QGridLayout *layout,
  ...
  // There is no Qt::ItemIsEditable flag for QListView, so instead we set that
  // flag for the QListViewItems
  m_listCurves = new QListView;
  m_listCurves->setWhatsThis(
      tr("List of the curves belonging to this document.\n\n"
  ...
  connect(m_curveNameList,
          SIGNAL(dataChanged(const QModelIndex &, const QModelIndex &,
                             const QVector<int> &)),
          this, SLOT(slotDataChanged(const QModelIndex &, const QModelIndex &,
                                     const QVector<int> &)));
  connect(m_listCurves->selectionModel(),
...
void DlgSettingsCurveAddRemove::createOptionalSaveDefault(QHBoxLayout *layout) {
  ...
  m_btnSaveDefault->setWhatsThis(
      tr("Save the curve names for use as defaults for future graph curves."));
...
void DlgSettingsCurveAddRemove::handleOk() {
  ...
  CmdSettingsCurveAddRemove *cmd = new CmdSettingsCurveAddRemove(
      mainWindow(), cmdMediator().document(), *m_curveNameList);
  cmdMediator().push(cmd);

  hide();
}

void DlgSettingsCurveAddRemove::insertCurveName(
    int row, const QString &curveNameNew, const QString &curveNameOriginal,
    int numPoints) {
  if (m_curveNameList->insertRow(row)) {

    LOG4CPP_INFO_S((*mainCat))
        << "DlgSettingsCurveAddRemove::insertCurveName curveName="
        << curveNameNew.toLatin1().data();

    CurveNameListEntry curvesEntry(curveNameNew, curveNameOriginal, numPoints);
    ...
    m_curveNameList->setData(m_curveNameList->index(row, 2), numPoints);
...
void DlgSettingsCurveAddRemove::load(CmdMediator &cmdMediator) {
  ...
  for (itr = curveNames.begin(); itr != curveNames.end(); itr++) {
    ...
    appendCurveName(curveName, curveName,
...
QString DlgSettingsCurveAddRemove::nextCurveName() const {
  ...
  if ((numSelectedItems == 0) && (numItems > 0)) {
    ...
    currentIndex =
        m_listCurves->selectionModel()->selectedIndexes().at(0).row();
  }
  ...
  if (curveNameBefore.isEmpty() && !curveNameAfter.isEmpty() &&
      endsWithNumber(curveNameAfter)) {
    ...
    if (pos >= 0) {
      ...
      curveNameNext =
          QString("%1%2").arg(curveNameAfter.left(pos)).arg(numberNew);
    ...
    curveNameNext = DEFAULT_GRAPH_CURVE_NAME; // If necessary, this will be
                                              // deconflicted below
    ...
    if (endsWithNumber(curveNameBefore)) {
      ...
      // Curve name ends with a number. Pick a name after curveNameBefore, being
      // sure to not match curveNameAfter
      ...
      if (pos >= 0) {
        ...
        curveNameNext =
            QString("%1%2").arg(curveNameBefore.left(pos)).arg(numberNew);
        if (curveNameNext == curveNameAfter) {

          // The difference between before and after is exactly one so we go to
          // a lower level
          curveNameNext = QString("%1%2").arg(curveNameBefore).arg(DASH_ONE);
        }
      }
    }
  }
  ...
  curveNameNext =
      settingsForGraph.defaultCurveName(indexOneBasedNext, curveNameNext);

  // At this point we have curveNameNext which does not conflict with
  // curveNameBefore or
  // curveNameAfter, but it may in rare cases conflict with some other curve
  // name. We keep
...
void DlgSettingsCurveAddRemove::removeSelectedCurves() {
  ...
  for (int i = m_listCurves->selectionModel()->selectedIndexes().count() - 1;
       i >= 0; i--) {
...
void DlgSettingsCurveAddRemove::slotNew() {
  ...
  appendCurveName(curveNameSuggestion, NO_ORIGINAL_CURVE_NAME, NO_POINTS);
...
void DlgSettingsCurveAddRemove::slotRemove() {
  ...
  int numPoints = 0;
  for (int i = 0; i < m_listCurves->selectionModel()->selectedIndexes().count();
       i++) {

    int row = m_listCurves->selectionModel()->selectedIndexes().at(i).row();
    QModelIndex idx =
        m_curveNameList->index(row, CurveNameListEntry::COL_NUM_POINTS());
  ...
  if (numPoints > 0) {
    ...
    rtn = QMessageBox::warning(0, tr("Curves With Points"), msg,
...
void DlgSettingsCurveAddRemove::slotSaveDefault() {
  ...
  for (int row = 0; row < m_curveNameList->rowCount(); row++) {
    ...
    settings.setValue(SETTINGS_CURVE_NAME, curveNameCurrent);
    settings.endGroup();
  }
}

void DlgSettingsCurveAddRemove::slotSelectionChanged(QItemSelection,
                                                     QItemSelection) {
  updateControls();
}
...
void DlgSettingsCurveAddRemove::updateControls() {
  ...
  m_btnRemove->setEnabled((numSelectedItems > 0) &&
