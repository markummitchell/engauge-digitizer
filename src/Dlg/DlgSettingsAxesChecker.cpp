...
DlgSettingsAxesChecker::DlgSettingsAxesChecker(MainWindow &mainWindow)
    : DlgSettingsAbstractBase("Axes Checker", "DlgSettingsAxesChecker",
                              mainWindow),
      m_checker(0), m_modelAxesCheckerBefore(0), m_modelAxesCheckerAfter(0),
      m_modelCoords(0) {
...
void DlgSettingsAxesChecker::createControls(QGridLayout *layout, int &row) {
  ...
  m_btnNSeconds =
      new QRadioButton(tr("Show for a number of seconds"), groupBox);
  m_btnNSeconds->setWhatsThis(tr(
  ...
  connect(
      m_cmbSeconds, SIGNAL(activated(const QString &)), this,
      SLOT(slotSeconds(const QString &))); // activated() ignores code changes
  ...
  connect(m_groupMode, SIGNAL(buttonReleased(QAbstractButton *)), this,
          SLOT(slotGroupMode(QAbstractButton *)));
  ...
  m_cmbLineColor->setWhatsThis(
      tr("Select a color for the highlight lines drawn at each axis point"));
...
void DlgSettingsAxesChecker::createOptionalSaveDefault(
    QHBoxLayout * /* layout */) {}
...
void DlgSettingsAxesChecker::createPoints() {
  ...
  // Create an invisible rectangular item that will guarantee a margin all
  // around the outside, since otherwise QGraphicsView
  // will zoom in on the points
  QGraphicsRectItem *itemRect =
      new QGraphicsRectItem(0, 0, RECT_WIDTH, RECT_HEIGHT);
  itemRect->setPen(Qt::NoPen);
  m_scenePreview->addItem(itemRect);

  // For a realistic background, draw a rectangle underneath (lower z value),
  // and some tick marks
  QGraphicsRectItem *frameBox = new QGraphicsRectItem(
...
void DlgSettingsAxesChecker::createPreview(QGridLayout *layout, int &row) {
  ...
  m_viewPreview->setWhatsThis(tr("Preview window that shows how current "
                                 "settings affect the displayed axes checker"));
...
void DlgSettingsAxesChecker::handleOk() {
  ...
  CmdSettingsAxesChecker *cmd = new CmdSettingsAxesChecker(
      mainWindow(), cmdMediator().document(), *m_modelAxesCheckerBefore,
...
void DlgSettingsAxesChecker::load(CmdMediator &cmdMediator) {
  ...
  m_modelAxesCheckerBefore =
      new DocumentModelAxesChecker(cmdMediator.document());
  m_modelAxesCheckerAfter =
      new DocumentModelAxesChecker(cmdMediator.document());
  ...
  int indexSeconds = m_cmbSeconds->findData(
      QVariant(m_modelAxesCheckerAfter->checkerSeconds()));
  ENGAUGE_ASSERT(indexSeconds >= 0);
  m_cmbSeconds->setCurrentIndex(indexSeconds);

  int indexLineColor =
      m_cmbLineColor->findData(QVariant(m_modelAxesCheckerAfter->lineColor()));
...
void DlgSettingsAxesChecker::slotLineColor(const QString &) {
  ...
  m_modelAxesCheckerAfter->setLineColor(
...
void DlgSettingsAxesChecker::slotSeconds(const QString &) {
  ...
  m_modelAxesCheckerAfter->setCheckerSeconds(
      m_cmbSeconds->currentData().toInt());
  updateControls();
}
...
void DlgSettingsAxesChecker::updatePreview() {
  ...
  m_checker->prepareForDisplay(
      polygon, ZERO_RADIUS_SINCE_NO_POINTS, *m_modelAxesCheckerAfter,
      *m_modelCoords,
