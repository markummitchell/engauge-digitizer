...
DlgValidatorNumber::DlgValidatorNumber(CoordScale coordScale,
                                       const QLocale &locale, QObject *parent)
    : DlgValidatorAbstract(parent), m_coordScale(coordScale), m_locale(locale) {
  LOG4CPP_INFO_S((*mainCat)) << "DlgValidatorNumber::DlgValidatorNumber";
}

QValidator::State DlgValidatorNumber::validate(QString &input, int &pos) const {
