...
DlgSettingsMainWindow::DlgSettingsMainWindow(MainWindow &mainWindow)
    : DlgSettingsAbstractBase("MainWindow", "DlgSettingsMainWindow",
                              mainWindow),
      m_modelMainWindowBefore(0), m_modelMainWindowAfter(0) {
...
void DlgSettingsMainWindow::createControls(QGridLayout *layout, int &row) {
  ...
  m_cmbZoomFactor->addItem(LABEL_ZOOM_PREVIOUS,
                           QVariant(ZOOM_INITIAL_PREVIOUS));
  m_cmbZoomFactor->setWhatsThis(
      tr("Initial Zoom\n\n"
         "Select the initial zoom factor when a new document is loaded. Either "
         "the previous "
         "zoom can be kept, or the specified zoom can be applied."));
  connect(m_cmbZoomFactor, SIGNAL(currentTextChanged(const QString)), this,
          SLOT(slotZoomFactor(const QString)));
  ...
  m_cmbZoomControl->addItem(tr("Menu and mouse wheel"),
                            QVariant(ZOOM_CONTROL_MENU_WHEEL));
  m_cmbZoomControl->addItem(tr("Menu and +/- keys"),
                            QVariant(ZOOM_CONTROL_MENU_PLUSMINUS));
  m_cmbZoomControl->addItem(tr("Menu, mouse wheel and +/- keys"),
                            QVariant(ZOOM_CONTROL_MENU_WHEEL_PLUSMINUS));
  m_cmbZoomControl->setWhatsThis(
      tr("Zoom Control\n\n"
         "Select which inputs are used to zoom in and out."));
  connect(m_cmbZoomControl, SIGNAL(currentTextChanged(const QString)), this,
          SLOT(slotZoomControl(const QString)));
  ...
  m_cmbLocale->setWhatsThis(tr(
      "Locale\n\n"
      "Select the locale that will be used when converting between numbers and "
      "strings. "
      "This affects the use of commas or periods as group delimiters in each "
      "number entered "
      "by the user, displayed in the user interface, or exported to a file."));
  for (int indexLang = QLocale::C; indexLang <= QLocale::LastLanguage;
       indexLang++) {
    QLocale::Language lang = static_cast<QLocale::Language>(indexLang);
    QList<QLocale::Country> countries = QLocale::countriesForLanguage(lang);
    for (int indexCountry = 0; indexCountry < countries.count();
         indexCountry++) {
      QLocale::Country country = countries.at(indexCountry);
      QString label = localeLabel(lang, country);
      QLocale locale(lang, country);
      m_cmbLocale->addItem(label, locale);
    }
  }
  m_cmbLocale->model()->sort(COLUMN0); // Sort the new entries
  connect(m_cmbLocale, SIGNAL(currentIndexChanged(int)), this,
          SLOT(slotLocale(int)));
  ...
  m_btnRecentClear->setWhatsThis(
      tr("Recent File List Clear\n\n"
         "Clear the recent file list in the File menu."));
  connect(m_btnRecentClear, SIGNAL(pressed()), &mainWindow(),
          SLOT(slotRecentFileClear()));
  connect(m_btnRecentClear, SIGNAL(pressed()), this,
          SLOT(slotRecentFileClear()));
  layout->addWidget(m_btnRecentClear, row++, 2);
}

void DlgSettingsMainWindow::createOptionalSaveDefault(
    QHBoxLayout * /* layout */) {
...
void DlgSettingsMainWindow::handleOk() {
  ...
  CmdSettingsMainWindow *cmd = new CmdSettingsMainWindow(
      mainWindow(), cmdMediator().document(), *m_modelMainWindowBefore,
...
void DlgSettingsMainWindow::loadMainWindowModel(
    CmdMediator &cmdMediator, const MainWindowModel &modelMainWindow) {
  ...
  // Populate controls
  int index =
      m_cmbZoomFactor->findData(m_modelMainWindowAfter->zoomFactorInitial());
...
void DlgSettingsMainWindow::slotZoomControl(const QString) {
  ...
  m_modelMainWindowAfter->setZoomControl(
      (ZoomControl)m_cmbZoomControl->currentData().toInt());
  updateControls();
}
...
void DlgSettingsMainWindow::slotZoomFactor(const QString) {
  ...
  m_modelMainWindowAfter->setZoomFactorInitial(
      (ZoomFactorInitial)m_cmbZoomFactor->currentData().toInt());
  updateControls();
}

void DlgSettingsMainWindow::updateControls() { enableOk(true); }
