...
DlgValidatorFactory::createCartesianOrPolarWithNonPolarPolar(
    CoordScale coordScale, bool isCartesian,
    CoordUnitsNonPolarTheta coordUnitsCartesian,
    CoordUnitsNonPolarTheta coordUnitsPolar, CoordUnitsDate coordUnitsDate,
    CoordUnitsTime coordUnitsTime, const QLocale &locale) const {
  LOG4CPP_INFO_S((*mainCat))
      << "DlgValidatorFactory::createCartesianOrPolarWithNonPolarPolar";

  if (isCartesian) {
    return createWithNonPolar(coordScale, coordUnitsCartesian, coordUnitsDate,
                              coordUnitsTime, locale);
  } else {
    return createWithNonPolar(coordScale, coordUnitsPolar, coordUnitsDate,
                              coordUnitsTime, locale);
  }
}

DlgValidatorAbstract *DlgValidatorFactory::createCartesianOrPolarWithPolarPolar(
    CoordScale coordScale, bool isCartesian,
    CoordUnitsNonPolarTheta coordUnitsCartesian,
    CoordUnitsPolarTheta coordUnitsPolar, CoordUnitsDate coordUnitsDate,
    CoordUnitsTime coordUnitsTime, const QLocale &locale) const {
  LOG4CPP_INFO_S((*mainCat))
  ...
  if (isCartesian) {
    ...
    return createWithPolar(coordScale, coordUnitsPolar, locale);
  }
}

DlgValidatorAbstract *DlgValidatorFactory::createWithNonPolar(
    CoordScale coordScale, CoordUnitsNonPolarTheta coordUnits,
    CoordUnitsDate coordUnitsDate, CoordUnitsTime coordUnitsTime,
    const QLocale &locale) const {
  LOG4CPP_INFO_S((*mainCat)) << "DlgValidatorFactory::createWithNonPolar";

  switch (coordUnits) {
  case COORD_UNITS_NON_POLAR_THETA_DATE_TIME:
    return new DlgValidatorDateTime(coordScale, coordUnitsDate, coordUnitsTime);
  ...
  case COORD_UNITS_NON_POLAR_THETA_NUMBER:
    return new DlgValidatorNumber(coordScale, locale);
...
DlgValidatorAbstract *
DlgValidatorFactory::createWithPolar(CoordScale coordScale,
