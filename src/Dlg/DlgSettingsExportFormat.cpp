...
DlgSettingsExportFormat::DlgSettingsExportFormat(MainWindow &mainWindow)
    : DlgSettingsAbstractBase("Export Format", "DlgSettingsExportFormat",
                              mainWindow),
      m_modelExportBefore(0), m_modelExportAfter(0) {
...
void DlgSettingsExportFormat::createCurveSelection(QGridLayout *layout,
                                                   int &row) {
  ...
  m_listIncluded->setWhatsThis(
      tr("List of curves to be included in the exported file.\n\n"
         "The order of the curves here does not affect the order in the "
         "exported file. That "
  ...
  connect(m_listIncluded, SIGNAL(itemSelectionChanged()), this,
          SLOT(slotListIncluded()));

  m_listExcluded = new QListWidget;
  m_listExcluded->setWhatsThis(
      tr("List of curves to be excluded from the exported file"));
  m_listExcluded->setSelectionMode(QAbstractItemView::MultiSelection);
  layout->addWidget(m_listExcluded, row++, 2, 4, 1);
  connect(m_listExcluded, SIGNAL(itemSelectionChanged()), this,
          SLOT(slotListExcluded()));

  m_btnInclude = new QPushButton(tr("<<Include"));
  m_btnInclude->setEnabled(false);
  m_btnInclude->setWhatsThis(
      tr("Move the currently selected curve(s) from the excluded list"));
  ...
  m_btnExclude->setWhatsThis(
      tr("Move the currently selected curve(s) from the included list"));
...
void DlgSettingsExportFormat::createDelimiters(QHBoxLayout *layoutMisc) {
  ...
  m_btnDelimitersCommas =
      new QRadioButton(exportDelimiterToString(EXPORT_DELIMITER_COMMA));
  m_btnDelimitersCommas->setWhatsThis(
      tr("Exported file will have commas between adjacent values.\n\n"
         "This setting is overridden for TSV files"));
  layoutDelimiters->addWidget(m_btnDelimitersCommas);
  connect(m_btnDelimitersCommas, SIGNAL(released()), this,
          SLOT(slotDelimitersCommas()));

  m_btnDelimitersSpaces =
      new QRadioButton(exportDelimiterToString(EXPORT_DELIMITER_SPACE));
  m_btnDelimitersSpaces->setWhatsThis(
      tr("Exported file will have spaces between adjacent values.\n\n"
         "This setting is overridden for CSV and TSV files"));
  layoutDelimiters->addWidget(m_btnDelimitersSpaces);
  connect(m_btnDelimitersSpaces, SIGNAL(released()), this,
          SLOT(slotDelimitersSpaces()));

  m_btnDelimitersTabs =
      new QRadioButton(exportDelimiterToString(EXPORT_DELIMITER_TAB));
  m_btnDelimitersTabs->setWhatsThis(
      tr("Exported file will have tabs between adjacent values.\n\n"
         "This setting is overridden for CSV files"));
  layoutDelimiters->addWidget(m_btnDelimitersTabs);
  connect(m_btnDelimitersTabs, SIGNAL(released()), this,
          SLOT(slotDelimitersTabs()));
}
...
void DlgSettingsExportFormat::createFileLayout(QHBoxLayout *layoutMisc) {
  ...
  m_btnFunctionsLayoutAllCurves =
      new QRadioButton(tr("All curves on each line"));
  m_btnFunctionsLayoutAllCurves->setWhatsThis(
      tr("Exported file will have, on each line, "
         "an X value, the Y value for the first curve, the Y value for the "
         "second curve,..."));
  layoutLayout->addWidget(m_btnFunctionsLayoutAllCurves);
  connect(m_btnFunctionsLayoutAllCurves, SIGNAL(released()), this,
          SLOT(slotFunctionsLayoutAllCurves()));

  m_btnFunctionsLayoutOneCurve = new QRadioButton(tr("One curve on each line"));
  m_btnFunctionsLayoutOneCurve->setWhatsThis(
      tr("Exported file will have all the points for "
         "the first curve, with one X-Y pair on each line, then the points for "
         "the second curve,..."));
  layoutLayout->addWidget(m_btnFunctionsLayoutOneCurve);
  connect(m_btnFunctionsLayoutOneCurve, SIGNAL(released()), this,
          SLOT(slotFunctionsLayoutOneCurve()));
}

void DlgSettingsExportFormat::createFunctionsPointsSelection(
  ...
  int row = 0;
  m_btnFunctionsPointsAllCurves =
      new QRadioButton(tr("Interpolate Ys at Xs from all curves"));
  m_btnFunctionsPointsAllCurves->setWhatsThis(
      tr("Exported file will have values at every unique X "
         "value from every curve. Y values will be linearly interpolated if "
         "necessary"));
  layoutPointsSelections->addWidget(m_btnFunctionsPointsAllCurves, row++, 0, 1,
                                    4);
  connect(m_btnFunctionsPointsAllCurves, SIGNAL(released()), this,
          SLOT(slotFunctionsPointsAllCurves()));

  m_btnFunctionsPointsFirstCurve =
      new QRadioButton(tr("Interpolate Ys at Xs from first curve"));
  m_btnFunctionsPointsFirstCurve->setWhatsThis(
      tr("Exported file will have values at every unique X "
         "value from the first curve. Y values will be linearly interpolated "
         "if necessary"));
  layoutPointsSelections->addWidget(m_btnFunctionsPointsFirstCurve, row++, 0, 1,
                                    4);
  connect(m_btnFunctionsPointsFirstCurve, SIGNAL(released()), this,
          SLOT(slotFunctionsPointsFirstCurve()));

  m_btnFunctionsPointsEvenlySpaced =
      new QRadioButton(tr("Interpolate Ys at evenly spaced X values."));
  m_btnFunctionsPointsEvenlySpaced->setWhatsThis(
      tr("Exported file will have values at evenly spaced X values, separated "
         "by the interval selected below."));
  layoutPointsSelections->addWidget(m_btnFunctionsPointsEvenlySpaced, row++, 0,
                                    1, 4);
  connect(m_btnFunctionsPointsEvenlySpaced, SIGNAL(released()), this,
          SLOT(slotFunctionsPointsEvenlySpaced()));

  QLabel *labelInterval = new QLabel(tr("Interval:"));
  layoutPointsSelections->addWidget(labelInterval, row, 1, 1, 1,
                                    Qt::AlignRight);

  m_editFunctionsPointsEvenlySpacing = new QLineEdit;
  m_validatorFunctionsPointsEvenlySpacing =
      new QDoubleValidator; // Minimum value, to prevent overflow, is set later
                            // according to settings
  m_editFunctionsPointsEvenlySpacing->setValidator(
      m_validatorFunctionsPointsEvenlySpacing);
  ...
  m_editFunctionsPointsEvenlySpacing->setWhatsThis(
      tr("Interval, in the units of X, between successive points in the X "
         "direction.\n\n"
         "If the scale is linear, then this interval is added to successive X "
         "values. If the scale is "
         "logarithmic, then this interval is multiplied to successive X "
         "values.\n\n"
         "The X values will be automatically aligned along simple numbers. If "
         "the first and/or last "
         "points are not along the aligned X values, then one or two "
         "additional points are added "
         "as necessary."));
  layoutPointsSelections->addWidget(m_editFunctionsPointsEvenlySpacing, row, 2,
                                    1, 1, Qt::AlignLeft);
  connect(m_editFunctionsPointsEvenlySpacing,
          SIGNAL(textChanged(const QString &)), this,
          SLOT(slotFunctionsPointsEvenlySpacedInterval(const QString &)));

  m_cmbFunctionsPointsEvenlySpacingUnits = new QComboBox;
  m_cmbFunctionsPointsEvenlySpacingUnits->setWhatsThis(
      tr("Units for spacing interval.\n\n"
         "Pixel units are preferred when the spacing is to be independent of "
         "the X scale. The spacing will be "
         "consistent across the graph, even if the X scale is logarithmic.\n\n"
         "Graph units are preferred when the spacing is to depend on the X "
         "scale."));
  m_cmbFunctionsPointsEvenlySpacingUnits->addItem(
      exportPointsIntervalUnitsToString(EXPORT_POINTS_INTERVAL_UNITS_GRAPH),
      QVariant(EXPORT_POINTS_INTERVAL_UNITS_GRAPH));
  m_cmbFunctionsPointsEvenlySpacingUnits->addItem(
      exportPointsIntervalUnitsToString(EXPORT_POINTS_INTERVAL_UNITS_SCREEN),
  ...
  layoutPointsSelections->addWidget(m_cmbFunctionsPointsEvenlySpacingUnits,
                                    row++, 3, 1, 1, Qt::AlignLeft);

  m_btnFunctionsPointsRaw = new QRadioButton(tr("Raw Xs and Ys"));
  m_btnFunctionsPointsRaw->setWhatsThis(
      tr("Exported file will have only original X and Y values"));
  layoutPointsSelections->addWidget(m_btnFunctionsPointsRaw, row++, 0, 1, 4);
  connect(m_btnFunctionsPointsRaw, SIGNAL(released()), this,
          SLOT(slotFunctionsPointsRaw()));
}
...
void DlgSettingsExportFormat::createHeader(QHBoxLayout *layoutMisc) {
  ...
  m_btnHeaderSimple->setWhatsThis(
      tr("Exported file will have simple header line"));
  layoutHeader->addWidget(m_btnHeaderSimple, row++, COLUMN_RADIO_BUTTONS, 1, 1);
  connect(m_btnHeaderSimple, SIGNAL(released()), this,
          SLOT(slotHeaderSimple()));

  m_btnHeaderGnuplot =
      new QRadioButton(exportHeaderToString(EXPORT_HEADER_GNUPLOT));
  m_btnHeaderGnuplot->setWhatsThis(
      tr("Exported file will have gnuplot header line"));
  layoutHeader->addWidget(m_btnHeaderGnuplot, row++, COLUMN_RADIO_BUTTONS, 1,
                          1);
  connect(m_btnHeaderGnuplot, SIGNAL(released()), this,
          SLOT(slotHeaderGnuplot()));

  createXLabel(layoutHeader, COLUMN_LABEL);
}
...
void DlgSettingsExportFormat::createOptionalSaveDefault(QHBoxLayout *layout) {
  ...
  m_btnSaveDefault->setWhatsThis(
      tr("Save the settings for use as future defaults."));
...
void DlgSettingsExportFormat::createPreview(QGridLayout *layout, int &row) {
  ...
  m_editPreview->setWhatsThis(
      tr("Preview window shows how current settings affect the exported file"));
  m_editPreview->setMinimumHeight(MINIMUM_PREVIEW_HEIGHT);

  layout->addWidget(m_editPreview, row++, 0, 1, 3);
}

void DlgSettingsExportFormat::createRelationsPointsSelection(
  ...
  int row = 0;
  m_btnRelationsPointsEvenlySpaced =
      new QRadioButton(tr("Interpolate Xs and Ys at evenly spaced intervals."));
  m_btnRelationsPointsEvenlySpaced->setWhatsThis(
      tr("Exported file will have points evenly spaced along each relation, "
         "separated by the interval "
         "selected below. If the last interval does not end at the last point, "
         "then a shorter last interval "
         "is added that ends on the last point."));
  layoutPointsSelections->addWidget(m_btnRelationsPointsEvenlySpaced, row++, 0,
                                    1, 4);
  connect(m_btnRelationsPointsEvenlySpaced, SIGNAL(released()), this,
          SLOT(slotRelationsPointsEvenlySpaced()));
  ...
  m_validatorRelationsPointsEvenlySpacing =
      new QDoubleValidator; // Minimum value, to prevent overflow, is set later
                            // according to settings
  m_editRelationsPointsEvenlySpacing->setValidator(
      m_validatorRelationsPointsEvenlySpacing);
  ...
  m_editRelationsPointsEvenlySpacing->setWhatsThis(
      tr("Interval between successive points when "
         "exporting at evenly spaced (X,Y) coordinates."));
  layoutPointsSelections->addWidget(m_editRelationsPointsEvenlySpacing, row, 2,
                                    1, 1, Qt::AlignLeft);
  connect(m_editRelationsPointsEvenlySpacing,
          SIGNAL(textChanged(const QString &)), this,
          SLOT(slotRelationsPointsEvenlySpacedInterval(const QString &)));

  m_cmbRelationsPointsEvenlySpacingUnits = new QComboBox;
  m_cmbRelationsPointsEvenlySpacingUnits->setWhatsThis(
      tr("Units for spacing interval.\n\n"
         "Pixel units are preferred when the spacing is to be independent of "
         "the X and Y scales. The spacing will be "
         "consistent across the graph, even if a scale is logarithmic or the X "
         "and Y scales are different.\n\n"
         "Graph units are usually preferred when the X and Y scales are "
         "identical."));
  m_cmbRelationsPointsEvenlySpacingUnits->addItem(
      exportPointsIntervalUnitsToString(EXPORT_POINTS_INTERVAL_UNITS_GRAPH),
      QVariant(EXPORT_POINTS_INTERVAL_UNITS_GRAPH));
  m_cmbRelationsPointsEvenlySpacingUnits->addItem(
      exportPointsIntervalUnitsToString(EXPORT_POINTS_INTERVAL_UNITS_SCREEN),
  ...
  connect(m_cmbRelationsPointsEvenlySpacingUnits,
          ...
          SLOT(slotRelationsPointsEvenlySpacedIntervalUnits(
              const QString &))); // activated() ignores code changes
  layoutPointsSelections->addWidget(m_cmbRelationsPointsEvenlySpacingUnits,
                                    row++, 3, 1, 1, Qt::AlignLeft);

  m_btnRelationsPointsRaw = new QRadioButton(tr("Raw Xs and Ys"));
  m_btnRelationsPointsRaw->setWhatsThis(
      tr("Exported file will have only original X and Y values"));
  layoutPointsSelections->addWidget(m_btnRelationsPointsRaw, row++, 0, 1, 4);
  connect(m_btnRelationsPointsRaw, SIGNAL(released()), this,
          SLOT(slotRelationsPointsRaw()));
}
...
void DlgSettingsExportFormat::createTabWidget(QGridLayout *layout, int &row) {
...
void DlgSettingsExportFormat::createXLabel(QGridLayout *layoutHeader,
  ...
  connect(m_editXLabel, SIGNAL(textChanged(const QString &)), this,
          SLOT(slotXLabel(const QString &)));
}
...
bool DlgSettingsExportFormat::goodIntervalFunctions() const {
  ...
  bool isGood = (m_validatorFunctionsPointsEvenlySpacing->validate(
                     textFunctions, posFunctions) == QValidator::Acceptable);
...
bool DlgSettingsExportFormat::goodIntervalRelations() const {
  ...
  return (m_validatorRelationsPointsEvenlySpacing->validate(
              textRelations, posRelations) == QValidator::Acceptable);
}

void DlgSettingsExportFormat::handleOk() {
  LOG4CPP_INFO_S((*mainCat)) << "DlgSettingsExportFormat::handleOk";

  CmdSettingsExportFormat *cmd =
      new CmdSettingsExportFormat(mainWindow(), cmdMediator().document(),
                                  *m_modelExportBefore, *m_modelExportAfter);
...
void DlgSettingsExportFormat::initializeIntervalConstraints() {
  ...
  Functor2wRet<const QString &, const Point &, CallbackSearchReturn>
      ftorWithCallback = functor_ret(ftor, &CallbackBoundingRects::callback);
  cmdMediator().iterateThroughCurvesPointsGraphs(ftorWithCallback);

  // If there are no points, then interval will be zero. That special case must
  // be handled downstream to prevent infinite loops
  bool isEmpty;
...
void DlgSettingsExportFormat::load(CmdMediator &cmdMediator) {
  ...
  for (itr = curveNamesExcluded.begin(); itr != curveNamesExcluded.end();
       ++itr) {
  ...
  ExportPointsSelectionFunctions pointsSelectionFunctions =
      m_modelExportAfter->pointsSelectionFunctions();
  m_btnFunctionsPointsAllCurves->setChecked(
      pointsSelectionFunctions ==
      EXPORT_POINTS_SELECTION_FUNCTIONS_INTERPOLATE_ALL_CURVES);
  m_btnFunctionsPointsFirstCurve->setChecked(
      pointsSelectionFunctions ==
      EXPORT_POINTS_SELECTION_FUNCTIONS_INTERPOLATE_FIRST_CURVE);
  m_btnFunctionsPointsEvenlySpaced->setChecked(
      pointsSelectionFunctions ==
      EXPORT_POINTS_SELECTION_FUNCTIONS_INTERPOLATE_PERIODIC);
  m_btnFunctionsPointsRaw->setChecked(pointsSelectionFunctions ==
                                      EXPORT_POINTS_SELECTION_FUNCTIONS_RAW);

  ExportLayoutFunctions layoutFunctions = m_modelExportAfter->layoutFunctions();
  m_btnFunctionsLayoutAllCurves->setChecked(layoutFunctions ==
                                            EXPORT_LAYOUT_ALL_PER_LINE);
  m_btnFunctionsLayoutOneCurve->setChecked(layoutFunctions ==
                                           EXPORT_LAYOUT_ONE_PER_LINE);

  ExportPointsSelectionRelations pointsSelectionRelations =
      m_modelExportAfter->pointsSelectionRelations();
  m_btnRelationsPointsEvenlySpaced->setChecked(
      pointsSelectionRelations ==
      EXPORT_POINTS_SELECTION_RELATIONS_INTERPOLATE);
  m_btnRelationsPointsRaw->setChecked(pointsSelectionRelations ==
  ...
  m_editFunctionsPointsEvenlySpacing->setText(
      QString::number(m_modelExportAfter->pointsIntervalFunctions()));
  m_editRelationsPointsEvenlySpacing->setText(
      QString::number(m_modelExportAfter->pointsIntervalRelations()));

  ExportPointsIntervalUnits pointsIntervalUnitsFunctions =
      m_modelExportAfter->pointsIntervalUnitsRelations();
  ExportPointsIntervalUnits pointsIntervalUnitsRelations =
      m_modelExportAfter->pointsIntervalUnitsRelations();
  int indexFunctions = m_cmbRelationsPointsEvenlySpacingUnits->findData(
      QVariant(pointsIntervalUnitsFunctions));
  int indexRelations = m_cmbRelationsPointsEvenlySpacingUnits->findData(
...
void DlgSettingsExportFormat::slotFunctionsPointsAllCurves() {
  ...
  m_modelExportAfter->setPointsSelectionFunctions(
      EXPORT_POINTS_SELECTION_FUNCTIONS_INTERPOLATE_ALL_CURVES);
  updateControls();
  updatePreview();
}
...
void DlgSettingsExportFormat::slotFunctionsPointsEvenlySpacedInterval(
    const QString &) {
  LOG4CPP_INFO_S((*mainCat))
      << "DlgSettingsExportFormat::slotFunctionsPointsEvenlySpacedInterval";

  // Prevent infinite loop on empty and "-" values which get treated as zero
  // interval
  if (goodIntervalFunctions()) {
    m_modelExportAfter->setPointsIntervalFunctions(
        m_editFunctionsPointsEvenlySpacing->text().toDouble());
    updateControls();
    updatePreview();
  } else {
...
void DlgSettingsExportFormat::slotFunctionsPointsEvenlySpacedIntervalUnits(
    const QString &) {
  LOG4CPP_INFO_S((*mainCat)) << "DlgSettingsExportFormat::"
                                "slotFunctionsPointsEvenlySpacedIntervalUnits";

  int index = m_cmbFunctionsPointsEvenlySpacingUnits->currentIndex();
  ExportPointsIntervalUnits units =
      (ExportPointsIntervalUnits)
          m_cmbFunctionsPointsEvenlySpacingUnits->itemData(index)
              .toInt();

  m_modelExportAfter->setPointsIntervalUnitsFunctions(units);
  updateIntervalConstraints(); // Call this before updateControls so constraint
                               // checking is updated for ok button
  updateControls();
  updatePreview();
}
...
void DlgSettingsExportFormat::slotRelationsPointsEvenlySpaced() {
  ...
  m_modelExportAfter->setPointsSelectionRelations(
      EXPORT_POINTS_SELECTION_RELATIONS_INTERPOLATE);
  updateControls();
  updatePreview();
}

void DlgSettingsExportFormat::slotRelationsPointsEvenlySpacedInterval(
    const QString &) {
  LOG4CPP_INFO_S((*mainCat))
      << "DlgSettingsExportFormat::slotRelationsPointsEvenlySpacedInterval";

  m_modelExportAfter->setPointsIntervalRelations(
      m_editRelationsPointsEvenlySpacing->text().toDouble());
  updateControls();
  updatePreview();
}

void DlgSettingsExportFormat::slotRelationsPointsEvenlySpacedIntervalUnits(
    const QString &) {
  LOG4CPP_INFO_S((*mainCat)) << "DlgSettingsExportFormat::"
                                "slotRelationsPointsEvenlySpacedIntervalUnits";

  int index = m_cmbRelationsPointsEvenlySpacingUnits->currentIndex();
  ExportPointsIntervalUnits units =
      (ExportPointsIntervalUnits)
          m_cmbRelationsPointsEvenlySpacingUnits->itemData(index)
              .toInt();
...
void DlgSettingsExportFormat::updateControls() {
  bool isGoodState = goodIntervalFunctions() && goodIntervalRelations();
  ...
  m_btnInclude->setEnabled(selectedForInclude >
                           0); // Need at least one selection
  m_btnExclude->setEnabled(
      (selectedForExclude > 0) &&
      (inInclude - selectedForExclude >
       0)); // Need at least one selection, and one left after the move

  m_editFunctionsPointsEvenlySpacing->setEnabled(
      m_btnFunctionsPointsEvenlySpaced->isChecked());
  m_editRelationsPointsEvenlySpacing->setEnabled(
      m_btnRelationsPointsEvenlySpaced->isChecked());

  m_editXLabel->setEnabled(!m_btnHeaderNone->isChecked());
}

void DlgSettingsExportFormat::updateIntervalConstraints() {
  double functionsMin = (m_modelExportAfter->pointsIntervalUnitsFunctions() ==
                                 EXPORT_POINTS_INTERVAL_UNITS_GRAPH
                             ? m_minIntervalGraph
                             : m_minIntervalScreen);
  double relationsMin = (m_modelExportAfter->pointsIntervalUnitsRelations() ==
                                 EXPORT_POINTS_INTERVAL_UNITS_GRAPH
                             ? m_minIntervalGraph
                             : m_minIntervalScreen);
...
void DlgSettingsExportFormat::updatePreview() {
  ...
  if (mainWindow().transformation().transformIsDefined()) {
    ...
    if (m_tabWidget->currentIndex() == TAB_WIDGET_INDEX_FUNCTIONS) {
      ...
      exportStrategy.exportToFile(*m_modelExportAfter, cmdMediator().document(),
                                  mainWindow().modelMainWindow(),
                                  mainWindow().transformation(), str);
