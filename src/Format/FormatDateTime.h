...
public:
  ...
  QString formatOutput(CoordUnitsDate coordUnitsDate,
                       CoordUnitsTime coordUnitsTime, double value) const;

  /// Parse the input string into a time value. Success flag is false if parsing
  /// failed.
...
private:
  ...
  void dateTimeLookup(
      const FormatsDate &formatsDate, const FormatsTime &formatsTime,
      CoordUnitsDate coordUnitsDate, CoordUnitsTime coordUnitsTime,
      const QString &string, bool useQDateTimeElseQRegExp,
  ...
  // For parsing input, using built in Qt date/time parsing according to
  // QDateTime
  FormatsDate m_formatsDateParseAcceptable;
  FormatsTime m_formatsTimeParseAcceptable;

  // For parsing input, after input has been found to not be Acceptable. Regular
  // expressions are used since Qt date/time
  // parsing of Intermediate strings does not work. Example, 'J' and 'Ja' would
  // be rejected although they are typed
