...
const double SECONDS_TO_DEGREES =
    1.0 / (DEGREES_TO_MINUTES * MINUTES_TO_SECONDS);

FormatDegreesMinutesSecondsBase::FormatDegreesMinutesSecondsBase() {}

FormatDegreesMinutesSecondsBase::~FormatDegreesMinutesSecondsBase() {}

QString FormatDegreesMinutesSecondsBase::formatOutputDegreesMinutesSeconds(
    double value) const {
...
QString FormatDegreesMinutesSecondsBase::formatOutputDegreesMinutesSecondsNsew(
    double value, bool isNsHemisphere) const {
  LOG4CPP_INFO_S((*mainCat))
...
QValidator::State
FormatDegreesMinutesSecondsBase::parseInput(const QString &stringUntrimmed,
                                            double &value) const {
  ...
  // Split on spaces
  QStringList fields = string.split(QRegExp("\\s+"), QString::SkipEmptyParts);
  ...
  stripSymbols(field0, field1, field2);
  ...
  // Required degrees
  QValidator::State state = valDegrees.validate(field0, pos);
  if (state == QValidator::Acceptable) {
    ...
    if (fields.count() > 1) {
      ...
      // Optional minutes
      state = valMinutesOrSeconds.validate(field1, pos);
      if (state == QValidator::Acceptable) {
        ...
        if (fields.count() > 2) {
          ...
          // Optional seconds
          state = valMinutesOrSeconds.validate(field2, pos);
          if (state == QValidator::Acceptable) {
  ...
  if (state == QValidator::Acceptable) {
    if (valueDegrees < 0) {
      ...
      // Apply the negative sign on the degrees components to minutes and
      // seconds components also
      value = valueDegrees - valueMinutes * MINUTES_TO_DEGREES -
              valueSeconds * SECONDS_TO_DEGREES;
...
void FormatDegreesMinutesSecondsBase::stripSymbols(QString &field0,
  ...
  // Clean up degrees symbols (167 or 248 in base 10 which are 247 and 370 in
  // base 8) and single quotes
  QString strExpDegrees =
      QString(".*\\0%1$").arg(COORD_SYMBOL_DEGREES, FIELD_WIDTH, BASE_8);
