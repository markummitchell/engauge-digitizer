...
QXmlStreamReader::TokenType loadNextFromReader(QXmlStreamReader &reader) {
  ...
  LOG4CPP_DEBUG_S((*mainCat))
      << "loadNextFromReader "
      << " tokenType="
      << QXmlStreamReaderTokenTypeToString(tokenType).toLatin1().data()
...
void xmlExitWithError(QXmlStreamReader &reader, const QString &message) {
  ...
  QMessageBox::critical(0, QObject::tr("Error reading xml"), adornedMsg);

  LOG4CPP_ERROR_S((*mainCat)) << "xmlExitWithError error='"
                              << adornedMsg.toLatin1().data() << "'";
