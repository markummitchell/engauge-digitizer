...
/// Draw an arc as an ellipse but without lines from the center to the start and
/// end points
class GraphicsArcItem : public QGraphicsEllipseItem {
public:
  /// Constructor with individual coordinates
  GraphicsArcItem(double x, double y, double width, double height,
                  QGraphicsItem *parent = 0);

  /// Constructor with coordinates specified as rectangle
  GraphicsArcItem(const QRectF &rect, QGraphicsItem *parent = 0);

  /// QGraphicsEllipseItem ignored transformation when computing bounding
  /// rectangle, although it should
