...
/// QGraphicsView class with event handling added. Typically the events are sent
/// to the active digitizing state.
...
public:
  ...
  /// Intercept mouse drop event to support drag-and-drop. This initiates
  /// asynchronous loading of the dragged image
  ...
  /// Intercept mouse move events to populate the current cursor position in
  /// StatusBar.
...
signals:
  ...
  /// Send dragged dig file to MainWindow for import. This comes from dragging
  /// an engauge dig file
  void signalDraggedDigFile(QString);

  /// Send dragged image to MainWindow for import. This typically comes from
  /// dragging a file
  void signalDraggedImage(QImage);

  /// Send dragged url to MainWindow for import. This typically comes from
  /// dragging an image from a browser
  void signalDraggedImageUrl(QUrl);

  /// Send keypress to MainWindow for eventual processing by
  ...
  /// Send mouse move to MainWindow for eventual display of cursor coordinates
  /// in StatusBar
