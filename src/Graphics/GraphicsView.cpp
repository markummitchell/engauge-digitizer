...
GraphicsView::GraphicsView(QGraphicsScene *scene, MainWindow &mainWindow)
    : QGraphicsView(scene) {
  connect(this, SIGNAL(signalContextMenuEvent(QString)), &mainWindow,
          SLOT(slotContextMenuEvent(QString)));
  connect(this, SIGNAL(signalDraggedDigFile(QString)), &mainWindow,
          SLOT(slotFileOpenDraggedDigFile(QString)));
  connect(this, SIGNAL(signalDraggedImage(QImage)), &mainWindow,
          SLOT(slotFileImportDraggedImage(QImage)));
  connect(this, SIGNAL(signalDraggedImageUrl(QUrl)), &mainWindow,
          SLOT(slotFileImportDraggedImageUrl(QUrl)));
  connect(this, SIGNAL(signalKeyPress(Qt::Key, bool)), &mainWindow,
          SLOT(slotKeyPress(Qt::Key, bool)));
  connect(this, SIGNAL(signalLeave()), &mainWindow, SLOT(slotLeave()));
  connect(this, SIGNAL(signalMouseMove(QPointF)), &mainWindow,
          SLOT(slotMouseMove(QPointF)));
  connect(this, SIGNAL(signalMousePress(QPointF)), &mainWindow,
          SLOT(slotMousePress(QPointF)));
  connect(this, SIGNAL(signalMouseRelease(QPointF)), &mainWindow,
          SLOT(slotMouseRelease(QPointF)));
  connect(this, SIGNAL(signalViewZoomIn()), &mainWindow,
          SLOT(slotViewZoomInFromWheelEvent()));
  connect(this, SIGNAL(signalViewZoomOut()), &mainWindow,
          SLOT(slotViewZoomOutFromWheelEvent()));
  ...
  // Skip setStatusTip here since that will overwrite much more important
  // messages, and trigger gratuitous showing of status bar
  setWhatsThis(tr(
      "Main Window\n\n"
      "After an image file is imported, or an Engauge Document opened, an "
      "image appears in this area. "
      "Points are added to the image.\n\n"
      "If the image is a graph with two axes and one or more curves, then "
      "three axis points must be "
      "created along those axes. Just put two axis points on one axis and a "
      "third axis point on the other "
      "axis, as far apart as possible for higher accuracy. Then curve points "
      "can be added along the curves.\n\n"
      "If the image is a map with a scale to define length, then two axis "
      "points must be "
...
void GraphicsView::contextMenuEvent(QContextMenuEvent *event) {
  ...
  if (items.count() == 1) {
    ...
    GraphicsItemType type =
        (GraphicsItemType)item->data(DATA_KEY_GRAPHICS_ITEM_TYPE).toInt();
    QString curveName = Point::curveNameFromPointIdentifier(pointIdentifier);

    if ((type == GRAPHICS_ITEM_TYPE_POINT) && (curveName == AXIS_CURVE_NAME)) {
...
void GraphicsView::dragEnterEvent(QDragEnterEvent *event) {
  LOG4CPP_INFO_S((*mainCat))
      ...
      << (event->mimeData()->hasUrls() ? "urls" : "non-urls");
...
void GraphicsView::dropEvent(QDropEvent *event) {
  ...
  LOG4CPP_INFO_S((*mainCat))
      ...
      << " formats=("
      << event->mimeData()->formats().join(", ").toLatin1().data() << ")"
      << " hasUrls=" << (event->mimeData()->hasUrls() ? "yes" : "no")
      << " urlCount=" << urlList.count() << " urls=(" << urls.toLatin1().data()
      << ")"
  ...
  if (loadFileInfo.loadsAsDigFile(textPlain)) {
    ...
    // This branch never seems to get executed, but will be kept in case it ever
    // applies (since hasUrls branch is messier and less reliable)
    QImage image = qvariant_cast<QImage>(event->mimeData()->imageData());
    LOG4CPP_INFO_S((*mainCat)) << "GraphicsView::dropEvent image";
    emit signalDraggedImage(image);

  } else if (event->mimeData()->hasUrls() && urlList.count() > 0) {

    // Sometimes images can be dragged in, but sometimes the url points to an
    // html page that
    // contains just the image, in which case importing will fail
    QUrl url = urlList.at(0);
    LOG4CPP_INFO_S((*mainCat)) << "GraphicsView::dropEvent url="
...
bool GraphicsView::inBounds(const QPointF &posScreen) {
  ...
  return 0 <= posScreen.x() && 0 <= posScreen.y() &&
...
void GraphicsView::keyPressEvent(QKeyEvent *event) {
  ...
  if (key == Qt::Key_Down || key == Qt::Key_Left || key == Qt::Key_Right ||
      key == Qt::Key_Up) {
...
void GraphicsView::mouseReleaseEvent(QMouseEvent *event) {
  ...
  // Send a signal, unless this is a right click. We still send if out of bounds
  // since
  // a click-and-drag often ends out of bounds (and user is unlikely to expect
  // different
...
void GraphicsView::wheelEvent(QWheelEvent *event) {
  ...
  // Criteria:
  // 1) User has enabled wheel zoom control (but that is not known here so
  // MainWindow will handle that part)
  //    in slotViewZoomInFromWheelEvent and slotViewZoomOutFromWheelEvent
  // 2) Angle is over a threshold to eliminate false events from just touching
  // wheel
  if ((event->modifiers() & Qt::ControlModifier) != 0) {
    ...
    // Accept the event as long as Control key was used and we are capturing
    // wheel event
    event->accept();
