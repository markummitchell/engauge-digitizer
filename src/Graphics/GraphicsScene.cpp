...
void GraphicsScene::addTemporaryPoint(const QString &identifier,
  ...
  m_graphicsLinesForCurves.addPoint(AXIS_CURVE_NAME, identifier,
                                    Point::UNDEFINED_ORDINAL(), *point);
}
...
GraphicsPoint *GraphicsScene::createPoint(const QString &identifier,
  ...
  // Ordinal value is initially computed as one plus the max ordinal seen so
  // far. This initial ordinal value will be overridden if the
  ...
  GraphicsPoint *point =
      pointFactory.createPoint(*this, identifier, posScreen, pointStyle);
...
QString GraphicsScene::dumpCursors() const {
  QString cursorOverride =
      (QApplication::overrideCursor() != 0)
...
void GraphicsScene::hideAllItemsExceptImage() {
  ...
  for (int index = 0; index < QGraphicsScene::items().count(); index++) {
    ...
    if (item->data(DATA_KEY_GRAPHICS_ITEM_TYPE).toInt() ==
...
void GraphicsScene::printStream(QString indentation, QTextStream &str) {
  m_graphicsLinesForCurves.printStream(indentation, str);
}

void GraphicsScene::removePoint(const QString &identifier) {
  LOG4CPP_INFO_S((*mainCat)) << "GraphicsScene::removePoint identifier="
...
void GraphicsScene::showCurves(bool show, bool showAll,
  ...
  for (itr = items.begin(); itr != items.end(); itr++) {
    ...
    bool isPoint = (item->data(DATA_KEY_GRAPHICS_ITEM_TYPE).toInt() ==
                    GRAPHICS_ITEM_TYPE_POINT);
    bool isCurve = (item->data(DATA_KEY_GRAPHICS_ITEM_TYPE).toInt() ==
    ...
    if (isPoint || isCurve) {
      ...
      if (show && !showAll) {
        ...
        if (isPoint) {
          ...
          QString curveNameGot =
              Point::curveNameFromPointIdentifier(identifier);
...
void GraphicsScene::updateGraphicsLinesToMatchGraphicsPoints(
  ...
  if (transformation.transformIsDefined()) {
    ...
    // Ordinals must be updated to reflect reordering that may have resulted
    // from dragging points
    m_graphicsLinesForCurves.updatePointOrdinalsAfterDrag(curveStyles,
                                                          transformation);

    // Recompute the lines one time for efficiency
    m_graphicsLinesForCurves.updateGraphicsLinesToMatchGraphicsPoints(
        curveStyles);
  }
}
...
void GraphicsScene::updatePointMembership(CmdMediator &cmdMediator) {
  ...
  CallbackSceneUpdateAfterCommand ftor(m_graphicsLinesForCurves, *this,
                                       cmdMediator.document());
  Functor2wRet<const QString &, const Point &, CallbackSearchReturn>
      ftorWithCallback =
          functor_ret(ftor, &CallbackSceneUpdateAfterCommand::callback);
  ...
  // 2) Add new points that were just created in the Document. The new points
  // are marked as Wanted
  ...
  m_graphicsLinesForCurves.lineMembershipPurge(
      cmdMediator.document().modelCurveStyles());
}
