...
/// This class stores the GraphicsLine objects for one Curve. The container is a
/// QMap since that container
...
public:
  ...
  /// Add new line.
  ///
  /// The GraphicsPoint arguments are not const since this line binds to the
  /// points, so dragging points also drags the lines.
  /// The ordinal is already in the GraphicsPoint as DATA_KEY_ORDINAL
  void addPoint(const QString &pointIdentifier, double ordinal,
                GraphicsPoint &point);

  ///  Get ordinal for specified identifier
  double identifierToOrdinal(const QString &identifier) const;

  /// Mark the end of addPoint calls. Remove stale lines, insert missing lines,
  /// and draw the graphics lines
  ...
  /// Remove the specified point. The act of deleting it will automatically
  /// remove it from the GraphicsScene
  ...
  /// Update the GraphicsScene with the specified Point from the Document. If it
  /// does not exist yet in the scene, we add it
  void updateAfterCommand(GraphicsScene &scene, const PointStyle &pointStyle,
                          const Point &point);

  /// Update the curve style for this curve
  void updateCurveStyle(const CurveStyle &curveStyle);

  /// Calls to moveLinesWithDraggedPoint have finished so update the lines
  /// correspondingly
  void updateGraphicsLinesToMatchGraphicsPoints(const LineStyle &lineStyle);

  /// See GraphicsScene::updateOrdinalsAfterDrag. Pretty much the same steps as
...
private:
  ...
  bool needOrdinalRenumbering()
      const; // True if m_graphicsPoints ordinal keys need renumbering
