...
const QString
    SERVER_URL("http://engaugedigitizer.net/receive_crash_report.php");

NetworkClient::NetworkClient(QObject *parent) : QNetworkAccessManager(parent) {
...
QString NetworkClient::cleanXml(const QString &before) {
  ...
  // Remove characters that are known to break the xml parsing in Document. Bad
  // characters AFTER the
  // CDATA will break the parsing of the CDATA, which is a very hard bug to
  // track down, so this
...
void NetworkClient::slotFinished(QNetworkReply *reply) { reply->deleteLater(); }
