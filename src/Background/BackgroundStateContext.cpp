...
BackgroundStateContext::BackgroundStateContext(MainWindow &mainWindow)
  ...
  m_states.insert(BACKGROUND_STATE_CURVE,
                  new BackgroundStateCurve(*this, mainWindow.scene()));
  m_states.insert(BACKGROUND_STATE_NONE,
                  new BackgroundStateNone(*this, mainWindow.scene()));
  m_states.insert(BACKGROUND_STATE_ORIGINAL,
                  new BackgroundStateOriginal(*this, mainWindow.scene()));
  m_states.insert(BACKGROUND_STATE_UNLOADED,
                  new BackgroundStateUnloaded(*this, mainWindow.scene()));
  ENGAUGE_ASSERT(m_states.size() == NUM_BACKGROUND_STATES);

  m_currentState =
      NUM_BACKGROUND_STATES; // Value that forces a transition right away
...
void BackgroundStateContext::close() {
  ...
  // It is safe to transition to the new state immediately since no
  // BackgroundState classes are on the stack
  requestStateTransition(BACKGROUND_STATE_UNLOADED);
  completeRequestedStateTransitionIfExists();
}
...
void BackgroundStateContext::fitInView(GraphicsView &view) {
  ...
  // After initialization, we should be in unloaded state or some other equally
  // valid state
  ENGAUGE_ASSERT(m_currentState != NUM_BACKGROUND_STATES);

  const QGraphicsPixmapItem *imageItem =
      &m_states[BACKGROUND_STATE_CURVE]->imageItem();
...
void BackgroundStateContext::requestStateTransition(
...
void BackgroundStateContext::setBackgroundImage(
    BackgroundImage backgroundImage) {
  LOG4CPP_INFO_S((*mainCat))
      << "BackgroundStateContext::setBackgroundImage"
      << " background="
      << backgroundImageToString(backgroundImage).toLatin1().data();
...
void BackgroundStateContext::setCurveSelected(
  ...
  for (int backgroundState = 0; backgroundState < NUM_BACKGROUND_STATES;
       backgroundState++) {

    m_states[backgroundState]->setCurveSelected(
        transformation, modelGridRemoval, modelColorFilter, curveSelected);
  }
}

void BackgroundStateContext::setPixmap(
    const Transformation &transformation,
    const DocumentModelGridRemoval &modelGridRemoval,
    const DocumentModelColorFilter &modelColorFilter,
  ...
  LOG4CPP_INFO_S((*mainCat))
      ...
      << " currentState="
      << m_states[m_currentState]->state().toLatin1().data();

  for (int backgroundState = 0; backgroundState < NUM_BACKGROUND_STATES;
       backgroundState++) {

    m_states[backgroundState]->setPixmap(transformation, modelGridRemoval,
                                         modelColorFilter, pixmapOriginal);
  }
}

void BackgroundStateContext::updateColorFilter(
    const Transformation &transformation,
    const DocumentModelGridRemoval &modelGridRemoval,
    const DocumentModelColorFilter &modelColorFilter) {
  ...
  for (int backgroundState = 0; backgroundState < NUM_BACKGROUND_STATES;
    ...
    m_states[backgroundState]->updateColorFilter(
        transformation, modelGridRemoval, modelColorFilter);
  }
}
