...
BackgroundStateAbstractBase::BackgroundStateAbstractBase(
    ...
    : m_context(context), m_scene(scene), m_imageItem(0) {
...
QImage BackgroundStateAbstractBase::image() const { return m_image; }

QGraphicsPixmapItem &BackgroundStateAbstractBase::imageItem() const {
  return *m_imageItem;
}

GraphicsScene &BackgroundStateAbstractBase::scene() { return m_scene; }
...
void BackgroundStateAbstractBase::setProcessedPixmap(const QPixmap &pixmap) {
  ...
  // Reset scene rectangle or else small image after large image will be
  // off-center
