...
/// Class that displays the current Segment Filter in a MainWindow toolbar. A
/// gradient is displayed. No border
/// is drawn so the appearance is consistent with ViewPointStyle which would not
/// work with a border.
...
public:
  ...
  /// Apply the color filter of the currently selected curve. The pixmap is
  /// included so the background color can be computed.
...
private:
  QColor colorFromSetting(ColorFilterMode colorFilterMode, int foreground,
                          int hue, int intensity, int saturation,
                          int value) const;
