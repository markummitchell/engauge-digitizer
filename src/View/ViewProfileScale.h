...
/// Linear horizontal scale, with the spectrum reflecting the active filter
/// parameter.
class ViewProfileScale : public QLabel {
public:
  /// Single constructor.
  explicit ViewProfileScale(int minimumWidth, QWidget *parent = 0);
...
private:
  ...
  void paintOneSpectrum(const QColor &colorStart,
                        const QColor &colorStop); // Generic processing for
                                                  // painting with only one
                                                  // spectrum
