...
// Insert a little space on the left and right so first and last points are
// visible. Although the
// ViewProfile will no longer be exactly aligned with the ViewScale underneath,
// the difference is insignificant
const double SLOP_ON_SIDES = 0.5;

ViewProfile::ViewProfile(QGraphicsScene *scene, int minimumWidth,
                         QWidget *parent)
...
void ViewProfile::refit() {
  ...
  QRectF bounds =
      QRectF(VIEW_PROFILE_X_MIN - SLOP_ON_SIDES, VIEW_PROFILE_Y_MIN,
             VIEW_PROFILE_X_MAX + 2 * SLOP_ON_SIDES, VIEW_PROFILE_Y_MAX);
  fitInView(bounds);
  setSceneRect(bounds);
}
