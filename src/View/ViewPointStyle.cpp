...
// Use solid background since transparency approach never worked, even with an
// alpha channel
...
QPixmap ViewPointStyle::pixmapForCurrentSettings() const {
  ...
  // Resize polygon to fit icon, by builiding a new scaled polygon from the
  // unscaled polygon
  ...
  for (int i = 0; i < polygonUnscaled.length(); i++) {
    ...
    polygonScaled.append(
        QPointF((width() - 1) * (pOld.x() - xMinGot) / (xMaxGot - xMinGot),
