...
void DigitizeStateSegment::end() {
  ...
  SegmentFactory segmentFactory((QGraphicsScene &)scene, context().isGnuplot());
...
void DigitizeStateSegment::handleCurveChange(CmdMediator *cmdMediator) {
  ...
  // Create new segments
  segmentFactory.makeSegments(img, cmdMediator->document().modelSegments(),
                              m_segments);
  ...
  for (itr = m_segments.begin(); itr != m_segments.end(); itr++) {
    ...
    connect(segment, SIGNAL(signalMouseClickOnSegment(QPointF)), this,
            SLOT(slotMouseClickOnSegment(QPointF)));
  }
}
...
Segment *DigitizeStateSegment::segmentFromSegmentStart(
    const QPointF &posSegmentStart) const {
...
void DigitizeStateSegment::slotMouseClickOnSegment(QPointF posSegmentStart) {
  ...
  // Generate point coordinates. Nothing is created in the GraphicsScene at this
  // point
  GraphicsScene &scene = context().mainWindow().scene();
  SegmentFactory segmentFactory((QGraphicsScene &)scene, context().isGnuplot());

  QList<QPoint> points = segmentFactory.fillPoints(
      m_cmdMediator->document().modelSegments(), segments);
  ...
  for (itr = points.begin(); itr != points.end(); itr++) {
    ...
    QPoint point = *itr;
    ordinals << ordinalGenerator.generateCurvePointOrdinal(
        document, transformation, point, activeCurve());
  }

  // Create command to add points
  QUndoCommand *cmd = new CmdAddPointsGraph(
      context().mainWindow(), document,
      context().mainWindow().selectedGraphCurve(), points, ordinals);
  context().appendNewCmd(m_cmdMediator, cmd);
}

QString DigitizeStateSegment::state() const { return "DigitizeStateSegment"; }

void DigitizeStateSegment::updateModelDigitizeCurve(
...
void DigitizeStateSegment::updateModelSegments(
