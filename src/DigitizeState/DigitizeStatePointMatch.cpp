...
DigitizeStatePointMatch::DigitizeStatePointMatch(DigitizeStateContext &context)
    : DigitizeStateAbstractBase(context), m_outline(0), m_candidatePoint(0) {}
...
void DigitizeStatePointMatch::createTemporaryPoint(CmdMediator *cmdMediator,
  ...
  const DocumentModelPointMatch &modelPointMatch =
      cmdMediator->document().modelPointMatch();
  ...
  GraphicsPoint *point = context().mainWindow().scene().createPoint(
      Point::temporaryPointIdentifier(), pointStyle, posScreen);

  context().mainWindow().scene().removeTemporaryPointIfExists(); // Only one
                                                                 // temporary
                                                                 // point at a
                                                                 // time is
                                                                 // allowed

  context().mainWindow().scene().addTemporaryPoint(
      Point::temporaryPointIdentifier(), point);
...
QList<PointMatchPixel> DigitizeStatePointMatch::extractSamplePointPixels(
    const QImage &img, const DocumentModelPointMatch &modelPointMatch,
    const QPointF &posScreen) const {
  LOG4CPP_INFO_S((*mainCat))
      << "DigitizeStatePointMatch::extractSamplePointPixels";

  // All points inside modelPointMatch.maxPointSize() are collected, whether or
  // not they
  // are on or off. Originally only the on points were collected, but obvious
  // mismatches
  // were happening (example, 3x3 point would appear to be found in several
  // places inside 8x32 rectangle)
  ...
  for (int xOffset = -radiusMax; xOffset <= radiusMax; xOffset++) {
    for (int yOffset = -radiusMax; yOffset <= radiusMax; yOffset++) {
      ...
      if (radius <= radiusMax) {
        ...
        bool pixelIsOn = colorFilter.pixelFilteredIsOn(img, x, y);

        PointMatchPixel point(xOffset, yOffset, pixelIsOn);
...
void DigitizeStatePointMatch::handleKeyPress(
    CmdMediator *cmdMediator, Qt::Key key, bool /* atLeastOneSelectedItem */) {
  LOG4CPP_INFO_S((*mainCat))
      << "DigitizeStatePointMatch::handleKeyPress"
      << " key=" << QKeySequence(key).toString().toLatin1().data();

  // The selected key button has to be compatible with
  // GraphicsView::keyPressEvent
  if (key == Qt::Key_Right) {

    promoteCandidatePointToPermanentPoint(
        cmdMediator); // This removes the current temporary point
...
void DigitizeStatePointMatch::handleMouseMove(CmdMediator *cmdMediator,
                                              QPointF posScreen) {
  //  LOG4CPP_DEBUG_S ((*mainCat)) <<
  //  "DigitizeStatePointMatch::handleMouseMove";
  ...
  bool pixelShouldBeOn =
      pixelIsOnInImage(img, posScreen.x(), posScreen.y(), radiusLimit);

  QColor penColorIs = m_outline->pen().color();
  bool pixelIsOn = (penColorIs.red() !=
                    penColorIs.green()); // Considered on if not gray scale
...
void DigitizeStatePointMatch::handleMouseRelease(CmdMediator *cmdMediator,
  ...
  createPermanentPoint(cmdMediator, posScreen);

  findPointsAndShowFirstCandidate(cmdMediator, posScreen);
}

void DigitizeStatePointMatch::findPointsAndShowFirstCandidate(
  ...
  QList<PointMatchPixel> samplePointPixels =
      extractSamplePointPixels(img, modelPointMatch, posScreen);

  QString curveName = activeCurve();
  const Document &doc = cmdMediator->document();
  const Curve *curve = doc.curveForCurveName(curveName);

  // The point match algorithm takes a few seconds, so set the cursor so user
  // knows we are processing
  QApplication::setOverrideCursor(Qt::WaitCursor);

  PointMatchAlgorithm pointMatchAlgorithm(context().isGnuplot());
  m_candidatePoints = pointMatchAlgorithm.findPoints(
      samplePointPixels, img, modelPointMatch, curve->points());

  QApplication::restoreOverrideCursor(); // Heavy duty processing has finished
  context().mainWindow().showTemporaryMessage(
      "Right arrow adds next matched point");

  popCandidatePoint(cmdMediator);
}

bool DigitizeStatePointMatch::pixelIsOnInImage(const QImage &img, int x, int y,
  ...
  for (int xOffset = -radiusLimit; xOffset <= radiusLimit; xOffset++) {
    for (int yOffset = -radiusLimit; yOffset <= radiusLimit; yOffset++) {
      ...
      if (radius <= radiusLimit) {
        ...
        if ((0 <= xNearby) && (0 <= yNearby) && (xNearby < img.width()) &&
            (yNearby < img.height())) {

          if (filter.pixelFilteredIsOn(img, xNearby, yNearby)) {
...
void DigitizeStatePointMatch::popCandidatePoint(CmdMediator *cmdMediator) {
  ...
  if (m_candidatePoints.count() > 0) {
    ...
    createTemporaryPoint(cmdMediator, posScreen);

  } else {

    // No more points. Inform user
    QMessageBox::information(0, QObject::tr("Point Match"),
...
void DigitizeStatePointMatch::promoteCandidatePointToPermanentPoint(
    CmdMediator *cmdMediator) {
  createPermanentPoint(cmdMediator, m_posCandidatePoint);
}
...
void DigitizeStatePointMatch::updateModelDigitizeCurve(
    CmdMediator * /* cmdMediator */,
...
void DigitizeStatePointMatch::updateModelSegments(
