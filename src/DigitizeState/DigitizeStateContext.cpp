...
DigitizeStateContext::DigitizeStateContext(MainWindow &mainWindow,
                                           QGraphicsView &view, bool isGnuplot)
    : m_mainWindow(mainWindow), m_view(view), m_imageIsLoaded(false),
  ...
  m_states.insert(DIGITIZE_STATE_COLOR_PICKER,
                  new DigitizeStateColorPicker(*this));
  ...
  m_states.insert(DIGITIZE_STATE_POINT_MATCH,
                  new DigitizeStatePointMatch(*this));
  ...
  m_currentState =
      NUM_DIGITIZE_STATES; // Value that forces a transition right away
...
void DigitizeStateContext::completeRequestedStateTransitionIfExists(
  ...
  if (m_currentState != m_requestedState) {
    ...
    m_states[m_requestedState]->begin(cmdMediator, previousState);

    // If transition was triggered from inside the state machine then MainWindow
    // controls need to be set accordingly
    // as if user had clicked on a digitize button
    mainWindow().updateDigitizeStateIfSoftwareTriggered(m_requestedState);
  }
}

void DigitizeStateContext::handleContextMenuEvent(
...
void DigitizeStateContext::handleKeyPress(CmdMediator *cmdMediator, Qt::Key key,
                                          bool atLeastOneSelectedItem) {
  m_states[m_currentState]->handleKeyPress(cmdMediator, key,
...
void DigitizeStateContext::handleMouseMove(CmdMediator *cmdMediator,
                                           QPointF pos) {
  m_states[m_currentState]->handleMouseMove(cmdMediator, pos);
...
void DigitizeStateContext::handleMousePress(CmdMediator *cmdMediator,
                                            QPointF pos) {
  m_states[m_currentState]->handleMousePress(cmdMediator, pos);
...
void DigitizeStateContext::handleMouseRelease(CmdMediator *cmdMediator,
                                              QPointF pos) {
  m_states[m_currentState]->handleMouseRelease(cmdMediator, pos);
...
void DigitizeStateContext::handleSetOverrideCursor(CmdMediator *cmdMediator,
  ...
  m_states[m_currentState]->handleSetOverrideCursor(cmdMediator, cursor);
}

bool DigitizeStateContext::isGnuplot() const { return m_isGnuplot; }

MainWindow &DigitizeStateContext::mainWindow() { return m_mainWindow; }

const MainWindow &DigitizeStateContext::mainWindow() const {
  return m_mainWindow;
}

void DigitizeStateContext::requestDelayedStateTransition(
    DigitizeState digitizeState) {
  m_requestedState = digitizeState;
}

void DigitizeStateContext::requestImmediateStateTransition(
    CmdMediator *cmdMediator, DigitizeState digitizeState) {
  m_requestedState = digitizeState;
  completeRequestedStateTransitionIfExists(cmdMediator);
}
...
void DigitizeStateContext::resetOnLoad(CmdMediator *cmdMediator) {
  ...
  // Reset current state. At this point, the current state is
  // DIGITIZE_STATE_EMPTY when opening the first document
  // so for consistency we always reset it so succeeding documents work the same
  // way
...
void DigitizeStateContext::updateModelDigitizeCurve(
    CmdMediator *cmdMediator,
...
void DigitizeStateContext::updateModelSegments(
...
QGraphicsView &DigitizeStateContext::view() { return m_view; }
