...
QString DigitizeStateAxis::activeCurve() const { return AXIS_CURVE_NAME; }
...
void DigitizeStateAxis::createTemporaryPoint(CmdMediator *cmdMediator,
  ...
  GraphicsPoint *point = context().mainWindow().scene().createPoint(
      Point::temporaryPointIdentifier(), pointStyleAxes, posScreen);
...
void DigitizeStateAxis::handleMouseRelease(CmdMediator *cmdMediator,
  ...
  if (context().mainWindow().transformIsDefined()) {
    ...
    QMessageBox::warning(0, QObject::tr("Application"),
                         QObject::tr("Three axis points have been defined, and "
                                     "no more are needed or allowed."));
    ...
    context().mainWindow().scene().removePoint(
        Point::temporaryPointIdentifier());

    if (rtn == QDialog::Accepted) {

      // User wants to add this axis point, but let's perform sanity checks
      // first

      bool isError;
      QString errorMessage;
      int nextOrdinal =
          cmdMediator->document().nextOrdinalForCurve(AXIS_CURVE_NAME);

      cmdMediator->document().checkAddPointAxis(posScreen, posGraph, isError,
                                                errorMessage, isXOnly);

      if (isError) {

        QMessageBox::warning(0, QObject::tr("Application"), errorMessage);
        ...
        QUndoCommand *cmd =
            new CmdAddPointAxis(context().mainWindow(), document, posScreen,
                                posGraph, nextOrdinal, isXOnly);
...
QString DigitizeStateAxis::state() const { return "DigitizeStateAxis"; }

void DigitizeStateAxis::updateModelDigitizeCurve(
...
void DigitizeStateAxis::updateModelSegments(
