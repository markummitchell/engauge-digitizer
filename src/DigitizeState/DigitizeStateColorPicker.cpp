...
void DigitizeStateColorPicker::begin(CmdMediator *cmdMediator,
  ...
  m_previousBackground = context().mainWindow().selectOriginal(
      BACKGROUND_IMAGE_ORIGINAL); // Only makes sense to have original image
                                  // with all its colors

  context().mainWindow().updateViewsOfSettings(activeCurve());
}

bool DigitizeStateColorPicker::computeFilterFromPixel(
    CmdMediator *cmdMediator, const QPointF &posScreen,
    const QString &curveName, DocumentModelColorFilter &modelColorFilterAfter) {
  LOG4CPP_INFO_S((*mainCat))
      << "DigitizeStateColorPicker::computeFilterFromPixel";

  bool rtn = false;

  // Filter for background color now, and then later, once filter mode is set,
  // processing of image
  ColorFilter filter;
  ...
  QColor pixel;
  rtn = findNearestNonBackgroundPixel(cmdMediator, image, posScreenPlusHalf,
                                      rgbBackground, pixel);
  if (rtn) {

    // The choice of which filter mode to use is determined, currently, by the
    // selected pixel. This
    // could be maybe made smarter by looking at other pixels, or even the
    // entire image
    ...
    // Bin for pixel
    int pixelBin = filterHistogram.binFromPixel(
        filter, modelColorFilterAfter.colorFilterMode(curveName), pixel,
        rgbBackground);

    // Identify the entire width of the peak that the selected pixel belongs to.
    // Go in both directions until the count
    ...
    int lowerValue = filterHistogram.valueFromBin(
        filter, modelColorFilterAfter.colorFilterMode(curveName), lowerBin);
    int upperValue = filterHistogram.valueFromBin(
        filter, modelColorFilterAfter.colorFilterMode(curveName), upperBin);

    saveLowerValueUpperValue(modelColorFilterAfter, curveName, lowerValue,
                             upperValue);
    ...
    QMessageBox::warning(
        0, QObject::tr("Color Picker"),
        QObject::tr("Sorry, but the color picker point must be near a "
                    "non-background pixel. Please try again."));
  }
...
QCursor
DigitizeStateColorPicker::cursor(CmdMediator * /* cmdMediator */) const {
  ...
  return QCursor(bitmap, bitmapMask, HOT_X_IN_BITMAP, HOT_Y_IN_BITMAP);
}

void DigitizeStateColorPicker::end() {
  LOG4CPP_INFO_S((*mainCat)) << "DigitizeStateColorPicker::end";

  // Restore original background. The state transition was triggered earlier by
  // either the user selecting
  // a valid point, or by user clicking on another digitize state button
  context().mainWindow().selectOriginal(m_previousBackground);
}

bool DigitizeStateColorPicker::findNearestNonBackgroundPixel(
    CmdMediator *cmdMediator, const QImage &image,
    const QPointF &posScreenPlusHalf, const QRgb &rgbBackground,
    QColor &pixel) {
...
void DigitizeStateColorPicker::handleKeyPress(
    CmdMediator * /* cmdMediator */, Qt::Key key,
...
void DigitizeStateColorPicker::handleMouseMove(CmdMediator * /* cmdMediator */,
  ...
  //  "DigitizeStateColorPicker::handleMouseMove";
}
...
void DigitizeStateColorPicker::handleMouseRelease(CmdMediator *cmdMediator,
  ...
  DocumentModelColorFilter modelColorFilterBefore =
      cmdMediator->document().modelColorFilter();
  DocumentModelColorFilter modelColorFilterAfter =
      cmdMediator->document().modelColorFilter();
  if (computeFilterFromPixel(cmdMediator, posScreen,
                             context().mainWindow().selectedGraphCurve(),
                             modelColorFilterAfter)) {

    // Trigger a state transition. The background restoration will be handled by
    // the end method
    context().requestDelayedStateTransition(m_previousDigitizeState);

    // Create command to change segment filter
    QUndoCommand *cmd = new CmdSettingsColorFilter(
        context().mainWindow(), cmdMediator->document(), modelColorFilterBefore,
        modelColorFilterAfter);
    context().appendNewCmd(cmdMediator, cmd);
  }
}

void DigitizeStateColorPicker::saveLowerValueUpperValue(
    DocumentModelColorFilter &modelColorFilterAfter, const QString &curveName,
    double lowerValue, double upperValue) {
  switch (modelColorFilterAfter.colorFilterMode(curveName)) {
  case COLOR_FILTER_MODE_FOREGROUND:
    modelColorFilterAfter.setForegroundLow(curveName, lowerValue);
    modelColorFilterAfter.setForegroundHigh(curveName, upperValue);
    break;

  case COLOR_FILTER_MODE_HUE:
    modelColorFilterAfter.setHueLow(curveName, lowerValue);
    modelColorFilterAfter.setHueHigh(curveName, upperValue);
    break;

  case COLOR_FILTER_MODE_INTENSITY:
    modelColorFilterAfter.setIntensityLow(curveName, lowerValue);
    modelColorFilterAfter.setIntensityHigh(curveName, upperValue);
    break;

  case COLOR_FILTER_MODE_SATURATION:
    modelColorFilterAfter.setSaturationLow(curveName, lowerValue);
    modelColorFilterAfter.setSaturationHigh(curveName, upperValue);
    break;

  case COLOR_FILTER_MODE_VALUE:
    modelColorFilterAfter.setValueLow(curveName, lowerValue);
    modelColorFilterAfter.setValueHigh(curveName, upperValue);
    break;
...
void DigitizeStateColorPicker::updateModelDigitizeCurve(
    CmdMediator * /* cmdMediator */,
...
void DigitizeStateColorPicker::updateModelSegments(
