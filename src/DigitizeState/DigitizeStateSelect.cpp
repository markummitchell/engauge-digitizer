...
void DigitizeStateSelect::handleKeyPress(CmdMediator *cmdMediator, Qt::Key key,
...
void DigitizeStateSelect::handleMousePress(CmdMediator * /* cmdMediator */,
  ...
  // Note that GraphicsView has already called
  // GraphicsPointAbstract::resetPositionHasChanged on all items
...
void DigitizeStateSelect::handleMouseRelease(CmdMediator *cmdMediator,
  ...
  QStringList positionHasChangedIdentifers =
      context().mainWindow().scene().positionHasChangedPointIdentifiers();

  bool positionHasChanged = (positionHasChangedIdentifers.count() > 0);

  if (positionHasChanged && (deltaScreen.x() != 0 || deltaScreen.y() != 0)) {

    QString moveText = moveTextFromDeltaScreen(deltaScreen);

    // Create command to move points
    CmdMoveBy *cmd =
        new CmdMoveBy(context().mainWindow(), cmdMediator->document(),
                      deltaScreen, moveText, positionHasChangedIdentifers);
    context().appendNewCmd(cmdMediator, cmd);

  } else {

    // Selection probably changed so update the MainWindow controls (especially
    // Cut)
    context().mainWindow().updateAfterMouseRelease();
  }
}

void DigitizeStateSelect::keyPressArrow(CmdMediator *cmdMediator, Qt::Key key) {
  QPointF deltaScreen;
  QString moveText;
  switch (key) {
  case Qt::Key_Down:
  ...
  // Create command to move points
  CmdMoveBy *cmd = new CmdMoveBy(
      context().mainWindow(), cmdMediator->document(), deltaScreen, moveText,
      context().mainWindow().scene().selectedPointIdentifiers());
  context().appendNewCmd(cmdMediator, cmd);
}

QString
DigitizeStateSelect::moveTextFromDeltaScreen(const QPointF &deltaScreen) {
  QString moveText;
...
QString DigitizeStateSelect::state() const { return "DigitizeStateSelect"; }
...
void DigitizeStateSelect::updateModelDigitizeCurve(
    CmdMediator * /* cmdMediator */,
    const DocumentModelDigitizeCurve & /*modelDigitizeCurve */) {
  LOG4CPP_INFO_S((*mainCat)) << "DigitizeStateSelect::updateModelDigitizeCurve";
}

void DigitizeStateSelect::updateModelSegments(
