...
const int TEMPORARY_MESSAGE_LIFETIME = 5000; // Milliseconds. Two seconds is too
                                             // fast even when the text is
                                             // anticipated

const int MIN_WIDTH_COMBO_UNITS = 160;
const int MAX_WIDTH_GROUP_UNITS = 400;
const int MAX_SIZE_EDIT_COORDS = 550; // Need lots of space in case date/time
                                      // and degrees/minutes/seconds are used
                                      // simultaneously
const int MAX_HEIGHT_EDIT_COORDS = 24;

StatusBar::StatusBar(QStatusBar &statusBar)
    : m_statusBar(statusBar), m_statusBarMode(STATUS_BAR_MODE_ALWAYS),
      m_timer(0) {
  createZoom();
  createGroupUnits();

  connect(&m_statusBar, SIGNAL(messageChanged(const QString &)), this,
          SLOT(slotStatusBarChanged(const QString &)));
...
void StatusBar::createGroupUnits() {
  ...
  m_cmbUnits->addItem(LABEL_COORDS_SCREEN,
                      QVariant(STATUS_BAR_UNITS_COORDS_SCREEN));
  m_cmbUnits->addItem(LABEL_COORDS_GRAPH,
                      QVariant(STATUS_BAR_UNITS_COORDS_GRAPH));
  m_cmbUnits->addItem(LABEL_RESOLUTION_GRAPH,
  ...
  m_cmbUnits->setWhatsThis(tr("Select Cursor Coordinate Values\n\n"
                              "Values at cursor coordinates to display. "
                              "Coordinates are in screen (pixels) or "
                              "graph units. Resolution (which is the number of "
                              "graph units per pixel) is "
                              "in graph units. Graph units are only available "
                              "after axis points have been defined."));
  connect(m_cmbUnits, SIGNAL(activated(const QString &)), this,
          SLOT(slotComboUnits(
  ...
  m_editCoords->setWhatsThis(
      tr("Cursor Coordinate Values\n\n"
         "Values at cursor coordinates. Coordinates are in screen (pixels) or "
         "graph units. Resolution (which is the number of graph units per "
         "pixel) is "
         "in graph units. Graph units are only available after axis points "
         "have been defined."));
...
void StatusBar::createZoom() {
  ...
  m_cmbZoom->setWhatsThis(
      tr("Select Zoom\n\n"
         "Points can be more accurately placed by zooming in."));
  // Zoom combobox must use currentTextChanged rather than activated or else
  // fill-zoom-at-startup never takes effect
  connect(m_cmbZoom, SIGNAL(currentTextChanged(const QString &)), this,
          SLOT(slotComboZoom(const QString &)));
...
void StatusBar::setCoordinates(const QString &coordsScreen,
  ...
  //                               << " screen=" << coordsScreen.toLatin1
  //                               ().data ()
  //                               << " graph=" << coordsGraph.toLatin1 ().data
  //                               ()
  //                               << " resolution=" << resolutionGraph.toLatin1
  //                               ().data ();
...
void StatusBar::showTemporaryMessage(const QString &message) {
  LOG4CPP_DEBUG_S((*mainCat)) << "StatusBar::showTemporaryMessage message="
                              << message.toLatin1().data();

  if (m_statusBarMode != STATUS_BAR_MODE_NEVER) {
    if (m_statusBarMode == STATUS_BAR_MODE_TEMPORARY) {
      // Calling m_statusBar.show here will have no effect since this is called
      // while processing a signal. Use a timer to
...
void StatusBar::slotComboUnits(const QString &text) {
  LOG4CPP_DEBUG_S((*mainCat)) << "StatusBar::slotComboUnits text="
                              << text.toLatin1().data();
...
void StatusBar::slotComboZoom(const QString &text) {
  LOG4CPP_DEBUG_S((*mainCat)) << "StatusBar::slotComboZoom text="
                              << text.toLatin1().data();
...
void StatusBar::slotStatusBarChanged(const QString &message) {
  LOG4CPP_DEBUG_S((*mainCat)) << "StatusBar::slotStatusBarChanged message="
                              << message.toLatin1().data();
